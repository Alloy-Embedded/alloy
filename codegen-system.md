# Alloy: Sistema de Geração de Código

## 1. Visão Geral

O sistema de geração de código do Alloy é uma ferramenta **interna** e **transparente** que permite suportar centenas de microcontroladores sem complexidade para o usuário final.
...
namespace alloy::platform::{{ mcu.family|lower }} {
...
function(alloy_generate_code)
    ...
    # Paths
    set(DATABASE_FILE "${ALLOY_ROOT}/tools/codegen/database/families/${GEN_FAMILY}.json")
    set(OUTPUT_DIR "${GEN_OUTPUT_DIR}")
    set(GENERATOR_SCRIPT "${ALLOY_ROOT}/tools/codegen/generator.py")
    ...
    set(ALLOY_GENERATED_DIR ${OUTPUT_DIR} PARENT_SCOPE)
    set(ALLOY_STARTUP_FILE "${OUTPUT_DIR}/startup.cpp" PARENT_SCOPE)
    set(ALLOY_LINKER_SCRIPT "${OUTPUT_DIR}/${GEN_MCU}.ld" PARENT_SCOPE)
endfunction()
```
...
alloy_generate_code(
    MCU ${ALLOY_MCU}
    FAMILY ${ALLOY_MCU_FAMILY}
    OUTPUT_DIR ${CMAKE_BINARY_DIR}/generated/${ALLOY_MCU}
)

# Adicionar código gerado ao build
add_library(alloy_platform STATIC
    ${ALLOY_GENERATED_DIR}/startup.cpp
    ${ALLOY_GENERATED_DIR}/vectors.cpp
    ${ALLOY_GENERATED_DIR}/system.cpp
)

target_include_directories(alloy_platform PUBLIC
    ${ALLOY_GENERATED_DIR}
)

# Configurar linker script
set(CMAKE_EXE_LINKER_FLAGS
    "${CMAKE_EXE_LINKER_FLAGS} -T${ALLOY_LINKER_SCRIPT}"
)
```
...
def main():
    parser = argparse.ArgumentParser(description='Alloy Code Generator')
...
TEST_OUTPUT="/tmp/alloy_codegen_test"

echo "Testing Alloy Code Generator..."
...
export module alloy.hal.adc;

export namespace alloy::hal {
...
module alloy.hal.adc:stm32f4;

import alloy.platform;  // Registros gerados!

export namespace alloy::hal::stm32f4 {
