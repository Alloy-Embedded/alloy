...
The pin file ships `arm-none-eabi-gcc`, `openocd`, `xtensa-esp-elf-gcc` (Xtensa ESP --
unified driver covering ESP32 LX6 and ESP32-S2/S3 LX7 since 13.x), and
`riscv32-esp-elf-gcc` (ESP32-C3) entries by default; tests and maintainers can override
the file via the `ALLOY_TOOLCHAIN_PINS` environment variable. The Espressif pins cover
the bare-metal compile path only; pulling the full ESP-IDF framework (FreeRTOS, WiFi,
BLE, NVS) is tracked separately and not implied by installing the toolchain.
