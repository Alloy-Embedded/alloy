...
| `smp` | `compile-review` | `CrossCoreChannel<T,N>` (acquire/release SPSC), `SharedEvent`, `SharedScheduler<N,B>` (CoreAffinity::Core0/Core1/Any, TAS spinlock), `board::start_app_cpu()` (ESP32 DPORT mailbox), `board::launch_core1()` (RP2040 SIO FIFO 5-word handshake). Host unit tests: `test_cross_core_channel` (1M items, two threads), `test_shared_scheduler` (pinned + Any affinity, two threads). Hardware validation pending on ESP32 and RP2040. See [SMP.md](SMP.md). |
| `async` | `compile-review` | UART async (DMA): hardware-validated on Nucleo (foundational). UART interrupt-event wait: `async::uart::wait_for<InterruptKind>` + `uart_event::token<P,Kind>` added; compile-pinned in `test_async_peripherals.cpp`. SPI / I2C / ADC / Timer / GPIO async: runtime wrappers + per-peripheral completion tokens (`dma_event`, `i2c_event`, `adc_event`, `timer_event`, `gpio_event`) ship in `src/runtime/async_{spi,i2c,adc,timer,gpio}.hpp`. Per-vendor ISR signal hooks for STM32/SAME70/ESP32 are tracked separately under follow-up `add-async-hal-vendor-isr-hooks`. See [ASYNC.md](ASYNC.md). |
| `filesystem` | `compile-review` | `BlockDevice<T>` concept + `Filesystem<Backend>` / `File<Backend>` templates; littlefs (v2.8.1 pinned) and FatFS (R0.15c pinned) backends; W25Q SPI NOR adapter (`W25qBlockDevice<Spi, N, CsPolicy>`) and SD-card SPI adapter (`SdCard<Spi, CsPolicy>`, SDHC/SDXC; SDSC rejected with `NotSupported`). Heap-free, RTOS-free, `noexcept` throughout. Compile tests verify concept conformance for both adapters and the API surface for both backends. Examples `filesystem_littlefs` (W25Q128 over SAME70 SPI0) and `filesystem_fatfs_sdcard` build clean. Host unit tests + on-bench W25Q power-cut + 100 MB SD throughput pending. See [FILESYSTEM.md](FILESYSTEM.md). |
