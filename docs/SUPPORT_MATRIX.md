...
| `async` | `compile-review` | UART async (DMA): hardware-validated on Nucleo (foundational). SPI / I2C / ADC / Timer / GPIO async: runtime wrappers + per-peripheral completion tokens (`dma_event`, `i2c_event`, `adc_event`, `timer_event`, `gpio_event`) ship in `src/runtime/async_{spi,i2c,adc,timer,gpio}.hpp`; compile test `tests/compile_tests/test_async_peripherals.cpp` pins concept conformance against duck-typed mock handles. Per-vendor ISR signal hooks for STM32/SAME70/ESP32 are tracked separately under follow-up `add-async-hal-vendor-isr-hooks`. See [ASYNC.md](ASYNC.md). |
