...
- **WHAT DOES REACH THE ENGINE FROM A SHIPPED EXAMPLE**: the memory→peripheral
  ...
  ci.yml's `build` matrix for both boards, so no CI change was needed. There is
