...
| F4/F7 | `DMA.STM32DMA` exists | to be established: minimum credible leg is dma_uart-shaped (m2p via a stream, TCIF IRQ). Ring/HTIF fidelity unknown | **untested by us** — phase 3 budgets a model-probe day before promising the leg in CI |
...
| **3** | alloy-devices: `dma_routes` rename + shipped-chip regen; `st/dma_v2` curation; `st_dma_v2.hpp`; F7 board assignments | anchors 2.2/2.3 on `nucleo_f767zi` under Renode (after the model-probe day); same portable main.cpp, one config line changed |
