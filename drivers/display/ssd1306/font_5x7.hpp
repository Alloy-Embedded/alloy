...
// drivers/display/ssd1306/font_5x7.hpp
//
// Minimal 5x7 ASCII font for the SSD1306 seed driver. Each glyph is 5 bytes
// (one per column, LSB = top pixel). Covers 0x20..0x7E; unknown chars fall
// back to blank. Derived from the classic public-domain "font5x7" table
// (original attribution: Adafruit GFX / public domain ROM fonts), stripped
// to the ASCII printable range to keep the seed driver small.
...
inline constexpr std::uint8_t kFontFirstChar = 0x20;
inline constexpr std::uint8_t kFontLastChar = 0x7E;
inline constexpr std::uint8_t kFontGlyphWidth = 5;
inline constexpr std::uint8_t kFontGlyphHeight = 7;

inline constexpr std::array<std::array<std::uint8_t, kFontGlyphWidth>,
                            (kFontLastChar - kFontFirstChar) + 1>
    kFont5x7 = {{
...
[[nodiscard]] inline constexpr auto font_glyph(char c) -> const std::uint8_t* {
    auto code = static_cast<std::uint8_t>(c);
    if (code < kFontFirstChar || code > kFontLastChar) {
        return kFont5x7[0].data();  // blank
    }
    return kFont5x7[code - kFontFirstChar].data();
}
