// examples/driver_ina219_probe/main.cpp
//
// SAME70 Xplained Ultra — INA219 current/power monitor probe.
//
// Wiring (INA219 breakout → EXT1 header):
//   INA219 VCC  → 3V3   (EXT1 pin 20)
//   INA219 GND  → GND   (EXT1 pin 19)
//   INA219 SDA  → PA3   (EXT1 pin 11)   TWI0_SDA
//   INA219 SCL  → PA4   (EXT1 pin 12)   TWI0_SCL
//   INA219 A0   → GND   (I2C address 0x40)
//   INA219 A1   → GND
//   0.1Ω shunt between VIN+ and VIN−, load on VOUT.
//
// Expected UART output:
//   [ina219] booting
//   [ina219] init ok
//   [ina219] bus=X.XX V  shunt=X.XX mV  current=X.XX A  power=X.XX W
//   [ina219] PROBE PASS
...
#    error "driver_ina219_probe requires BOARD_UART_HEADER for the selected board"
#endif
#ifndef BOARD_I2C_HEADER
#    error "driver_ina219_probe requires BOARD_I2C_HEADER for the selected board"
#endif
...
namespace drv  = alloy::drivers::power::ina219;
...
int main() {
    ...
    uart::write_line(debug, "[ina219] booting");

    auto bus = board::make_i2c();
    if (bus.configure().is_err()) {
        uart::write_line(debug, "[ina219] bus configure failed");
        halt_blink(100);
    }

    // Default config: address=0x40, shunt=0.1Ω, max_amps=3.2A.
    drv::Device sensor{bus};
    if (sensor.init().is_err()) {
        uart::write_line(debug, "[ina219] init failed");
        halt_blink(100);
    }
    uart::write_line(debug, "[ina219] init ok");

    auto m = sensor.read();
    if (m.is_err()) {
        uart::write_line(debug, "[ina219] read failed");
        halt_blink(100);
    }

    const auto& meas = m.unwrap();

    // Build: "[ina219] bus=X.XX V  shunt=X.XX mV  current=X.XX A  power=X.XX W"
    char line[96] = "[ina219] bus=";
    std::size_t pos = 13;

    char num[16]{};
    fmt_fixed(meas.bus_voltage_v, 2, num, sizeof(num));
    append(line, sizeof(line), pos, num);
    append(line, sizeof(line), pos, " V  shunt=");
    fmt_fixed(meas.shunt_voltage_mv, 2, num, sizeof(num));
    append(line, sizeof(line), pos, num);
    append(line, sizeof(line), pos, " mV  current=");
    fmt_fixed(meas.current_a, 3, num, sizeof(num));
    append(line, sizeof(line), pos, num);
    append(line, sizeof(line), pos, " A  power=");
    fmt_fixed(meas.power_w, 3, num, sizeof(num));
    append(line, sizeof(line), pos, num);
    append(line, sizeof(line), pos, " W");
    line[pos] = '\0';

    uart::write_line(debug, line);
    uart::write_line(debug, "[ina219] PROBE PASS");
    halt_blink(500);
}
