...
enum class ResourceID : core::u8 { Display = 1, Sensor = 2, Logger = 3, Network = 4 };
...
constexpr bool task_a_lock_order_valid =
    has_consistent_lock_order<static_cast<core::u8>(ResourceID::Display),
...
constexpr bool task_b_lock_order_valid =
    has_consistent_lock_order<static_cast<core::u8>(ResourceID::Sensor),
...
constexpr bool task_c_lock_order_valid = has_consistent_lock_order<
    static_cast<core::u8>(ResourceID::Display), static_cast<core::u8>(ResourceID::Sensor),
    static_cast<core::u8>(ResourceID::Logger), static_cast<core::u8>(ResourceID::Network)>();
...
constexpr size_t total_queue_memory = priority_queue_size + sensor_queue_size + log_queue_size;

// Validate against RAM budget (e.g., 4KB for queues)
static_assert(
    queue_memory_fits_budget<4096, priority_queue_size, sensor_queue_size, log_queue_size>(),
    ...
    while (true) {
        ...
        if (result.is_ok()) {
            ...
            if (sensor_lock.is_locked()) {
                ...
                // Create log entry
                LogEntry log{.timestamp = data.timestamp,
                             .priority = 3,
...
using MyTaskSet =
    TaskSet<decltype(high_priority_task), decltype(sensor_task), decltype(logger_task)>;
...
constexpr size_t worst_case =
    worst_case_stack_usage<high_priority_stack, sensor_proc_stack, logger_stack>();
