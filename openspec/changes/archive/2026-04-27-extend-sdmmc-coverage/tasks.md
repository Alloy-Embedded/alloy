...
- [x] 2.1 `set_block_size(std::uint16_t)`,
      `set_block_count(std::uint16_t)`.
- [x] 2.2 `read_blocks(std::span<std::byte>) -> Result<void,
      ErrorCode>`,
      `write_blocks(std::span<const std::byte>) -> Result<void,
      ErrorCode>`.
- [x] 2.3 `configure_dma(const DmaChannel&)`, `enable_dma(bool)`
      — gated on `kHasDma`.
- [x] 2.4 `set_data_timeout(std::uint32_t cycles)`,
      `set_completion_timeout(std::uint32_t cycles)`.

## 3. Interrupts + IRQ vector + BlockDevice conformance

- [x] 3.1 `enum class InterruptKind { CommandComplete,
...
- [x] 3.3 `port_handle<P>` satisfies `BlockDevice` from
      `add-filesystem-hal` (in main spec). `static_assert` in a
      new compile test.

## 4. Compile tests + async + example + HW

- [x] 4.1 New `tests/compile_tests/test_sdmmc_api.cpp` against
