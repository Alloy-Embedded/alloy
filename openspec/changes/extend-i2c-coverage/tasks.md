...
- [ ] 1.1 `set_clock_speed(std::uint32_t hz)` — re-resolves
      TIMINGR / CCR from current kernel clock; returns
      `InvalidArgument` on >±5 % deviation.
- [ ] 1.2 `enum class SpeedMode { Standard100kHz, Fast400kHz,
      FastPlus1MHz }`. `set_speed_mode(SpeedMode)`. `FastPlus1MHz`
      gated on `kSupportsFastPlus`.
- [ ] 1.3 `enum class DutyCycle { Duty2, Duty169 }`.
      `set_duty_cycle(DutyCycle)` — gated on `kDutyField.valid`.
- [ ] 1.4 `enum class AddressingMode { Bits7, Bits10 }`.
      `set_addressing_mode(AddressingMode)`. Backends with
      7-bit-only width return `NotSupported` for `Bits10`.
- [ ] 1.5 `set_own_address(std::uint16_t addr, AddressingMode mode)`,
      `set_dual_address(std::uint16_t addr2)` (gated on second OAR
      field).
- [ ] 1.6 `set_clock_stretching(bool enabled)` — gated on the
      NOSTRETCH field.

## 2. Status / interrupts / IRQ vector

- [ ] 2.1 Status accessors: `nack_received()`, `arbitration_lost()`,
      `bus_error()`. `clear_*` mirrors per descriptor.
- [ ] 2.2 `enum class InterruptKind { Tx, Rx, Stop, Tc, AddrMatch,
...
- [ ] 2.4 `set_kernel_clock_source(KernelClockSource)` — gated on
      `kKernelClockSelectorField.valid`.

## 3. SMBus / PEC

- [ ] 3.1 SMBus (`kSupportsSmbus`): `enable_smbus(bool)`,
      `set_smbus_role(SmbusRole)` (Host / Device).
- [ ] 3.2 PEC (`kSupportsPec`): `enable_pec(bool)`,
      `last_pec() -> std::uint8_t`, `pec_error() -> bool`,
      `clear_pec_error() -> Result<void, ErrorCode>`.

## 4. Compile tests

- [ ] 4.1 Extend `tests/compile_tests/test_i2c_api.cpp`:
      instantiate every new method against `nucleo_g071rb` I2C1.
- [ ] 4.2 Add a `nucleo_f401re`-targeted compile test for legacy
      CCR-style timing.
- [ ] 4.3 Add a `same70_xplained`-targeted compile test for
      CWGR-style + SMBus.
