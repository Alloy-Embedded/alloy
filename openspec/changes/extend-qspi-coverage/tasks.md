...
- [ ] 1.1 `enum class FrameWidth { Single, Dual, Quad, Octal }`.
      `set_instruction_width(FrameWidth)`,
      `set_address_width(FrameWidth)`,
      `set_data_width(FrameWidth)`.
- [ ] 1.2 `set_instruction(std::uint8_t opcode)`,
      `set_address(std::uint32_t addr, std::uint8_t bits = 24)`,
      `set_dummy_cycles(std::uint8_t)`.
- [ ] 1.3 `set_bits_per_transfer(std::uint8_t bits)` — gated on
      `kBitsPerTransferField.valid`.
- [ ] 1.4 `read(std::span<std::byte>)`,
      `write(std::span<const std::byte>)`,
      `last_transfer_done() -> bool`.

## 2. Mode + chip-select + continuous-read + scrambling

- [ ] 2.1 `enum class QspiMode { Indirect, MemoryMapped, AutoPoll }`.
      `set_mode(QspiMode)`.
- [ ] 2.2 `enum class CsMode { LowOnTransfer, HighOnTransfer,
      PerInstruction }`.
      `set_cs_mode(CsMode)` — gated on
      `kChipSelectModeField.valid`.
- [ ] 2.3 `enable_continuous_read(bool)` — gated on
      `kContinuousReadModeField.valid`.
- [ ] 2.4 Scrambling (gated on `kHasScrambling`):
      `enable_scrambling(bool)`,
      `set_scrambling_key(std::uint32_t)`.
- [ ] 2.5 `invalidate_cache_for_memory_map()` — hook for D-cache
      invalidation after writes when in MemoryMapped mode.

## 3. Kernel clock + DMA + interrupts + IRQ vector

- [ ] 3.1 `set_kernel_clock_source(KernelClockSource)`.
- [ ] 3.2 `configure_dma(const DmaChannel&)` — gated on `kHasDma`.
- [ ] 3.3 `enum class InterruptKind { TransferComplete,
      FifoThreshold, StatusMatch, Timeout, Error }`.
      `enable_interrupt` / `disable_interrupt` — per-kind gated.
- [ ] 3.4 `irq_numbers() -> std::span<const std::uint32_t>`.

## 4. Compile tests + async + example + HW

- [ ] 4.1 New `tests/compile_tests/test_qspi_api.cpp` against
      `nucleo_f401re` QSPI.
