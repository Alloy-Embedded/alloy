...
- [x] 1.4 `set_kernel_clock_source(KernelClockSource)` — gated on
...
- [x] 3.5 Multiprocessor: `set_address(std::uint8_t addr,
      AddressLength len)`, `mute_until_address(bool)` —
      `AddressLength` is `Bits4` or `Bits7`.
- [x] 3.6 `enum class WakeupTrigger { AddressMatch, RxneNonEmpty,
