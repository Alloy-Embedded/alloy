...
- [ ] 1.1 `enum class UsbMode { Device, Host }`.
      `set_mode(UsbMode)` — gated on
      `kForceDeviceModeField.valid && kForceHostModeField.valid`.
- [ ] 1.2 `set_address(std::uint8_t)`, `enable_address(bool)`.
- [ ] 1.3 `freeze_clock()`, `unfreeze_clock()`,
      `clock_usable() -> bool` — gated on `kHasClockFreeze`.
- [ ] 1.4 `enable_crystalless(bool)` — gated on `kCrystalless`.
- [ ] 1.5 `dm_pin()` and `dp_pin()` static accessors returning
      descriptor's `kDmPin` / `kDpPin`.

## 2. Endpoint configuration + DPRAM allocator

- [ ] 2.1 `struct EndpointConfig { number, direction, type,
      max_packet_size }`.
- [ ] 2.2 `enum class EndpointType { Control, Bulk, Interrupt,
      Iso }`,
      `enum class Direction { In, Out }`.
- [ ] 2.3 `configure_endpoint(EndpointConfig)` — gated on
      `kHasDedicatedEndpointConfig`.
- [ ] 2.4 `allocate_endpoint_buffer(std::size_t bytes) ->
      Result<std::span<std::byte>, ErrorCode>`,
      `reset_dpram()` — gated on `kDpramBaseAddress` non-zero.

## 3. Status + interrupts + IRQ vector

- [ ] 3.1 `enum class InterruptKind { Reset, Suspend, Resume,
...
- [ ] 4.2 Compile test instantiates handle on every device with
