...
Currently, Alloy has basic ESP32 HAL support but doesn't fully leverage ESP-IDF's rich ecosystem of drivers, components, and services. Users must manually configure ESP-IDF components and cannot easily access WiFi, Bluetooth, advanced peripherals, or ESP-IDF's optimized drivers. This creates friction when developers want to use ESP32's full capabilities.

By integrating ESP-IDF as a first-class framework option, developers can seamlessly use all ESP-IDF components (WiFi, BLE, HTTP server, MQTT, etc.) with simple `#include` statements, while Alloy's build system automatically handles component registration, linking, and configuration.

## What Changes

- **Build System Enhancement**: Extend CMake integration to automatically register Alloy code as ESP-IDF components
- **Driver Abstraction Layer**: Create seamless mapping between Alloy HAL and ESP-IDF drivers
- **Automatic Component Detection**: Build system detects which ESP-IDF components are needed based on includes
- **Header Forwarding**: Provide Alloy-style headers that forward to ESP-IDF components (`#include "wifi/station.hpp"` → esp_wifi)
- **Configuration Integration**: Merge sdkconfig with Alloy build configuration
...
- **NEW**: `driver-abstraction` - Alloy HAL to ESP-IDF driver mapping
...
```cpp
#include "wifi/station.hpp"  // Alloy detects esp_wifi component
...
3. Alloy HAL can optionally use ESP-IDF drivers as backend
