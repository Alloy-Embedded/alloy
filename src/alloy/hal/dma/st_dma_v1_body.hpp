// The programming sequence of the ST v1 channel engine + DMAMUX pairing —
// the ENTIRE driver body, split out of st_dma_v1.hpp so a host test can reach
// it (the driver proper is constrained on the generated alloy::ip::st::dma_v1
// tag, which exists only inside a built project; tests/ deliberately never
// reads generated headers). Same seam as st_usart_v4_body.hpp and
// st_tim_adv_dtg.hpp: the logic lives once, here; the constrained
// dma_impl<Inst> specialization in st_dma_v1.hpp inherits it unchanged.
//
// Everything below reaches the register layout through `typename Inst::ip`
// and the mux through `typename Inst::mux_t`, so a host test supplies a
...
struct st_dma_v1_engine {
    ...
    struct callback {
        ...
        // closed). Witnessed on the host: test_st_dma_v1_latch.cpp runs THIS
        // ISR and THIS accessor over a memory-backed register file and fails
        // if either half-latch line below is reverted.
    ...
    // (alloy::dma::ring does, exactly — see its cursor()/write_pos()).
    template <unsigned Ch>
