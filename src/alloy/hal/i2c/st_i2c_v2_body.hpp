// The programming sequence of the ST i2c_v2 IP (TIMINGR/ISR era) — the ENTIRE
// driver body, split out of st_i2c_v2.hpp so a host test can reach it (the
// driver proper is constrained on the generated alloy::ip::st::i2c_v2 tag,
// which exists only inside a built project; tests/ deliberately never reads
// generated headers). Same seam as st_dma_v1_body.hpp and st_usart_v4_body.hpp:
// the logic lives once, here; the constrained i2c_impl<Inst> specialization in
// st_i2c_v2.hpp inherits it unchanged.
//
...
// file and exercises the REAL sequences — which is what makes the DMA arming
// rules below witnessable off-target: tests/test_i2c_dma.cpp fails if the
// CR1.RXDMAEN / CR1.TXDMAEN write, the CR2 address phase or the AUTOEND STOP
// wait is reverted.
//
...
//
// THREE transfer paths, one bus: the polled write()/read()/write_read()
// (bounded spins), the interrupt-driven one (see the block comment above
// start_transfer_irq()) and the one-shot DMA payload path (see the block
// comment above dma_rx_begin()). None of the three may overlap another on one
// bus — they consume the same flags. Ask transfer_busy() first.
