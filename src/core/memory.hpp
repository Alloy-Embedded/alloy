...
struct MemoryBudget {
    ...
    static constexpr bool fits_in(size_t max_bytes) { return sizeof(T) <= max_bytes; }

    /// Check if type has zero overhead vs. raw size
    static constexpr bool is_zero_overhead(size_t raw_size) { return sizeof(T) == raw_size; }
