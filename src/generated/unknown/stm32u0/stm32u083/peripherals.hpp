/// Auto-generated code for STM32U083
/// Generated by Alloy Code Generator
/// Source: st_stm32u083.json
/// DO NOT EDIT - Changes will be overwritten
///
/// Generated: 2025-10-31 17:44:42
#ifndef ALLOY_GENERATED_STM32U083_PERIPHERALS_HPP
#define ALLOY_GENERATED_STM32U083_PERIPHERALS_HPP

#include <cstdint>

namespace alloy::generated::stm32u083 {
...
namespace traits {
    ...
    constexpr uint32_t num_tim_instances = 10;
    constexpr bool has_usart = true;
    constexpr uint32_t num_usart_instances = 7;
...
namespace crypto {
    ...
    struct Registers {
        ...
        volatile uint32_t AES_SR;  ///< Offset: 0x04 - AES status register
        ...
        volatile uint32_t AES_SUSPR0;  ///< Offset: 0x40 - AES suspend registers
        volatile uint32_t AES_SUSPR1;  ///< Offset: 0x44 - AES suspend registers
        volatile uint32_t AES_SUSPR2;  ///< Offset: 0x48 - AES suspend registers
        volatile uint32_t AES_SUSPR3;  ///< Offset: 0x4C - AES suspend registers
        volatile uint32_t AES_SUSPR4;  ///< Offset: 0x50 - AES suspend registers
        volatile uint32_t AES_SUSPR5;  ///< Offset: 0x54 - AES suspend registers
        volatile uint32_t AES_SUSPR6;  ///< Offset: 0x58 - AES suspend registers
        volatile uint32_t AES_SUSPR7;  ///< Offset: 0x5C - AES suspend registers
        volatile uint32_t AES_IER;  ///< Offset: 0x300 - AES interrupt enable register
        volatile uint32_t AES_ISR;  ///< Offset: 0x304 - AES interrupt status register
        volatile uint32_t AES_ICR;  ///< Offset: 0x308 - AES interrupt clear register
    };
    ...
    namespace aes_cr_bits {
        constexpr uint32_t EN = (1U << 0);  ///< Enable This bit enables/disables the AES peripheral. At any moment, clearing then setting the bit re-initializes the AES peripheral. This bit is automatically cleared by hardware upon the completion of the key preparation (MODE[1:0] at 0x1) and upon the completion of GCM/GMAC/CCM initialization phase. The bit cannot be set as long as KEYVALID1is cleared
        constexpr uint32_t DATATYPE = (2 << 1);  ///< Data type This bitfield defines the format of data written in the AES_DINR register or read from the AES_DOUTR register, through selecting the mode of data swapping. This swapping is defined in Section121.4.14: AES data registers and data swapping. Attempts to write the bitfield are ignored when EN is set before the write access and it is not cleared by that write access.
        constexpr uint32_t MODE = (2 << 3);  ///< Operating mode This bitfield selects the AES operating mode: Attempts to write the bitfield are ignored when EN is set before the write access and it is not cleared by that write access.
        constexpr uint32_t CHMOD = (2 << 5);  ///< CHMOD[1:0]: Chaining mode This bitfield selects the AES chaining mode: others: Reserved Attempts to write the bitfield are ignored when EN is set before the write access and it is not cleared by that write access.
        constexpr uint32_t DMAINEN = (1U << 11);  ///< DMA input enable This bit enables automatic generation of DMA requests during the data phase, for incoming data transfers to AES via DMA. Setting this bit is ignored when MODE[1:0] is at 0x1 (key derivation).
        constexpr uint32_t DMAOUTEN = (1U << 12);  ///< DMA output enable This bit enables automatic generation of DMA requests during the data phase, for outgoing data transfers from AES via DMA. Setting this bit is ignored when MODE[1:0] is at 0x1 (key derivation).
        constexpr uint32_t GCMPH = (2 << 13);  ///< GCM or CCM phase selection This bitfield selects the phase, applicable only with GCM, GMAC or CCM chaining modes.
        constexpr uint32_t CHMOD_1 = (1U << 16);  ///< CHMOD[2]
        constexpr uint32_t KEYSIZE = (1U << 18);  ///< Key size selection This bitfield defines the key length in bits of the key used by AES. Attempts to write the bit are ignored when the EN is set before the write access and it is not cleared by that write access.
        constexpr uint32_t NPBLB = (4 << 20);  ///< Number of padding bytes in last block This padding information must be filled by software before processing the last block of GCM payload encryption or CCM payload decryption, otherwise authentication tag computation is incorrect. ...
        constexpr uint32_t IPRST = (1U << 31);  ///< AES peripheral software reset Setting the bit resets the AES peripheral, putting all registers to their default values, except the IPRST bit itself. Hence, any key-relative data are lost. For this reason, it is recommended to set the bit before handing over the AES to a less secure application. The bit must be kept low while writing any configuration registers.
    }

    /// AES_SR Register bits
    namespace aes_sr_bits {
        constexpr uint32_t RDERRF = (1U << 1);  ///< Read error flag This bit is set when an unexpected read to the AES_DOUTR register occurred. When set RDERRF bit has no impact on the AES operations. The flag setting generates an interrupt if the RWEIE bit of the AES_IER register is set. The flag is cleared by setting the RWEIF bit of the AES_ICR register.
        constexpr uint32_t WRERRF = (1U << 2);  ///< Write error flag This bit is set when an unexpected write to the AES_DINR register occurred. When set WRERRF bit has no impact on the AES operations. The flag setting generates an interrupt if the RWEIE bit of the AES_IER register is set. The flag is cleared by setting the RWEIF bit of the AES_ICR register.
        constexpr uint32_t BUSY = (1U << 3);  ///< Busy This flag indicates whether AES is idle or busy. AES is flagged as idle when disabled (when EN is low) or when the last processing is completed. AES is flagged as busy when processing a block data, preparing a key (ECB or CBC decryption only). When GCM encryption is selected, this flag must be at zero before suspending current process to manage a higher-priority message.
        constexpr uint32_t KEYVALID = (1U << 7);  ///< Key valid flag This bit is set by hardware when the key of size defined by KEYSIZE is loaded in AES_KEYRx key registers. The EN bit can only be set when KEYVALID is set. The key must be written in the key registers in the correct sequence, otherwise the KEIF flag is set and KEYVALID remains cleared. If set, KEIF must be cleared through the AES_ICR register, otherwise KEYVALID cannot be set. See the KEIF flag description for more details. For further information on key loading, refer to Section121.4.15: AES key registers.
    }

    /// AES_DINR Register bits
    namespace aes_dinr_bits {
        constexpr uint32_t DIN = (32 << 0);  ///< Data input A four-fold sequential write to this bitfield during the Input phase results in writing a complete 16-bytes block of input data to the AES peripheral. From the first to the fourth write, the corresponding data weights are [127:96], [95:64], [63:32], and [31:0]. Upon each write, the data from the 32-bit input buffer are handled by the data swap block according to the DATATYPE[1:0] bitfield, then written into the AES core 16-bytes input buffer. Reads return zero.
    }

    /// AES_DOUTR Register bits
    namespace aes_doutr_bits {
        constexpr uint32_t DOUT = (32 << 0);  ///< Data output This read-only bitfield fetches a 32-bit output buffer. A four-fold sequential read of this bitfield, upon the computation completion (CCF flag set), virtually reads a complete 16-byte block of output data from the AES peripheral. Before reaching the output buffer, the data produced by the AES core are handled by the data swap block according to the DATATYPE[1:0] bitfield. Data weights from the first to the fourth read operation are: [127:96], [95:64], [63:32], and [31:0].
    }

    /// AES_KEYR0 Register bits
    namespace aes_keyr0_bits {
        constexpr uint32_t KEY = (32 << 0);  ///< Cryptographic key, bits [31:0] These are bits [31:0] of the write-only bitfield KEY[255:0] AES encryption or decryption key, depending on the MODE[1:0] bitfield of the AES_CR register. Writes to AES_KEYRx registers are ignored when AES is enabled (EN bit set). A special writing sequence is required. In this sequence, any valid write to AES_KEYRx register clears the KEYVALID flag except for the sequence-completing write that sets it. Also refer to the description of the KEYVALID flag in the AES_SR register.
    }

    /// AES_KEYR1 Register bits
    namespace aes_keyr1_bits {
        constexpr uint32_t KEY = (32 << 0);  ///< Cryptographic key, bits [63:32] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield and for information relative to writing AES_KEYRx registers.
    }

    /// AES_KEYR2 Register bits
    namespace aes_keyr2_bits {
        constexpr uint32_t KEY = (32 << 0);  ///< Cryptographic key, bits [95:64] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield and for information relative to writing AES_KEYRx registers.
    }

    /// AES_KEYR3 Register bits
    namespace aes_keyr3_bits {
        constexpr uint32_t KEY = (32 << 0);  ///< Cryptographic key, bits [127:96] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield and for information relative to writing AES_KEYRx registers.
    }

    /// AES_IVR0 Register bits
    namespace aes_ivr0_bits {
        constexpr uint32_t IVI = (32 << 0);  ///< Initialization vector input, bits [31:0] AES_IVRx registers store the 128-bit initialization vector or the nonce, depending on the chaining mode selected. This value is updated by hardware after each computation round (when applicable). Write to this register is ignored when EN bit is set in AES_SR register
    }
    ...
    namespace aes_keyr4_bits {
        constexpr uint32_t KEY = (32 << 0);  ///< Cryptographic key, bits [159:128] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield and for information relative to writing AES_KEYRx registers.
    }

    /// AES_KEYR5 Register bits
    namespace aes_keyr5_bits {
        constexpr uint32_t KEY = (32 << 0);  ///< Cryptographic key, bits [191:160] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield and for information relative to writing AES_KEYRx registers.
    }

    /// AES_KEYR6 Register bits
    namespace aes_keyr6_bits {
        constexpr uint32_t KEY = (32 << 0);  ///< Cryptographic key, bits [223:192] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield and for information relative to writing AES_KEYRx registers.
    }

    /// AES_KEYR7 Register bits
    namespace aes_keyr7_bits {
        constexpr uint32_t KEY = (32 << 0);  ///< Cryptographic key, bits [255:224] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield and for information relative to writing AES_KEYRx registers.
    }

    /// AES_SUSPR0 Register bits
    namespace aes_suspr0_bits {
        constexpr uint32_t SUSP = (32 << 0);  ///< Suspend data AES_SUSPRx registers contain the complete internal register states of the AES when the GCM, GMAC or CCM processing of the current task is suspended to process a higher-priority task. Refer to Section121.4.8: AES suspend and resume operations for more details. Read to this register returns zero when EN bit is cleared in AES_SR register. AES_SUSPRx registers are not used in other chaining modes than GCM, GMAC or CCM.
    }

    /// AES_SUSPR1 Register bits
    namespace aes_suspr1_bits {
        constexpr uint32_t SUSP = (32 << 0);  ///< Suspend data AES_SUSPRx registers contain the complete internal register states of the AES when the GCM, GMAC or CCM processing of the current task is suspended to process a higher-priority task. Refer to Section121.4.8: AES suspend and resume operations for more details. Read to this register returns zero when EN bit is cleared in AES_SR register. AES_SUSPRx registers are not used in other chaining modes than GCM, GMAC or CCM.
    }

    /// AES_SUSPR2 Register bits
    namespace aes_suspr2_bits {
        constexpr uint32_t SUSP = (32 << 0);  ///< Suspend data AES_SUSPRx registers contain the complete internal register states of the AES when the GCM, GMAC or CCM processing of the current task is suspended to process a higher-priority task. Refer to Section121.4.8: AES suspend and resume operations for more details. Read to this register returns zero when EN bit is cleared in AES_SR register. AES_SUSPRx registers are not used in other chaining modes than GCM, GMAC or CCM.
    }

    /// AES_SUSPR3 Register bits
    namespace aes_suspr3_bits {
        constexpr uint32_t SUSP = (32 << 0);  ///< Suspend data AES_SUSPRx registers contain the complete internal register states of the AES when the GCM, GMAC or CCM processing of the current task is suspended to process a higher-priority task. Refer to Section121.4.8: AES suspend and resume operations for more details. Read to this register returns zero when EN bit is cleared in AES_SR register. AES_SUSPRx registers are not used in other chaining modes than GCM, GMAC or CCM.
    }

    /// AES_SUSPR4 Register bits
    namespace aes_suspr4_bits {
        constexpr uint32_t SUSP = (32 << 0);  ///< Suspend data AES_SUSPRx registers contain the complete internal register states of the AES when the GCM, GMAC or CCM processing of the current task is suspended to process a higher-priority task. Refer to Section121.4.8: AES suspend and resume operations for more details. Read to this register returns zero when EN bit is cleared in AES_SR register. AES_SUSPRx registers are not used in other chaining modes than GCM, GMAC or CCM.
    }

    /// AES_SUSPR5 Register bits
    namespace aes_suspr5_bits {
        constexpr uint32_t SUSP = (32 << 0);  ///< Suspend data AES_SUSPRx registers contain the complete internal register states of the AES when the GCM, GMAC or CCM processing of the current task is suspended to process a higher-priority task. Refer to Section121.4.8: AES suspend and resume operations for more details. Read to this register returns zero when EN bit is cleared in AES_SR register. AES_SUSPRx registers are not used in other chaining modes than GCM, GMAC or CCM.
    }

    /// AES_SUSPR6 Register bits
    namespace aes_suspr6_bits {
        constexpr uint32_t SUSP = (32 << 0);  ///< Suspend data AES_SUSPRx registers contain the complete internal register states of the AES when the GCM, GMAC or CCM processing of the current task is suspended to process a higher-priority task. Refer to Section121.4.8: AES suspend and resume operations for more details. Read to this register returns zero when EN bit is cleared in AES_SR register. AES_SUSPRx registers are not used in other chaining modes than GCM, GMAC or CCM.
    }

    /// AES_SUSPR7 Register bits
    namespace aes_suspr7_bits {
        constexpr uint32_t SUSP = (32 << 0);  ///< Suspend data AES_SUSPRx registers contain the complete internal register states of the AES when the GCM, GMAC or CCM processing of the current task is suspended to process a higher-priority task. Refer to Section121.4.8: AES suspend and resume operations for more details. Read to this register returns zero when EN bit is cleared in AES_SR register. AES_SUSPRx registers are not used in other chaining modes than GCM, GMAC or CCM.
    }

    /// AES_IER Register bits
    namespace aes_ier_bits {
        constexpr uint32_t CCFIE = (1U << 0);  ///< Computation complete flag interrupt enable This bit enables or disables (masks) the AES interrupt generation when CCF (computation complete flag) is set.
        constexpr uint32_t RWEIE = (1U << 1);  ///< Read or write error interrupt enable This bit enables or disables (masks) the AES interrupt generation when RWEIF (read and/or write error flag) is set.
        constexpr uint32_t KEIE = (1U << 2);  ///< Key error interrupt enable This bit enables or disables (masks) the AES interrupt generation when KEIF (key error flag) is set.
    }

    /// AES_ISR Register bits
    namespace aes_isr_bits {
        constexpr uint32_t CCF = (1U << 0);  ///< Computation complete flag This flag indicates whether the computation is completed. It is significant only when the DMAOUTEN bit is cleared, and it may stay high when DMAOUTEN is set. The flag setting generates an interrupt if the CCFIE bit of the AES_IER register is set. The flag is cleared by setting the corresponding bit of the AES_ICR register.
        constexpr uint32_t RWEIF = (1U << 1);  ///< Read or write error interrupt flag This read-only bit is set by hardware when a RDERRF or a WRERRF error flag is set in the AES_SR register. The flag setting generates an interrupt if the RWEIE bit of the AES_IER register is set. The flag is cleared by setting the corresponding bit of the AES_ICR register. The flags has no meaning when key derivation mode is selected. See the AES_SR register for details.
        constexpr uint32_t KEIF = (1U << 2);  ///< Key error interrupt flag This read-only bit is set by hardware when the key information fails to load into key registers. The flag setting generates an interrupt if the KEIE bit of the AES_IER register is set. The flag is cleared by setting the corresponding bit of the AES_ICR register. KEIF is raised upon any of the following events: AES_KEYRx register write does not respect the correct order. (For KEYSIZE1cleared, AES_KEYR0 then AES_KEYR1 then AES_KEYR2 then AES_KEYR3 register, or reverse. For KEYSIZE set, AES_KEYR0 then AES_KEYR1 then AES_KEYR2 then AES_KEYR3 then AES_KEYR4 then AES_KEYR5 then AES_KEYR6 then AES_KEYR7, or reverse). KEIF must be cleared by the application software, otherwise KEYVALID cannot be set.
    }

    /// AES_ICR Register bits
    namespace aes_icr_bits {
        constexpr uint32_t CCF = (1U << 0);  ///< Computation complete flag clear Setting this bit clears the CCF status bit of the AES_ISR register.
        constexpr uint32_t RWEIF = (1U << 1);  ///< Read or write error interrupt flag clear Setting this bit clears the RWEIF status bit of the AES_ISR register, and clears both RDERRF and WRERRF flags in the AES_SR register.
        constexpr uint32_t KEIF = (1U << 2);  ///< Key error interrupt flag clear Setting this bit clears the KEIF status bit of the AES_ISR register.
    }
...
namespace crs {
    /// Base addresses
    constexpr uint32_t CRS_BASE = 0x40006C00;

    /// CRS Register structure
    struct Registers {
        volatile uint32_t CRS_CR;  ///< Offset: 0x00 - CRS control register
        volatile uint32_t CRS_CFGR;  ///< Offset: 0x04 - CRS configuration register
    ...
    namespace crs_cr_bits {
        ...
        constexpr uint32_t AUTOTRIMEN = (1U << 6);  ///< Automatic trimming enable This bit enables the automatic hardware adjustment of TRIM bits according to the measured frequency error between two SYNC events. If this bit is set, the TRIM bits are read-only. The TRIM value can be adjusted by hardware by one or two steps at a time, depending on the measured frequency error value. Refer to Section15.4.4 for more details.
        ...
        constexpr uint32_t TRIM = (7 << 8);  ///< HSI48 oscillator smooth trimming The default value of the HSI48 oscillator smooth trimming is 64, which corresponds to the middle of the trimming interval.
    }

    /// CRS_CFGR Register bits
    namespace crs_cfgr_bits {
        constexpr uint32_t RELOAD = (16 << 0);  ///< Counter reload value RELOAD is the value to be loaded in the frequency error counter with each SYNC event. Refer to Section15.4.3 for more details about counter behavior.
        constexpr uint32_t FELIM = (8 << 16);  ///< Frequency error limit FELIM contains the value to be used to evaluate the captured frequency error value latched in the FECAP[15:0] bits of the CRS_ISR register. Refer to Section15.4.4 for more details about FECAP evaluation.
    ...
    namespace crs_isr_bits {
        ...
        constexpr uint32_t FECAP = (16 << 16);  ///< Frequency error capture FECAP is the frequency error counter value latched in the time of the last SYNC event. Refer to Section15.4.4 for more details about FECAP usage.
    }
...
namespace lcd {
    ...
    struct Registers {
        volatile uint32_t LCD_CR;  ///< Offset: 0x00 - LCD control register
        volatile uint32_t LCD_FCR;  ///< Offset: 0x04 - LCD frame control register
        volatile uint32_t LCD_SR;  ///< Offset: 0x08 - LCD status register
        volatile uint32_t LCD_CLR;  ///< Offset: 0x0C - LCD clear register
        volatile uint32_t LCD_RAM0;  ///< Offset: 0x14 - LCD display memory
        volatile uint32_t LCD_RAM1;  ///< Offset: 0x18 - LCD display memory
        volatile uint32_t LCD_RAM2;  ///< Offset: 0x1C - LCD display memory
        volatile uint32_t LCD_RAM3;  ///< Offset: 0x20 - LCD display memory
        volatile uint32_t LCD_RAM4;  ///< Offset: 0x24 - LCD display memory
        volatile uint32_t LCD_RAM5;  ///< Offset: 0x28 - LCD display memory
        volatile uint32_t LCD_RAM6;  ///< Offset: 0x2C - LCD display memory
        volatile uint32_t LCD_RAM7;  ///< Offset: 0x30 - LCD display memory
        volatile uint32_t LCD_RAM8;  ///< Offset: 0x34 - LCD display memory
        volatile uint32_t LCD_RAM9;  ///< Offset: 0x38 - LCD display memory
        volatile uint32_t LCD_RAM10;  ///< Offset: 0x3C - LCD display memory
        volatile uint32_t LCD_RAM11;  ///< Offset: 0x40 - LCD display memory
        volatile uint32_t LCD_RAM12;  ///< Offset: 0x44 - LCD display memory
        volatile uint32_t LCD_RAM13;  ///< Offset: 0x48 - LCD display memory
        volatile uint32_t LCD_RAM14;  ///< Offset: 0x4C - LCD display memory
        volatile uint32_t LCD_RAM15;  ///< Offset: 0x50 - LCD display memory
    };
    ...
    // Bit definitions
    /// LCD_CR Register bits
    namespace lcd_cr_bits {
        constexpr uint32_t LCDEN = (1U << 0);  ///< LCD controller enable This bit is set by software to enable the LCD controller/driver. It is cleared by software to turn off the LCD at the beginning of the next frame. When the LCD is disabled, all COM and SEG pins are driven to V<sub>SS</sub>.
        constexpr uint32_t VSEL = (1U << 1);  ///< Voltage source selection This bit determines the voltage source for the LCD.
        constexpr uint32_t DUTY = (3 << 2);  ///< Duty selection These bits determine the duty cycle. Values 101, 110 and 111 are forbidden. Others: Reserved
        constexpr uint32_t BIAS = (2 << 5);  ///< Bias selector These bits determine the bias used. Value 11 is forbidden.
        constexpr uint32_t MUX_SEG = (1U << 7);  ///< Mux segment enable This bit is used to enable SEG pin remapping. Four SEG pins can be multiplexed with1SEG[31:28] or SEG[15:12]. See Section118.3.7.
        constexpr uint32_t BUFEN = (1U << 8);  ///< Voltage output buffer enable This bit is used to enable/disable the voltage output buffer for higher driving capability.
    }

    /// LCD_FCR Register bits
    namespace lcd_fcr_bits {
        constexpr uint32_t HD = (1U << 0);  ///< High drive enable This bit is written by software to enable a low resistance divider. Displays with high internal resistance may need a longer drive time to achieve satisfactory contrast. This bit is useful in this case if some additional power consumption can be tolerated.
        constexpr uint32_t SOFIE = (1U << 1);  ///< Start of frame interrupt enable This bit is set and cleared by software.
        constexpr uint32_t UDDIE = (1U << 3);  ///< Update display done interrupt enable This bit is set and cleared by software.
        constexpr uint32_t PON = (3 << 4);  ///< Pulse ON duration These bits are written by software to define the pulse duration in terms of ck_ps pulses. A1short pulse leads to lower power consumption, but displays with high internal resistance may need a longer pulse to achieve satisfactory contrast. Note that the pulse is never longer than one half prescaled LCD clock period. PON duration example with LCDCLK = 32.7681kHz and PS=0x03:
        constexpr uint32_t DEAD = (3 << 7);  ///< Dead time duration These bits are written by software to configure the length of the dead time between frames. During the dead time the COM and SEG voltage levels are held at 0 V to reduce the contrast without modifying the frame rate. ......
        constexpr uint32_t CC = (3 << 10);  ///< Contrast control These bits specify one of the V<sub>LCD </sub>maximum voltages (independent of V<sub>DD</sub>). It ranges from12.60 V to 3.51V. Note: Refer to the datasheet for the V<sub>LCDx</sub> values.
        constexpr uint32_t BLINKF = (3 << 13);  ///< Blink frequency selection
        constexpr uint32_t BLINK = (2 << 16);  ///< Blink mode selection
        constexpr uint32_t DIV = (4 << 18);  ///< DIV clock divider These bits are written by software to define the division factor of the DIV divider (see1Section118.3.2.) ...
        constexpr uint32_t PS = (4 << 22);  ///< PS 16-bit prescaler These bits are written by software to define the division factor of the PS 16-bit prescaler. ck_ps = LCDCLK/(2<sup>PS[3:0]</sup>). See<sub> </sub>Section118.3.2. ...
    }

    /// LCD_SR Register bits
    namespace lcd_sr_bits {
        constexpr uint32_t ENS = (1U << 0);  ///< LCD enabled status This bit is set and cleared by hardware. It indicates the LCD controller status. Note: This bit is set immediately when LCDEN in LCD_CR goes from 0 to 1. On deactivation, it reflects the real LCD status. It becomes 0 at the end of the last displayed frame.
        constexpr uint32_t SOF = (1U << 1);  ///< Start-of-frame flag This bit is set by hardware at the beginning of a new frame, at the same time as the display data is updated. It is cleared by writing a 1 to SOFC in LCD_CLR. The bit clear has priority over the set.
        constexpr uint32_t UDR = (1U << 2);  ///< Update display request Each time software modifies the LCD_RAM, it must set this bit to transfer the updated data to the second level buffer. This bit stays set until the end of the update. During this time, the LCD_RAM is write protected. When the display is disabled, the update is performed for all LCD_DISPLAY locations. When the display is enabled, the update is performed only for locations for which commons are active (depending on DUTY). For example if DUTY = 1/2, Note: only the LCD_DISPLAY of COM0 and COM1 are updated. Note: Writing 0 on this bit or writing 1 when it is already 1 has no effect. This bit can be cleared by hardware only. It can be cleared only when LCDEN = 1
        constexpr uint32_t UDD = (1U << 3);  ///< Update display done This bit is set by hardware. It is cleared by writing 1 to UDDC in LCD_CLR. The bit set has priority over the clear. Note: If the device is in Stop mode (PCLK not provided), UDD does not generate an interrupt even if UDDIE = 1. If the display is not enabled, the UDD interrupt never occurs.
        constexpr uint32_t RDY = (1U << 4);  ///< Ready flag This bit is set and cleared by hardware. It indicates the status of the stepup converter.
        constexpr uint32_t FCRSF = (1U << 5);  ///< LCD frame control register synchronization flag This bit is set by hardware each time the LCD_FCR register is updated in the LCDCLK domain. It is cleared by hardware when writing to the LCD_FCR register.
    }

    /// LCD_CLR Register bits
    namespace lcd_clr_bits {
        constexpr uint32_t SOFC = (1U << 1);  ///< Start-of-frame flag clear This bit is written by software to clear SOF in LCD_SR.
        constexpr uint32_t UDDC = (1U << 3);  ///< Update display done clear This bit is written by software to clear UDD in LCD_SR.
    }

    /// LCD_RAM0 Register bits
    namespace lcd_ram0_bits {
        constexpr uint32_t SEGMENT_DATA = (32 << 0);  ///< Each bit corresponds to one pixel of the LCD display.
    }

    /// LCD_RAM1 Register bits
    namespace lcd_ram1_bits {
        constexpr uint32_t SEGMENT_DATA = (20 << 0);  ///< Each bit corresponds to one pixel of the LCD display.
    }

    /// LCD_RAM2 Register bits
    namespace lcd_ram2_bits {
        constexpr uint32_t SEGMENT_DATA = (32 << 0);  ///< Each bit corresponds to one pixel of the LCD display.
    }

    /// LCD_RAM3 Register bits
    namespace lcd_ram3_bits {
        constexpr uint32_t SEGMENT_DATA = (20 << 0);  ///< Each bit corresponds to one pixel of the LCD display.
    }

    /// LCD_RAM4 Register bits
    namespace lcd_ram4_bits {
        constexpr uint32_t SEGMENT_DATA = (32 << 0);  ///< Each bit corresponds to one pixel of the LCD display.
    }

    /// LCD_RAM5 Register bits
    namespace lcd_ram5_bits {
        constexpr uint32_t SEGMENT_DATA = (20 << 0);  ///< Each bit corresponds to one pixel of the LCD display.
    }

    /// LCD_RAM6 Register bits
    namespace lcd_ram6_bits {
        constexpr uint32_t SEGMENT_DATA = (32 << 0);  ///< Each bit corresponds to one pixel of the LCD display.
    }

    /// LCD_RAM7 Register bits
    namespace lcd_ram7_bits {
        constexpr uint32_t SEGMENT_DATA = (20 << 0);  ///< Each bit corresponds to one pixel of the LCD display.
    }

    /// LCD_RAM8 Register bits
    namespace lcd_ram8_bits {
        constexpr uint32_t SEGMENT_DATA = (32 << 0);  ///< Each bit corresponds to one pixel of the LCD display.
    }

    /// LCD_RAM9 Register bits
    namespace lcd_ram9_bits {
        constexpr uint32_t SEGMENT_DATA = (16 << 0);  ///< Each bit corresponds to one pixel of the LCD display.
    }

    /// LCD_RAM10 Register bits
    namespace lcd_ram10_bits {
        constexpr uint32_t SEGMENT_DATA = (32 << 0);  ///< Each bit corresponds to one pixel of the LCD display.
    }

    /// LCD_RAM11 Register bits
    namespace lcd_ram11_bits {
        constexpr uint32_t SEGMENT_DATA = (16 << 0);  ///< Each bit corresponds to one pixel of the LCD display.
    }

    /// LCD_RAM12 Register bits
    namespace lcd_ram12_bits {
        constexpr uint32_t SEGMENT_DATA = (32 << 0);  ///< Each bit corresponds to one pixel of the LCD display.
    }

    /// LCD_RAM13 Register bits
    namespace lcd_ram13_bits {
        constexpr uint32_t SEGMENT_DATA = (16 << 0);  ///< Each bit corresponds to one pixel of the LCD display.
    }

    /// LCD_RAM14 Register bits
    namespace lcd_ram14_bits {
        constexpr uint32_t SEGMENT_DATA = (32 << 0);  ///< Each bit corresponds to one pixel of the LCD display.
    }

    /// LCD_RAM15 Register bits
    namespace lcd_ram15_bits {
        constexpr uint32_t SEGMENT_DATA = (16 << 0);  ///< Each bit corresponds to one pixel of the LCD display.
    }
...
namespace tim {
    ...
    constexpr uint32_t LPTIM3_BASE = 0x40009000;
...
namespace usart {
    ...
    constexpr uint32_t LPUART3_BASE = 0x40008C00;
...
namespace rcc {
    ...
    namespace rcc_ahbrstr_bits {
        ...
        constexpr uint32_t AESRST = (1U << 16);  ///< AES hardware accelerator reset Set and cleared by software.
    ...
    namespace rcc_apbrstr1_bits {
        ...
        constexpr uint32_t LPUART3RST = (1U << 12);  ///< LPUART3 reset<sup>(1)</sup> Set and cleared by software.
        ...
        constexpr uint32_t SPI3RST = (1U << 15);  ///< SPI3 reset<sup>(1)</sup> Set and cleared by software.
        constexpr uint32_t CRSRST = (1U << 16);  ///< CRS reset<sup>(1)</sup> Set and cleared by software.
        ...
        constexpr uint32_t I2C4RST = (1U << 25);  ///< I2C4 reset<sup>(1)</sup> Set and cleared by software.
        constexpr uint32_t LPTIM3RST = (1U << 26);  ///< LPTIM3 reset Set and cleared by software.
    ...
    namespace rcc_ahbenr_bits {
        ...
        constexpr uint32_t AESEN = (1U << 16);  ///< AES hardware accelerator Set and cleared by software.
    ...
    namespace rcc_apbenr1_bits {
        ...
        constexpr uint32_t LPUART3EN = (1U << 12);  ///< LPUART3 clock enable Set and cleared by software.
        ...
        constexpr uint32_t SPI3EN = (1U << 15);  ///< SPI3 clock enable<sup>(1)</sup> Set and cleared by software.
        constexpr uint32_t CRSEN = (1U << 16);  ///< CRS clock enable<sup>(1)</sup> Set and cleared by software.
        ...
        constexpr uint32_t I2C4EN = (1U << 25);  ///< I2C4EN clock enable<sup>(1)</sup> Set and cleared by software.
        constexpr uint32_t LPTIM3EN = (1U << 26);  ///< LPTIM3 clock enable Set and cleared by software.
    ...
    namespace rcc_ahbsmenr_bits {
        ...
        constexpr uint32_t AESSMEN = (1U << 16);  ///< AES hardware accelerator clock enable during Sleep mode Set and cleared by software.
    ...
    namespace rcc_apbsmenr1_bits {
        ...
        constexpr uint32_t LPUART3SMEN = (1U << 12);  ///< LPUART3 clock enable during Sleep and Stop modes Set and cleared by software.
        ...
        constexpr uint32_t SPI3SMEN = (1U << 15);  ///< SPI3 clock enable during Sleep mode<sup>(1)</sup> Set and cleared by software.
        constexpr uint32_t CRSSMEN = (1U << 16);  ///< CRS clock enable during Sleep and Stop modes<sup>(1)</sup> Set and cleared by software.
        ...
        constexpr uint32_t I2C4SMEN = (1U << 25);  ///< I2C4 clock enable during Sleep mode<sup>(1)</sup> Set and cleared by software.
        constexpr uint32_t LPTIM3SMEN = (1U << 26);  ///< Low power timer 3 clock enable during Sleep mode Set and cleared by software.
    ...
    namespace rcc_ccipr_bits {
        ...
        constexpr uint32_t LPUART3SEL = (2 << 6);  ///< LPUART3 clock source selection<sup>(1)</sup> This bitfield is controlled by software to select LPUART3 clock source as follows:
        ...
        constexpr uint32_t LPTIM3SEL = (2 << 22);  ///< LPTIM3 clock source selection This bitfield is controlled by software to select LPTIM3 clock source as follows:
...
namespace syscfg {
    ...
    namespace syscfg_itline4_bits {
        ...
        constexpr uint32_t CRS = (1U << 1);  ///< CRS interrupt request pending
    }
    ...
    namespace syscfg_itline12_bits {
        ...
        constexpr uint32_t COMP2 = (1U << 2);  ///< Comparator 2 interrupt request pending (EXTI line 18)
    }
    ...
    namespace syscfg_itline19_bits {
        ...
        constexpr uint32_t LPTIM3 = (1U << 1);  ///< Low-power timer 3 interrupt request pending
    }
    ...
    namespace syscfg_itline24_bits {
        ...
        constexpr uint32_t I2C4 = (1U << 1);  ///< I2C4 interrupt request pending
    ...
    namespace syscfg_itline26_bits {
        ...
        constexpr uint32_t SPI3 = (1U << 1);  ///< SPI3 interrupt request pending
    }
    ...
    namespace syscfg_itline31_bits {
        ...
        constexpr uint32_t AES = (1U << 1);  ///< AES interrupt request pending
    }
...
namespace usb {
    ...
    struct Registers {
        volatile uint32_t USB_CHEP0R;  ///< Offset: 0x00 - USB endpoint/channel 0 register
        volatile uint32_t USB_CHEP1R;  ///< Offset: 0x04 - USB endpoint/channel 1 register
        volatile uint32_t USB_CHEP2R;  ///< Offset: 0x08 - USB endpoint/channel 2 register
        volatile uint32_t USB_CHEP3R;  ///< Offset: 0x0C - USB endpoint/channel 3 register
        volatile uint32_t USB_CHEP4R;  ///< Offset: 0x10 - USB endpoint/channel 4 register
        volatile uint32_t USB_CHEP5R;  ///< Offset: 0x14 - USB endpoint/channel 5 register
        volatile uint32_t USB_CHEP6R;  ///< Offset: 0x18 - USB endpoint/channel 6 register
        volatile uint32_t USB_CHEP7R;  ///< Offset: 0x1C - USB endpoint/channel 7 register
        volatile uint32_t USB_CNTR;  ///< Offset: 0x40 - USB control register
        volatile uint32_t USB_ISTR;  ///< Offset: 0x44 - USB interrupt status register
        volatile uint32_t USB_FNR;  ///< Offset: 0x48 - USB frame number register
        volatile uint32_t USB_DADDR;  ///< Offset: 0x4C - USB Device address
        volatile uint32_t USB_LPMCSR;  ///< Offset: 0x54 - LPM control and status register
        volatile uint32_t USB_BCDR;  ///< Offset: 0x58 - Battery charging detector
    };
    ...
    /// USB_CHEP2R Register bits
    namespace usb_chep2r_bits {
        constexpr uint32_t EA = (4 << 0);  ///< endpoint/channel address Device mode Software must write in this field the 4-bit address used to identify the transactions directed to this endpoint. A value must be written before enabling the corresponding endpoint. Host mode Software must write in this field the 4-bit address used to identify the channel addressed by the host transaction.
        constexpr uint32_t STATTX = (2 << 4);  ///< Status bits, for transmission transfers
        constexpr uint32_t DTOGTX = (1U << 6);  ///< Data toggle, for transmission transfers If the endpoint/channel is non-isochronous, this bit contains the required value of the data toggle bit (01=1DATA0, 11=1DATA1) for the next data packet to be transmitted. Hardware toggles this bit when the ACK handshake is received from the USB host, following a data packet transmission. If the endpoint/channel is defined as a control one, hardware sets this bit to 1 at the reception of a SETUP PID addressed to this endpoint (in device mode) or when a SETUP transaction is acknowledged by the device (in host mode). If the endpoint/channel is using the double buffer feature, this bit is used to support packet buffer swapping too (Refer to Section134.5.3: Double-buffered endpoints and usage in Device mode). If the endpoint/channel is isochronous, this bit is used to support packet buffer swapping since no data toggling is used for this sort of endpoints and only DATA0 packet are transmitted (refer to Section134.5.5: Isochronous transfers in Device mode). Hardware toggles this bit just after the end of data packet transmission, since no handshake is used for isochronous transfers. This bit can also be toggled by the software to initialize its value (mandatory when the endpoint/channel is not a control one) or to force a specific data toggle/packet buffer usage. When the application software writes 0, the value of DTOGTX remains unchanged, while writing 1 makes the bit value to toggle. This bit is read/write but it can only be toggled by writing 1.
        constexpr uint32_t VTTX = (1U << 7);  ///< Valid USB transaction transmitted Device mode This bit is set by the hardware when an IN transaction is successfully completed on this endpoint; the software can only clear this bit. If the CTRM bit in the USB_CNTR register is set accordingly, a generic interrupt condition is generated together with the endpoint related interrupt condition, which is always activated. A transaction ended with a NAK or STALL handshake does not set this bit, since no data is actually transferred, as in the case of protocol errors or data toggle mismatches. This bit is read/write but only 0 can be written. Host mode Same as VTRX behavior but for USB OUT and SETUP transactions.
    ...
    /// USB_CHEP3R Register bits
    namespace usb_chep3r_bits {
        constexpr uint32_t EA = (4 << 0);  ///< endpoint/channel address Device mode Software must write in this field the 4-bit address used to identify the transactions directed to this endpoint. A value must be written before enabling the corresponding endpoint. Host mode Software must write in this field the 4-bit address used to identify the channel addressed by the host transaction.
        constexpr uint32_t STATTX = (2 << 4);  ///< Status bits, for transmission transfers
        constexpr uint32_t DTOGTX = (1U << 6);  ///< Data toggle, for transmission transfers If the endpoint/channel is non-isochronous, this bit contains the required value of the data toggle bit (01=1DATA0, 11=1DATA1) for the next data packet to be transmitted. Hardware toggles this bit when the ACK handshake is received from the USB host, following a data packet transmission. If the endpoint/channel is defined as a control one, hardware sets this bit to 1 at the reception of a SETUP PID addressed to this endpoint (in device mode) or when a SETUP transaction is acknowledged by the device (in host mode). If the endpoint/channel is using the double buffer feature, this bit is used to support packet buffer swapping too (Refer to Section134.5.3: Double-buffered endpoints and usage in Device mode). If the endpoint/channel is isochronous, this bit is used to support packet buffer swapping since no data toggling is used for this sort of endpoints and only DATA0 packet are transmitted (refer to Section134.5.5: Isochronous transfers in Device mode). Hardware toggles this bit just after the end of data packet transmission, since no handshake is used for isochronous transfers. This bit can also be toggled by the software to initialize its value (mandatory when the endpoint/channel is not a control one) or to force a specific data toggle/packet buffer usage. When the application software writes 0, the value of DTOGTX remains unchanged, while writing 1 makes the bit value to toggle. This bit is read/write but it can only be toggled by writing 1.
        constexpr uint32_t VTTX = (1U << 7);  ///< Valid USB transaction transmitted Device mode This bit is set by the hardware when an IN transaction is successfully completed on this endpoint; the software can only clear this bit. If the CTRM bit in the USB_CNTR register is set accordingly, a generic interrupt condition is generated together with the endpoint related interrupt condition, which is always activated. A transaction ended with a NAK or STALL handshake does not set this bit, since no data is actually transferred, as in the case of protocol errors or data toggle mismatches. This bit is read/write but only 0 can be written. Host mode Same as VTRX behavior but for USB OUT and SETUP transactions.
    ...
    /// USB_CHEP4R Register bits
    namespace usb_chep4r_bits {
        constexpr uint32_t EA = (4 << 0);  ///< endpoint/channel address Device mode Software must write in this field the 4-bit address used to identify the transactions directed to this endpoint. A value must be written before enabling the corresponding endpoint. Host mode Software must write in this field the 4-bit address used to identify the channel addressed by the host transaction.
        constexpr uint32_t STATTX = (2 << 4);  ///< Status bits, for transmission transfers
        constexpr uint32_t DTOGTX = (1U << 6);  ///< Data toggle, for transmission transfers If the endpoint/channel is non-isochronous, this bit contains the required value of the data toggle bit (01=1DATA0, 11=1DATA1) for the next data packet to be transmitted. Hardware toggles this bit when the ACK handshake is received from the USB host, following a data packet transmission. If the endpoint/channel is defined as a control one, hardware sets this bit to 1 at the reception of a SETUP PID addressed to this endpoint (in device mode) or when a SETUP transaction is acknowledged by the device (in host mode). If the endpoint/channel is using the double buffer feature, this bit is used to support packet buffer swapping too (Refer to Section134.5.3: Double-buffered endpoints and usage in Device mode). If the endpoint/channel is isochronous, this bit is used to support packet buffer swapping since no data toggling is used for this sort of endpoints and only DATA0 packet are transmitted (refer to Section134.5.5: Isochronous transfers in Device mode). Hardware toggles this bit just after the end of data packet transmission, since no handshake is used for isochronous transfers. This bit can also be toggled by the software to initialize its value (mandatory when the endpoint/channel is not a control one) or to force a specific data toggle/packet buffer usage. When the application software writes 0, the value of DTOGTX remains unchanged, while writing 1 makes the bit value to toggle. This bit is read/write but it can only be toggled by writing 1.
        constexpr uint32_t VTTX = (1U << 7);  ///< Valid USB transaction transmitted Device mode This bit is set by the hardware when an IN transaction is successfully completed on this endpoint; the software can only clear this bit. If the CTRM bit in the USB_CNTR register is set accordingly, a generic interrupt condition is generated together with the endpoint related interrupt condition, which is always activated. A transaction ended with a NAK or STALL handshake does not set this bit, since no data is actually transferred, as in the case of protocol errors or data toggle mismatches. This bit is read/write but only 0 can be written. Host mode Same as VTRX behavior but for USB OUT and SETUP transactions.
    ...
    /// USB_CHEP5R Register bits
    namespace usb_chep5r_bits {
        constexpr uint32_t EA = (4 << 0);  ///< endpoint/channel address Device mode Software must write in this field the 4-bit address used to identify the transactions directed to this endpoint. A value must be written before enabling the corresponding endpoint. Host mode Software must write in this field the 4-bit address used to identify the channel addressed by the host transaction.
        constexpr uint32_t STATTX = (2 << 4);  ///< Status bits, for transmission transfers
        constexpr uint32_t DTOGTX = (1U << 6);  ///< Data toggle, for transmission transfers If the endpoint/channel is non-isochronous, this bit contains the required value of the data toggle bit (01=1DATA0, 11=1DATA1) for the next data packet to be transmitted. Hardware toggles this bit when the ACK handshake is received from the USB host, following a data packet transmission. If the endpoint/channel is defined as a control one, hardware sets this bit to 1 at the reception of a SETUP PID addressed to this endpoint (in device mode) or when a SETUP transaction is acknowledged by the device (in host mode). If the endpoint/channel is using the double buffer feature, this bit is used to support packet buffer swapping too (Refer to Section134.5.3: Double-buffered endpoints and usage in Device mode). If the endpoint/channel is isochronous, this bit is used to support packet buffer swapping since no data toggling is used for this sort of endpoints and only DATA0 packet are transmitted (refer to Section134.5.5: Isochronous transfers in Device mode). Hardware toggles this bit just after the end of data packet transmission, since no handshake is used for isochronous transfers. This bit can also be toggled by the software to initialize its value (mandatory when the endpoint/channel is not a control one) or to force a specific data toggle/packet buffer usage. When the application software writes 0, the value of DTOGTX remains unchanged, while writing 1 makes the bit value to toggle. This bit is read/write but it can only be toggled by writing 1.
        constexpr uint32_t VTTX = (1U << 7);  ///< Valid USB transaction transmitted Device mode This bit is set by the hardware when an IN transaction is successfully completed on this endpoint; the software can only clear this bit. If the CTRM bit in the USB_CNTR register is set accordingly, a generic interrupt condition is generated together with the endpoint related interrupt condition, which is always activated. A transaction ended with a NAK or STALL handshake does not set this bit, since no data is actually transferred, as in the case of protocol errors or data toggle mismatches. This bit is read/write but only 0 can be written. Host mode Same as VTRX behavior but for USB OUT and SETUP transactions.
    ...
    /// USB_CHEP6R Register bits
    namespace usb_chep6r_bits {
        constexpr uint32_t EA = (4 << 0);  ///< endpoint/channel address Device mode Software must write in this field the 4-bit address used to identify the transactions directed to this endpoint. A value must be written before enabling the corresponding endpoint. Host mode Software must write in this field the 4-bit address used to identify the channel addressed by the host transaction.
        constexpr uint32_t STATTX = (2 << 4);  ///< Status bits, for transmission transfers
        constexpr uint32_t DTOGTX = (1U << 6);  ///< Data toggle, for transmission transfers If the endpoint/channel is non-isochronous, this bit contains the required value of the data toggle bit (01=1DATA0, 11=1DATA1) for the next data packet to be transmitted. Hardware toggles this bit when the ACK handshake is received from the USB host, following a data packet transmission. If the endpoint/channel is defined as a control one, hardware sets this bit to 1 at the reception of a SETUP PID addressed to this endpoint (in device mode) or when a SETUP transaction is acknowledged by the device (in host mode). If the endpoint/channel is using the double buffer feature, this bit is used to support packet buffer swapping too (Refer to Section134.5.3: Double-buffered endpoints and usage in Device mode). If the endpoint/channel is isochronous, this bit is used to support packet buffer swapping since no data toggling is used for this sort of endpoints and only DATA0 packet are transmitted (refer to Section134.5.5: Isochronous transfers in Device mode). Hardware toggles this bit just after the end of data packet transmission, since no handshake is used for isochronous transfers. This bit can also be toggled by the software to initialize its value (mandatory when the endpoint/channel is not a control one) or to force a specific data toggle/packet buffer usage. When the application software writes 0, the value of DTOGTX remains unchanged, while writing 1 makes the bit value to toggle. This bit is read/write but it can only be toggled by writing 1.
        constexpr uint32_t VTTX = (1U << 7);  ///< Valid USB transaction transmitted Device mode This bit is set by the hardware when an IN transaction is successfully completed on this endpoint; the software can only clear this bit. If the CTRM bit in the USB_CNTR register is set accordingly, a generic interrupt condition is generated together with the endpoint related interrupt condition, which is always activated. A transaction ended with a NAK or STALL handshake does not set this bit, since no data is actually transferred, as in the case of protocol errors or data toggle mismatches. This bit is read/write but only 0 can be written. Host mode Same as VTRX behavior but for USB OUT and SETUP transactions.
    ...
    /// USB_CHEP7R Register bits
    namespace usb_chep7r_bits {
        constexpr uint32_t EA = (4 << 0);  ///< endpoint/channel address Device mode Software must write in this field the 4-bit address used to identify the transactions directed to this endpoint. A value must be written before enabling the corresponding endpoint. Host mode Software must write in this field the 4-bit address used to identify the channel addressed by the host transaction.
        constexpr uint32_t STATTX = (2 << 4);  ///< Status bits, for transmission transfers
        constexpr uint32_t DTOGTX = (1U << 6);  ///< Data toggle, for transmission transfers If the endpoint/channel is non-isochronous, this bit contains the required value of the data toggle bit (01=1DATA0, 11=1DATA1) for the next data packet to be transmitted. Hardware toggles this bit when the ACK handshake is received from the USB host, following a data packet transmission. If the endpoint/channel is defined as a control one, hardware sets this bit to 1 at the reception of a SETUP PID addressed to this endpoint (in device mode) or when a SETUP transaction is acknowledged by the device (in host mode). If the endpoint/channel is using the double buffer feature, this bit is used to support packet buffer swapping too (Refer to Section134.5.3: Double-buffered endpoints and usage in Device mode). If the endpoint/channel is isochronous, this bit is used to support packet buffer swapping since no data toggling is used for this sort of endpoints and only DATA0 packet are transmitted (refer to Section134.5.5: Isochronous transfers in Device mode). Hardware toggles this bit just after the end of data packet transmission, since no handshake is used for isochronous transfers. This bit can also be toggled by the software to initialize its value (mandatory when the endpoint/channel is not a control one) or to force a specific data toggle/packet buffer usage. When the application software writes 0, the value of DTOGTX remains unchanged, while writing 1 makes the bit value to toggle. This bit is read/write but it can only be toggled by writing 1.
        constexpr uint32_t VTTX = (1U << 7);  ///< Valid USB transaction transmitted Device mode This bit is set by the hardware when an IN transaction is successfully completed on this endpoint; the software can only clear this bit. If the CTRM bit in the USB_CNTR register is set accordingly, a generic interrupt condition is generated together with the endpoint related interrupt condition, which is always activated. A transaction ended with a NAK or STALL handshake does not set this bit, since no data is actually transferred, as in the case of protocol errors or data toggle mismatches. This bit is read/write but only 0 can be written. Host mode Same as VTRX behavior but for USB OUT and SETUP transactions.
    ...
    namespace usb_cntr_bits {
        ...
        constexpr uint32_t RST_DCONM = (1U << 10);  ///< USB reset request (Device mode) or device connect/disconnect (Host mode) interrupt mask
        constexpr uint32_t SUSPM = (1U << 11);  ///< Suspend mode interrupt mask
        constexpr uint32_t WKUPM = (1U << 12);  ///< Wake-up interrupt mask
    ...
    namespace usb_fnr_bits {
        ...
        constexpr uint32_t LSOF = (2 << 11);  ///< Lost SOF Device mode These bits are written by the hardware when an ESOF interrupt is generated, counting the number of consecutive SOF packets lost. At the reception of an SOF packet, these bits are cleared.
        constexpr uint32_t LCK = (1U << 13);  ///< Locked Device mode This bit is set by the hardware when at least two consecutive SOF packets have been received after the end of an USB reset condition or after the end of an USB resume sequence. Once locked, the frame timer remains in this state until an USB reset or USB suspend event occurs.
        constexpr uint32_t RXDM = (1U << 14);  ///< Receive data - line status This bit can be used to observe the status of received data minus upstream port data line. It can be used during end-of-suspend routines to help determining the wake-up event.
        constexpr uint32_t RXDP = (1U << 15);  ///< Receive data + line status This bit can be used to observe the status of received data plus upstream port data line. It can be used during end-of-suspend routines to help determining the wake-up event.
    }

    /// USB_DADDR Register bits
    namespace usb_daddr_bits {
        constexpr uint32_t ADD = (7 << 0);  ///< Device address Device mode These bits contain the USB function address assigned by the host PC during the enumeration process. Both this field and the endpoint/channel address (EA) field in the associated USB_CHEPnR register must match with the information contained in a USB token in order to handle a transaction to the required endpoint. Host mode These bits contain the address transmitted with the LPM transaction
        constexpr uint32_t EF = (1U << 7);  ///< Enable function This bit is set by the software to enable the USB Device. The address of this device is contained in the following ADD[6:0] bits. If this bit is at 0 no transactions are handled, irrespective of the settings of USB_CHEPnR registers.
    }

    /// USB_LPMCSR Register bits
    namespace usb_lpmcsr_bits {
        constexpr uint32_t LPMEN = (1U << 0);  ///< LPM support enable Device mode This bit is set by the software to enable the LPM support within the USB Device. If this bit is at 0 no LPM transactions are handled.
        constexpr uint32_t LPMACK = (1U << 1);  ///< LPM token acknowledge enable Device mode: The NYET/ACK is returned only on a successful LPM transaction: No errors in both the EXT token and the LPM token (else ERROR) A valid bLinkState = 0001B (L1) is received (else STALL)
        constexpr uint32_t REMWAKE = (1U << 3);  ///< bRemoteWake value Device mode This bit contains the bRemoteWake value received with last ACKed LPM Token
        constexpr uint32_t BESL = (4 << 4);  ///< BESL value Device mode These bits contain the BESL value received with last ACKed LPM Token
    }

    /// USB_BCDR Register bits
    namespace usb_bcdr_bits {
        constexpr uint32_t BCDEN = (1U << 0);  ///< Battery charging detector (BCD) enable Device mode This bit is set by the software to enable the BCD support within the USB Device. When enabled, the USB PHY is fully controlled by BCD and cannot be used for normal communication. Once the BCD discovery is finished, the BCD must be placed in OFF mode by clearing this bit to 0 in order to allow the normal USB operation.
        constexpr uint32_t DCDEN = (1U << 1);  ///< Data contact detection (DCD) mode enable Device mode This bit is set by the software to put the BCD into DCD mode. Only one detection mode (DCD, PD, SD or OFF) must be selected to work correctly.
        constexpr uint32_t PDEN = (1U << 2);  ///< Primary detection (PD) mode enable Device mode This bit is set by the software to put the BCD into PD mode. Only one detection mode (DCD, PD, SD or OFF) must be selected to work correctly.
        constexpr uint32_t SDEN = (1U << 3);  ///< Secondary detection (SD) mode enable Device mode This bit is set by the software to put the BCD into SD mode. Only one detection mode (DCD, PD, SD or OFF) must be selected to work correctly.
        constexpr uint32_t DCDET = (1U << 4);  ///< Data contact detection (DCD) status Device mode This bit gives the result of DCD.
        constexpr uint32_t PDET = (1U << 5);  ///< Primary detection (PD) status Device mode This bit gives the result of PD.
        constexpr uint32_t SDET = (1U << 6);  ///< Secondary detection (SD) status Device mode This bit gives the result of SD.
        constexpr uint32_t PS2DET = (1U << 7);  ///< DM pull-up detection status Device mode This bit is active only during PD and gives the result of comparison between DM voltage level and V<sub>LGC</sub> threshold. In normal situation, the DM level must be below this threshold. If it is above, it means that the DM is externally pulled high. This can be caused by connection to a PS2 port (which pulls-up both DP and DM lines) or to some proprietary charger not following the BCD specification.
        constexpr uint32_t DPPU_DPD = (1U << 15);  ///< DP pull-up / DPDM pull-down Device mode This bit is set by software to enable the embedded pull-up on DP line. Clearing it to 0 can be used to signal disconnect to the host when needed by the user software. Host mode This bit is set by software to enable the embedded pull-down on DP and DM lines.
    }
...
#endif // ALLOY_GENERATED_STM32U083_PERIPHERALS_HPP