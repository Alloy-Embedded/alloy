...
struct AdcDmaConfig {
    ...
    static constexpr bool has_timer_trigger() { return !std::is_void_v<TimerTrigger>; }

    static constexpr AdcDmaConfig create(PeripheralId peripheral, AdcChannel channel) {
        if constexpr (has_dma()) {
            static_assert(DmaConnection::is_compatible(), "Invalid DMA");
        }

        return AdcDmaConfig{.adc_config = {.peripheral = peripheral,
                                           .channel = channel,
    ...
    constexpr bool is_valid() const { return adc_config.is_valid(); }
};

template <typename DmaConn>
inline Result<void, ErrorCode> adc_dma_start(AdcChannel channel, void* buffer, usize size) {
    static_assert(DmaConn::is_compatible(), "Invalid DMA");

    auto dma_config =
        DmaTransferConfig<DmaConn>::peripheral_to_memory(buffer, size, DmaDataWidth::Bits16);
