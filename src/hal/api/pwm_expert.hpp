...
struct PwmExpertConfig {
    ...
    constexpr bool is_valid() const {
        if (channel > 3)
            return false;
        if (frequency_hz == 0 || frequency_hz > 100000000)
            return false;
    ...
    static constexpr PwmExpertConfig motor_bridge(u8 ch, float duty, bool inv = false,
                                                  u16 dead_time = 100) {
        return PwmExpertConfig{.channel = ch,
                               .frequency_hz = 20000,
                               .duty_percent = duty,
    ...
    static constexpr PwmExpertConfig custom(u8 ch, u32 freq_hz, float duty, bool inv = false,
                                            u8 prescale = 0) {
        return PwmExpertConfig{.channel = ch,
