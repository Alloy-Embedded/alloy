...
inline Result<void, ErrorCode> configure_peripheral_interrupt(
    PeripheralId peripheral, IrqPriority priority = IrqPriority::Normal) {
 ...
 * @return Result with error code
 */
inline Result<void, ErrorCode> configure_peripheral_interrupt_expert(PeripheralId peripheral,
                                                                     u8 preempt_priority,
                                                                     u8 sub_priority = 0) {
    auto irq = peripheral_to_irq(peripheral);

    InterruptExpertConfig config = {.irq_number = irq,
