...
/// GpioSchema — legacy closed enum for GPIO backend selection.
/// @deprecated (gpio-schema-open-traits task 5.1): superseded by
/// open GpioSchemaImpl concept types (StGpioSchema, MicrochipPioSchema, etc.)
/// available via pin_handle::schema_type.  Will be removed in a future
/// semver-minor release after all devices are regenerated with schema_type.
/// New HAL code must use pin_handle::schema_type; do NOT add new enumerators.
...
/// @deprecated (gpio-schema-open-traits task 5.1): resolved by HAL internally
/// as a backward-compat bridge; new code must not call this.
/// Use pin_handle::schema_type instead.
...
// ---------------------------------------------------------------------------
// Clock abstraction traits (add-clock-management-hal, task 1.1)
// ---------------------------------------------------------------------------

/// Bus domain for a peripheral — used by peripheral_frequency() to select
/// the correct live divider register to read.
enum class PeripheralBusDomain : std::uint8_t {
    apb1   = 0,
    apb2   = 1,
    ahb    = 2,
    sysclk = 3,
...
/// Primary template — no clock-tree data available for this peripheral.
/// Specializations are generated by alloy-cpp-emit from the IR clock_tree block.
template <device::runtime::PeripheralId P>
struct ClockSemanticTraits {
    static constexpr bool kPresent = false;
    static constexpr FieldRef kKernelClockMuxField = kInvalidFieldRef;
    static constexpr FieldRef kEnableBit           = kInvalidFieldRef;
    static constexpr const char* kBus              = "";
};

/// Primary template — no bus-domain mapping for this peripheral.
/// Specializations can be provided by device-level generated code or board HAL.
template <device::runtime::PeripheralId P>
struct PeripheralBusTraits {
    // kBusDomain not defined → peripheral_frequency falls back to APB1 estimate
};
