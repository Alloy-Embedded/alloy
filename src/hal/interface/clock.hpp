...
struct PllConfig {
    ...
    constexpr PllConfig(ClockSource src = ClockSource::ExternalCrystal,
                        core::u32 input_hz = 8000000, core::u8 mul = 9, core::u8 div = 1)
        : input_source(src),
...
struct ClockConfig {
    ...
    constexpr ClockConfig(ClockSource src = ClockSource::InternalRC, core::u32 crystal_hz = 8000000,
                          core::u8 pll_mul = 0, core::u8 pll_div = 1, core::u8 ahb_div = 1,
                          core::u8 apb1_div = 1, core::u8 apb2_div = 1, core::u32 target_hz = 0)
        : source(src),
          crystal_frequency_hz(crystal_hz),
          pll_multiplier(pll_mul),
          pll_divider(pll_div),
          ahb_divider(ahb_div),
...
template <typename T>
concept SystemClock =
    requires(T device, const T const_device, ClockConfig config, PllConfig pll_config,
             Peripheral peripheral, core::u32 frequency_hz) {
...
inline constexpr core::u8 calculate_flash_latency(core::u32 frequency_hz) {
    ...
    if (frequency_hz <= 24000000)
        return 0;  // 0 wait states up to 24MHz
    if (frequency_hz <= 48000000)
        return 1;  // 1 wait state up to 48MHz
    if (frequency_hz <= 72000000)
        return 2;  // 2 wait states up to 72MHz
...
inline constexpr bool is_frequency_valid(core::u32 frequency_hz,
                                         core::u32 max_frequency_hz = 168000000) {
...
inline constexpr core::u32 calculate_bus_frequency(core::u32 system_frequency_hz,
                                                   core::u8 divider) {
    if (divider == 0)
        divider = 1;  // Prevent division by zero
