...
/// SPI bit order
enum class SpiBitOrder : core::u8 {
...
struct SpiConfig {
    SpiMode mode;
    core::u32 clock_speed;  ///< Clock speed in Hz
...
concept SpiMaster = requires(T device, const T const_device, std::span<core::u8> buffer,
    ...
    { device.transfer(const_buffer, buffer) } -> std::same_as<core::Result<void>>;
    ...
    /// @param tx_buffer Buffer containing data to transmit
    /// @return Ok on success, error code on failure
    { device.transmit(const_buffer) } -> std::same_as<core::Result<void>>;
    ...
    /// @param rx_buffer Buffer to store received data
    /// @return Ok on success, error code on failure
    { device.receive(buffer) } -> std::same_as<core::Result<void>>;
