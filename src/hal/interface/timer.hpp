...
struct TimerConfig {
    ...
    constexpr TimerConfig(TimerMode m = TimerMode::Periodic,
                         core::u32 period = 1000,
                         core::u32 presc = 1,
                         CaptureEdge edge = CaptureEdge::Rising,
                         core::u32 compare = 0)
        : mode(m), period_us(period), prescaler(presc),
          capture_edge(edge), compare_value(compare) {}
};
...
template<typename T>
concept TimerDevice = requires(T device, const T const_device,
                               TimerConfig config,
                               core::u32 counter_value,
                               core::u32 period_us,
