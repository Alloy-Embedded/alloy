/// STM32F4 SysTick Timer Implementation
///
/// Uses ARM Cortex-M4F SysTick peripheral for microsecond time tracking.
///
/// Hardware Resources:
...
///
/// Configuration:
/// - Clock source: CPU / 8 (168MHz / 8 = 21MHz)
/// - Interrupt every 1ms (reload = 21000)
...
#ifndef ALLOY_HAL_STM32F4_SYSTICK_HPP
#define ALLOY_HAL_STM32F4_SYSTICK_HPP
...
/// STM32F4 SysTick implementation
///
...
public:
    ...
    /// - CPU clock / 8 = 21MHz (at 168MHz CPU)
    ...
    static core::Result<void> init() {
        ...
        // Configure reload value for 1ms at 21MHz
        // 21MHz / 1000 = 21000 ticks per millisecond
        constexpr core::u32 RELOAD_VALUE = 21000 - 1;
    ...
    static core::u32 micros() {
        ...
        // At 21MHz: each tick is ~47.6ns, so we divide by 21 to get microseconds
        core::u32 ticks_down = SysTick->VAL;
        core::u32 ticks_elapsed = 21000 - ticks_down;
        core::u32 us_frac = ticks_elapsed / 21;
...
static_assert(alloy::hal::SystemTick<SystemTick>,
              "STM32F4 SystemTick must satisfy SystemTick concept");

} // namespace alloy::hal::st::stm32f4

/// Global namespace implementation for STM32F4
namespace alloy::systick::detail {
    inline core::u32 get_micros() {
        return alloy::hal::st::stm32f4::SystemTick::micros();
    }
}
...
extern "C" {
    inline void SysTick_Handler() {
        alloy::hal::st::stm32f4::SystemTick::handle_interrupt();
    }
}

#endif // ALLOY_HAL_STM32F4_SYSTICK_HPP
