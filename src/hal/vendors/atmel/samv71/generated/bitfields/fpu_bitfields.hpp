...
namespace fpccr {
/// Lazy state preservation is active. Floating-point stack frame has been allocated but saving
/// state to it has been deferred. Position: 0, Width: 1
...
/// Priority permitted setting the HardFault handler to the pending state when the floating-point
/// stack frame was allocated. Position: 4, Width: 1
using HFRDY = BitField<4, 1>;
constexpr uint32_t HFRDY_Pos = 4;
constexpr uint32_t HFRDY_Msk = HFRDY::mask;

/// MemManage is enabled and priority permitted setting the MemManage handler to the pending state
/// when the floating-point stack frame was allocated. Position: 5, Width: 1
using MMRDY = BitField<5, 1>;
constexpr uint32_t MMRDY_Pos = 5;
constexpr uint32_t MMRDY_Msk = MMRDY::mask;

/// BusFault is enabled and priority permitted setting the BusFault handler to the pending state
/// when the floating-point stack frame was allocated. Position: 6, Width: 1
using BFRDY = BitField<6, 1>;
constexpr uint32_t BFRDY_Pos = 6;
constexpr uint32_t BFRDY_Msk = BFRDY::mask;

/// DebugMonitor is enabled and priority permits setting MON_PEND when the floating-point stack
/// frame was allocated. Position: 8, Width: 1
...
/// Enables CONTROL.FPCA setting on execution of a floating-point instruction. This results in
/// automatic hardware state preservation and restoration, for floating-point context, on exception
/// entry and exit. Position: 31, Width: 1
...
namespace fpcar {
/// The location of the unpopulated floating-point register space allocated on an exception stack
/// frame. Position: 3, Width: 29
...
namespace mvfr1 {
/// Indicates whether the FP hardware implementation supports only the Flush-to-Zero mode of
/// operation Position: 0, Width: 4
