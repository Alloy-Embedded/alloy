...
/// (rx_flow_thrhd_h3 rx_flow_thrhd). Position: 0, Width: 1 Access: read-only
...
/// This interrupt raw bit turns to high level when the amount of data in transmitter's fifo is less
/// than ((tx_mem_cnttxfifo_cnt) . Position: 1, Width: 1 Access: read-only
...
/// This interrupt raw bit turns to high level when receiver receives more data than the fifo can
/// store. Position: 4, Width: 1 Access: read-only
...
/// This interrupt raw bit turns to high level when receiver takes more time than rx_tout_thrhd to
/// receive a byte. Position: 8, Width: 1 Access: read-only
...
/// This interrupt raw bit turns to high level when receiver receives xoff char with
/// uart_sw_flow_con_en is set to 1. Position: 9, Width: 1 Access: read-only
...
/// This interrupt raw bit turns to high level when receiver receives xon char with
/// uart_sw_flow_con_en is set to 1. Position: 10, Width: 1 Access: read-only
...
/// This interrupt raw bit turns to high level when transmitter completes sendding 0 after all the
/// datas in transmitter's fifo are send. Position: 12, Width: 1 Access: read-only
...
/// This interrupt raw bit turns to high level when transmitter has kept the shortest duration after
/// the last data has been send. Position: 13, Width: 1 Access: read-only
...
/// This interrupt raw bit turns to high level when rs485 detects the clash between transmitter and
/// receiver. Position: 17, Width: 1 Access: read-only
...
/// when input pulse width is lower then this value igore this pulse.this register is used in
/// autobaud detect process. Position: 8, Width: 8 Access: read-write
...
/// STATUS - STATUS
namespace status {
/// (rx_mem_cnt rxfifo_cnt) stores the byte num of valid datas in receiver's fifo. rx_mem_cnt
/// register stores the 3 most significant bits rxfifo_cnt stores the 8 least significant bits.
/// Position: 0, Width: 8
/// Access: read-only
...
/// This register stores the value of receiver's finite state machine. 0:RX_IDLE 1:RX_STRT 2:RX_DAT0
/// 3:RX_DAT1 4:RX_DAT2 5:RX_DAT3 6:RX_DAT4 7:RX_DAT5 8:RX_DAT6 9:RX_DAT7 10:RX_PRTY 11:RX_STP1
/// 12:RX_STP2 13:RX_DL1 Position: 8, Width: 4 Access: read-only
...
/// (tx_mem_cnt txfifo_cnt) stores the byte num of valid datas in transmitter's fifo.tx_mem_cnt
/// stores the 3 most significant bits txfifo_cnt stores the 8 least significant bits. Position: 16,
/// Width: 8 Access: read-only
using TXFIFO_CNT = BitField<16, 8>;
constexpr uint32_t TXFIFO_CNT_Pos = 16;
constexpr uint32_t TXFIFO_CNT_Msk = TXFIFO_CNT::mask;

/// This register stores the value of transmitter's finite state machine. 0:TX_IDLE 1:TX_STRT
/// 2:TX_DAT0 3:TX_DAT1 4:TX_DAT2 5:TX_DAT3 6:TX_DAT4 7:TX_DAT5 8:TX_DAT6 9:TX_DAT7 10:TX_PRTY
/// 11:TX_STP1 12:TX_STP2 13:TX_DL0 14:TX_DL1 Position: 24, Width: 4 Access: read-only
...
/// This register is used to configure the software rts signal which is used in software flow
/// control. Position: 6, Width: 1 Access: read-write
using SW_RTS = BitField<6, 1>;
constexpr uint32_t SW_RTS_Pos = 6;
constexpr uint32_t SW_RTS_Msk = SW_RTS::mask;

/// This register is used to configure the software dtr signal which is used in software flow
/// control.. Position: 7, Width: 1 Access: read-write
...
/// 1.the irda transmitter's 11th bit is the same to the 10th bit. 0.set irda transmitter's 11th bit
/// to 0. Position: 11, Width: 1 Access: read-write
...
/// 1.receiver stops storing data int fifo when data is wrong. 0.receiver stores the data even if
/// the received data is wrong. Position: 26, Width: 1 Access: read-write
...
/// CONF1 - CONF1
namespace conf1 {
/// When receiver receives more data than its threshold value.receiver will produce
/// rxfifo_full_int_raw interrupt.the threshold value is (rx_flow_thrhd_h3 rxfifo_full_thrhd).
/// Position: 0, Width: 7
/// Access: read-write
...
/// when the data amount in transmitter fifo is less than its threshold value. it will produce
/// txfifo_empty_int_raw interrupt. the threshold value is (tx_mem_empty_thrhd txfifo_empty_thrhd)
/// Position: 8, Width: 7
/// Access: read-write
...
/// when receiver receives more data than its threshold value. receiver produce signal to tell the
/// transmitter stop transferring data. the threshold value is (rx_flow_thrhd_h3 rx_flow_thrhd).
/// Position: 16, Width: 7
/// Access: read-write
...
/// This is the flow enable bit for uart receiver. 1:choose software flow control with configuring
/// sw_rts signal Position: 23, Width: 1 Access: read-write
...
/// LOWPULSE - LOWPULSE
namespace lowpulse {
/// This register stores the value of the minimum duration time for the low level pulse. it is used
/// in baudrate-detect process. Position: 0, Width: 20 Access: read-only
...
/// HIGHPULSE - HIGHPULSE
namespace highpulse {
/// This register stores the value of the maxinum duration time for the high level pulse. it is used
/// in baudrate-detect process. Position: 0, Width: 20 Access: read-only
using MIN_CNT = BitField<0, 20>;
constexpr uint32_t MIN_CNT_Pos = 0;
constexpr uint32_t MIN_CNT_Msk = MIN_CNT::mask;
...
/// When the input rxd edge changes more than this register value. the uart is active from light
/// sleeping mode. Position: 0, Width: 10 Access: read-write
...
/// SWFC_CONF - SWFC_CONF
namespace swfc_conf {
/// when the data amount in receiver's fifo is more than this register value. it will send a xoff
/// char with uart_sw_flow_con_en set to 1. Position: 0, Width: 8 Access: read-write
...
/// When the data amount in receiver's fifo is less than this register value. it will send a xon
/// char with uart_sw_flow_con_en set to 1. Position: 8, Width: 8 Access: read-write
...
/// IDLE_CONF - IDLE_CONF
namespace idle_conf {
/// when receiver takes more time than this register value to receive a byte data. it will produce
/// frame end signal for uhci to stop receiving data. Position: 0, Width: 10 Access: read-write
...
/// it is active when txd_brk is set to 1. Position: 20, Width: 8 Access: read-write
...
/// Set this bit to enable loopback transmitter's output data signal to receiver's input data
/// signal. Position: 3, Width: 1 Access: read-write
...
/// 1: enable rs485's transmitter to send data when rs485's receiver is busy. 0:rs485's transmitter
/// should not send data when its receiver is busy. Position: 4, Width: 1 Access: read-write
...
/// This register is used to configure the idle duration time before the first at_cmd is received by
/// receiver. when the the duration is less than this register value it will not take the next data
/// received as at_cmd char. Position: 0, Width: 24 Access: read-write
...
/// when the duration is less than this register value it will not take the previous data as at_cmd
/// char. Position: 0, Width: 24 Access: read-write
...
/// This register is used to configure the duration time between the at_cmd chars. when the duration
/// time is less than this register value it will not take the datas as continous at_cmd chars.
/// Position: 0, Width: 24
/// Access: read-write
...
/// MEM_CONF - MEM_CONF
namespace mem_conf {
/// Set this bit to power down mem.when reg_mem_pd registers in the 3 uarts are all set to 1 mem
/// will enter low power mode. Position: 0, Width: 1 Access: read-write
using MEM_PD = BitField<0, 1>;
constexpr uint32_t MEM_PD_Pos = 0;
constexpr uint32_t MEM_PD_Msk = MEM_PD::mask;

/// This register is used to configure the amount of mem allocated to receiver's fifo. the default
/// byte num is 128. Position: 3, Width: 4 Access: read-write
using RX_SIZE = BitField<3, 4>;
constexpr uint32_t RX_SIZE_Pos = 3;
constexpr uint32_t RX_SIZE_Msk = RX_SIZE::mask;

/// This register is used to configure the amount of mem allocated to transmitter's fifo.the default
/// byte num is 128. Position: 7, Width: 4 Access: read-write
