...
/// CTR - CTR
namespace ctr {
/// 1: normally ouput sda data 0: exchange the function of sda_o and sda_oe (sda_o is the original
/// internal output sda signal sda_oe is the enable bit for the internal output sda signal)
/// Position: 0, Width: 1
/// Access: read-write
...
/// 1: normally ouput scl clock 0: exchange the function of scl_o and scl_oe (scl_o is the original
/// internal output scl signal scl_oe is the enable bit for the internal output scl signal)
/// Position: 1, Width: 1
/// Access: read-write
...
/// Set this bit to configure the module as i2c master clear this bit to configure the module as i2c
/// slave. Position: 4, Width: 1 Access: read-write
...
/// This bit is used to control the sending mode for data need to be send. 1: receive data from most
/// significant bit 0: receive data from least significant bit Position: 6, Width: 1 Access:
/// read-write
...
/// This bit is used to control the storage mode for received datas. 1: receive data from most
/// significant bit 0: receive data from least significant bit Position: 7, Width: 1 Access:
/// read-write
...
/// when I2C takes more than time_out_reg clocks to receive a data then this register changes to
/// high level. Position: 2, Width: 1 Access: read-only
...
/// when configured as i2c slave and the address send by master is equal to slave's address then
/// this bit will be high level. Position: 5, Width: 1 Access: read-only
...
/// This register stores the value of state machine for i2c module. 3'h0: SCL_MAIN_IDLE 3'h1:
/// SCL_ADDRESS_SHIFT 3'h2: SCL_ACK_ADDRESS 3'h3: SCL_RX_DATA 3'h4 SCL_TX_DATA 3'h5:SCL_SEND_ACK
/// 3'h6:SCL_WAIT_ACK Position: 24, Width: 3 Access: read-only
...
/// This register stores the value of state machine to produce SCL. 3'h0: SCL_IDLE 3'h1:SCL_START
/// 3'h2:SCL_LOW_EDGE 3'h3: SCL_LOW 3'h4:SCL_HIGH_EDGE 3'h5:SCL_HIGH 3'h6:SCL_STOP Position: 28,
/// Width: 3 Access: read-only
...
/// This is the offset address of the first receiving data as described in
/// nonfifo_rx_thres_register. Position: 5, Width: 5 Access: read-only
...
/// When this bit is set to 1 then the byte after address represent the offset address of I2C
/// Slave's ram. Position: 11, Width: 1 Access: read-write
...
/// when I2C receives more than nonfifo_rx_thres data it will produce rx_send_full_int_raw interrupt
/// and update the current offset address of the receiving data. Position: 14, Width: 6 Access:
/// read-write
...
/// when I2C sends more than nonfifo_tx_thres data it will produce tx_send_empty_int_raw interrupt
/// and update the current offset address of the sending data. Position: 20, Width: 6 Access:
/// read-write
...
/// The raw interrupt status bit for end_detect_int interrupt. when I2C deals with the END command
/// it will produce end_detect_int interrupt. Position: 3, Width: 1 Access: read-only
...
/// The raw interrupt status bit for slave_tran_comp_int interrupt. when I2C Slave detectsthe STOP
/// bit it will produce slave_tran_comp_int interrupt. Position: 4, Width: 1 Access: read-only
...
/// The raw interrupt status bit for arbitration_lost_int interrupt.when I2C lost the usage right of
/// I2C BUS it will produce arbitration_lost_int interrupt. Position: 5, Width: 1 Access: read-only
...
/// The raw interrupt status bit for master_tra_comp_int interrupt. when I2C Master sends or
/// receives a byte it will produce master_tran_comp_int interrupt. Position: 6, Width: 1 Access:
/// read-only
...
/// The raw interrupt status bit for trans_complete_int interrupt. when I2C Master finished STOP
/// command it will produce trans_complete_int interrupt. Position: 7, Width: 1 Access: read-only
...
/// The raw interrupt status bit for time_out_int interrupt. when I2C takes a lot of time to receive
/// a data it will produce time_out_int interrupt. Position: 8, Width: 1 Access: read-only
...
/// The raw interrupt status bit for trans_start_int interrupt. when I2C sends the START bit it will
/// produce trans_start_int interrupt. Position: 9, Width: 1 Access: read-only
...
/// The raw interrupt status bit for ack_err_int interrupt. when I2C receives a wrong ACK bit it
/// will produce ack_err_int interrupt.. Position: 10, Width: 1 Access: read-only
...
/// The raw interrupt status bit for rx_rec_full_int interrupt. when I2C receives more data than
/// nonfifo_rx_thres it will produce rx_rec_full_int interrupt. Position: 11, Width: 1 Access:
/// read-only
...
/// The raw interrupt status bit for tx_send_empty_int interrupt.when I2C sends more data than
/// nonfifo_tx_thres it will produce tx_send_empty_int interrupt.. Position: 12, Width: 1 Access:
/// read-only
...
/// SDA_HOLD - SDA_HOLD
namespace sda_hold {
/// This register is used to configure the clock num I2C used to hold the data after the negedge of
/// SCL. Position: 0, Width: 10 Access: read-write
...
/// This register is used to configure the clock num I2C used to sample data on SDA after the
/// posedge of SCL Position: 0, Width: 10 Access: read-write
using TIME = BitField<0, 10>;
constexpr uint32_t TIME_Pos = 0;
constexpr uint32_t TIME_Msk = TIME::mask;
...
/// This register is used to configure the clock num between the negedge of SDA and negedge of SCL
/// for start mark. Position: 0, Width: 10 Access: read-write
using TIME = BitField<0, 10>;
constexpr uint32_t TIME_Pos = 0;
constexpr uint32_t TIME_Msk = TIME::mask;
...
/// This register is used to configure the clock num between the posedge of SCL and the negedge of
/// SDA for restart mark. Position: 0, Width: 10 Access: read-write
using TIME = BitField<0, 10>;
constexpr uint32_t TIME_Pos = 0;
constexpr uint32_t TIME_Msk = TIME::mask;
...
/// This register is used to configure the clock num between the posedge of SCL and the posedge of
/// SDA. Position: 0, Width: 10 Access: read-write
using TIME = BitField<0, 10>;
constexpr uint32_t TIME_Pos = 0;
constexpr uint32_t TIME_Msk = TIME::mask;
...
/// COMD[16] - COMD[16]
namespace comd {
/// This is the content of command0. It consists of three part. op_code is the command 0: RSTART 1:
/// WRITE 2: READ 3: STOP . 4:END. Byte_num represent the number of data need to be send or data
/// need to be received. ack_check_en ack_exp and ack value are used to control the ack bit.
/// Position: 0, Width: 14
/// Access: read-write
