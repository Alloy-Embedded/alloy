...
constexpr uintptr_t XIP_SSI = 0x18000000;   // DW_apb_ssi has the following features:\n
        ...
        * Programmable delay on the sample time of the received serial data bit (rxd);
        enables programmable control of routing delays resulting in higher serial data -
            bit rates.\n* Programmable features :\n -
            Serial interface operation – Choice of Motorola SPI,
            Texas Instruments Synchronous Serial Protocol or
                National Semiconductor Microwire.\n - Clock bit -
                    rate – Dynamic control of the serial bit rate of the data transfer;
        used in only serial - master mode of operation.\n -
            Data Item size(4 to 32 bits) – Item size of each data transfer under the control of the
            programmer.\n* Configured features :\n
            - FIFO depth – 16 words deep.The FIFO width is fixed at 32 bits.\n -
            1 slave select output.\n - Hardware slave - select – Dedicated hardware slave
            - select line.\n - Combined interrupt line
            - one combined interrupt line from the DW_apb_ssi to the interrupt controller.\n
            - Interrupt polarity – active high interrupt lines.\n
            - Serial clock polarity – low serial - clock polarity directly after reset.\n
            - Serial clock phase – capture on first edge of serial
            - clock directly after reset.constexpr uintptr_t SYSINFO = 0x40000000;
        ...
        constexpr uintptr_t BUSCTRL =
            0x40030000;  // Register block for busfabric control signals and performance counters
        ...
        time is a 64 bit value indicating the time in usec since power -
            ...
            the corresponding alarm_irq is set and alarm_running is cleared\n To clear the interrupt
                write a 1 to the corresponding alarm_irq constexpr uintptr_t WATCHDOG = 0x40058000;
        constexpr uintptr_t RTC = 0x4005C000;  // Register block to control RTC
        constexpr uintptr_t ROSC = 0x40060000;
        constexpr uintptr_t VREG_AND_CHIP_RESET =
            0x40064000;  // control and status for on-chip voltage regulator and chip level reset
                         // subsystem
        constexpr uintptr_t TBMAN = 0x4006C000;  // Testbench manager. Allows the programmer to know
                                                 // what platform their software is running on.
