 ...
 * @brief Hardware Policy for UART on STM32F1 (Policy-Based Design)
 *
 ...
 *
 * Auto-generated from: stm32f1/usart.json
 * Generator: hardware_policy_generator.py
 * Generated: 2025-11-11 07:17:04
 *
...
#include "hal/vendors/st/stm32f1/registers/usart1_registers.hpp"

// Bitfield definitions
#include "hal/vendors/st/stm32f1/bitfields/usart1_bitfields.hpp"
...
/**
 * @brief Hardware Policy for USART on STM32F1
 *
 ...
 * using Uart0 = UartImpl<Stm32f1UartHardwarePolicy<UART0_BASE, 150000000>>;
 * @endcode
 *
...
struct Stm32f1UartHardwarePolicy {
    // ========================================================================
    // Type Definitions
    // ========================================================================
    ...
    static constexpr uint32_t UART_TIMEOUT = 100000;  ///< UART timeout in loop iterations
    ...
    static inline void configure_8n1() {
        ...
        hw()->CR1 &= ~(usart::cr1::M::mask | usart::cr1::PCE::mask); hw()->CR2 &= ~(usart::cr2::STOP::mask);
    }
    ...
    static inline void set_baudrate(uint32_t baud) {
        ...
        uint32_t usartdiv = (PERIPH_CLOCK_HZ + (baud / 2)) / baud; uint32_t mantissa = usartdiv >> 4; uint32_t fraction = usartdiv & 0xF; hw()->BRR = (mantissa << 4) | fraction;
    }
    ...
    /**
     * @brief Check if receive data register is not empty
     * @return bool
     *
    ...
    static inline bool wait_tx_ready(uint32_t timeout_loops) {
        ...
        uint32_t timeout = timeout_loops; while (!is_tx_ready() && --timeout); return timeout != 0;
    }
    ...
    static inline bool wait_rx_ready(uint32_t timeout_loops) {
        ...
        uint32_t timeout = timeout_loops; while (!is_rx_ready() && --timeout); return timeout != 0;
    }
...
using Usart2Hardware = Stm32f1UartHardwarePolicy<0x40004400, 72000000>;
