 ...
 * @brief STM32F4 Clock Configuration Policy
 *
 * Provides compile-time clock configuration for STM32F4 family.
 ...
 * - **Documented**: References to STM32F4 Reference Manual (RM0368)
 *
 * ## Usage
 *
 * @code
 ...
 *     static constexpr uint32_t pll_p_div = 4;  // Actual divider value (not register value)
 ...
 * };
 *
 * using BoardClock = Stm32f4Clock<BoardClockConfig>;
 *
...
#include "hal/vendors/st/stm32f4/generated/registers/flash_registers.hpp"
 ...
 *
 * Implements clock configuration for STM32F4 family using Policy-Based Design.
 *
...
class Stm32f4Clock {
   private:
    ...
    // PLLCFGR register bit positions (STM32F4 RM0368)
   ...
   public:
     ...
     *
     * Configures the STM32F4 clock tree:
     * 1. Enable HSE
    ...
    static Result<void, ErrorCode> enable_gpio_clocks() {
        ...
        rcc::RCC()->AHB1ENR |= ahb1enr::GPIOAEN::mask | ahb1enr::GPIOBEN::mask |
                               ...
                               ahb1enr::GPIOEEN::mask | ahb1enr::GPIOHEN::mask;
