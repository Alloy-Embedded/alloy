...
/// Synchronization jump width Must always be smaller than DTSEG2, valid values are 0 to 15. The
/// value used by the hardware is the one programmed, incremented by 1: tSJW = (DSJW + 1) x tq.
/// Position: 0, Width: 4
/// Access: read-write
...
/// Data time segment after sample point Valid values are 0 to 15. The value used by the hardware is
/// the one programmed, incremented by 1, i.e. tBS2 = (DTSEG2 + 1) x tq. Position: 4, Width: 4
/// Access: read-write
using DTSEG2 = BitField<4, 4>;
constexpr uint32_t DTSEG2_Pos = 4;
constexpr uint32_t DTSEG2_Msk = DTSEG2::mask;

/// Data time segment before sample point Valid values are 0 to 31. The value used by the hardware
/// is the one programmed, incremented by 1, i.e. tBS1 = (DTSEG1 + 1) x tq. Position: 8, Width: 5
/// Access: read-write
using DTSEG1 = BitField<8, 5>;
constexpr uint32_t DTSEG1_Pos = 8;
constexpr uint32_t DTSEG1_Msk = DTSEG1::mask;

/// Data bit rate prescaler The value by which the oscillator frequency is divided to generate the
/// bit time quanta. The bit time is built up from a multiple of this quanta. Valid values for the
/// Baud Rate Prescaler are 0 to 31. The hardware interpreters this value as the value programmed
/// plus 1. Position: 16, Width: 5 Access: read-write
...
namespace fdcan_rwd {
/// Watchdog configuration Start value of the message RAM watchdog counter. With the reset value of
/// 00, the counter is disabled. These are protected write (P) bits, write access is possible only
/// when the bit 1 [CCE] and bit 0 [INIT] of FDCAN_CCCR register are set to 1. Position: 0, Width: 8
/// Access: read-write
...
/// ASM restricted operation mode The restricted operation mode is intended for applications that
/// adapt themselves to different CAN bit rates. The application tests different bit rates and
/// leaves the Restricted Operation Mode after it has received a valid frame. In the optional
/// Restricted Operation Mode the node is able to transmit and receive data and remote frames and it
/// gives acknowledge to valid frames, but it does not send active error frames or overload frames.
/// In case of an error condition or overload condition, it does not send dominant bits, instead it
/// waits for the occurrence of bus idle condition to resynchronize itself to the CAN communication.
/// The error counters are not incremented. Bit ASM can only be set by software when both CCE and
/// INIT are set to 1. The bit can be reset by the software at any time. Position: 2, Width: 1
/// Access: read-write
...
/// Bus monitoring mode Bit MON can only be set by software when both CCE and INIT are set to 1. The
/// bit can be reset by the Host at any time. Position: 5, Width: 1 Access: read-write
...
/// If this bit is set, the FDCAN pauses for two CAN bit times before starting the next transmission
/// after successfully transmitting a frame. Position: 14, Width: 1 Access: read-write
...
/// Non ISO operation If this bit is set, the FDCAN uses the CAN FD frame format as specified by the
/// Bosch CAN FD Specification V1.0. Position: 15, Width: 1 Access: read-write
...
/// Nominal time segment after sample point Valid values are 0 to 127. The actual interpretation by
/// the hardware of this value is such that one more than the programmed value is used. Position: 0,
...
/// Nominal time segment before sample point Valid values are 0 to 255. The actual interpretation by
/// the hardware of this value is such that one more than the programmed value is used. These are
/// protected write (P) bits, write access is possible only when the bit 1 [CCE] and bit 0 [INIT] of
/// CCCR register are set to 1. Position: 8, Width: 8 Access: read-write
using NTSEG1 = BitField<8, 8>;
constexpr uint32_t NTSEG1_Pos = 8;
constexpr uint32_t NTSEG1_Msk = NTSEG1::mask;

/// Bit rate prescaler Value by which the oscillator frequency is divided for generating the bit
/// time quanta. The bit time is built up from a multiple of this quanta. Valid values are 0 to 511.
/// The actual interpretation by the hardware of this value is such that one more than the value
/// programmed here is used. These are protected write (P) bits, write access is possible only when
/// the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1. Position: 16, Width: 9 Access:
/// read-write
using NBRP = BitField<16, 9>;
constexpr uint32_t NBRP_Pos = 16;
constexpr uint32_t NBRP_Msk = NBRP::mask;

/// Nominal (re)synchronization jump width Valid values are 0 to 127. The actual interpretation by
/// the hardware of this value is such that the used value is the one programmed incremented by one.
/// These are protected write (P) bits, write access is possible only when the bit 1 [CCE] and bit 0
/// [INIT] of CCCR register are set to 1. Position: 25, Width: 7 Access: read-write
...
/// Timestamp select These are protected write (P) bits, write access is possible only when the bit
/// 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1. Position: 0, Width: 2 Access: read-write
...
/// Timestamp counter prescaler Configures the timestamp and timeout counters time unit in multiples
/// of CAN bit times [1 â¦ 16]. The actual interpretation by the hardware of this value is such
/// that one more than the value programmed here is used. In CAN FD mode the internal timestamp
/// counter TCP does not provide a constant time base due to the different CAN bit times between
/// arbitration phase and data phase. Thus CAN FD requires an external counter for timestamp
/// generation (TSS = 10). These are protected write (P) bits, write access is possible only when
/// the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1. Position: 16, Width: 4 Access:
/// read-write
...
/// Timestamp counter The internal/external timestamp counter value is captured on start of frame
/// (both Rx and Tx). When TSCC[TSS] = 01, the timestamp counter is incremented in multiples of CAN
/// bit times [1 â¦ 16] depending on the configuration of TSCC[TCP]. A wrap around sets
/// interrupt flag IR[TSW]. Write access resets the counter to 0. When TSCC.TSS = 10, TSC reflects
/// the external timestamp counter value. A write access has no impact. Position: 0, Width: 16
/// Access: read-write
...
/// Timeout counter enable This is a protected write (P) bit, write access is possible only when the
/// bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1. Position: 0, Width: 1 Access:
/// read-write
...
/// Timeout select When operating in Continuous mode, a write to TOCV presets the counter to the
/// value configured by TOCC[TOP] and continues down-counting. When the timeout counter is
/// controlled by one of the FIFOs, an empty FIFO presets the counter to the value configured by
/// TOCC[TOP]. Down-counting is started when the first FIFO element is stored. These are protected
/// write (P) bits, write access is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR
/// register are set to 1. Position: 1, Width: 2 Access: read-write
...
/// Timeout counter The timeout counter is decremented in multiples of CAN bit times [1 â¦ 16]
/// depending on the configuration of TSCC.TCP. When decremented to 0, interrupt flag IR.TOO is set
/// and the Timeout Counter is stopped. Start and reset/restart conditions are configured via
/// TOCC.TOS. Position: 0, Width: 16 Access: read-write
...
/// protocol error is detected, but CEL is still incremented. Position: 0, Width: 8 Access:
/// read-only
...
/// CAN error logging The counter is incremented each time when a CAN protocol error causes the
/// transmit error counter or the receive error counter to be incremented. It is reset by read
/// access to CEL. The counter stops at 0xFF; the next increment of TEC or REC sets interrupt flag
/// IR[ELO]. Access type is RX: reset on read. Position: 16, Width: 8 Access: read-write
...
namespace fdcan_psr {
/// Last error code The LEC indicates the type of the last error to occur on the CAN bus. This field
/// is cleared to 0 when a message has been transferred (reception or transmission) without error.
/// Access type is RS: set on read. Position: 0, Width: 3 Access: read-write
...
/// Data last error code Type of last error that occurred in the data phase of a FDCAN format frame
/// with its BRS flag set. Coding is the same as for LEC. This field is cleared to 0 when a FDCAN
/// format frame with its BRS flag set has been transferred (reception or transmission) without
/// error. Access type is RS: set on read. Position: 8, Width: 3 Access: read-write
using DLEC = BitField<8, 3>;
constexpr uint32_t DLEC_Pos = 8;
constexpr uint32_t DLEC_Msk = DLEC::mask;

/// ESI flag of last received FDCAN message This bit is set together with REDL, independent of
/// acceptance filtering. Access type is RX: reset on read. Position: 11, Width: 1 Access:
/// read-write
...
/// BRS flag of last received FDCAN message This bit is set together with REDL, independent of
/// acceptance filtering. Access type is RX: reset on read. Position: 12, Width: 1 Access:
/// read-write
...
/// Received FDCAN message This bit is set independent of acceptance filtering. Access type is RX:
/// reset on read. Position: 13, Width: 1 Access: read-write
...
/// Transmitter delay compensation value Position of the secondary sample point, defined by the sum
/// of the measured delay from FDCAN_TX to FDCAN_RX and TDCR.TDCO. The SSP position is, in the data
/// phase, the number of minimum time quanta (mtq) between the start of the transmitted bit and the
/// secondary sample point. Valid values are 0 to 127 mtq. Position: 16, Width: 7 Access: read-only
...
/// Transmitter delay compensation filter window length Defines the minimum value for the SSP
/// position, dominant edges on FDCAN_RX that would result in an earlier SSP position are ignored
/// for transmitter delay measurements. These are protected write (P) bits, which means that write
/// access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are
/// set to 1. Position: 0, Width: 7 Access: read-write
using TDCF = BitField<0, 7>;
constexpr uint32_t TDCF_Pos = 0;
constexpr uint32_t TDCF_Msk = TDCF::mask;

/// Transmitter delay compensation offset Offset value defining the distance between the measured
/// delay from FDCAN_TX to FDCAN_RX and the secondary sample point. Valid values are 0 to 127 mtq.
/// These are protected write (P) bits, which means that write access by the bits is possible only
/// when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1. Position: 8, Width: 7
/// Access: read-write
...
/// Message RAM access failure The flag is set when the Rx handler: has not completed acceptance
/// filtering or storage of an accepted message until the arbitration field of the following message
/// has been received. In this case acceptance filtering or message storage is aborted and the Rx
/// Handler starts processing of the following message. was unable to write a message to the message
/// RAM. In this case message storage is aborted. In both cases the FIFO put index is not updated.
/// The partly stored message is overwritten when the next message is stored to this location. The
/// flag is also set when the Tx Handler was not able to read a message from the Message RAM in
/// time. In this case message transmission is aborted. In case of a Tx Handler access failure the
/// FDCAN is switched into Restricted Operation Mode (see mode). To leave Restricted Operation Mode,
/// the Host CPU has to reset CCCR.ASM. Position: 14, Width: 1 Access: read-write
...
namespace fdcan_ils {
/// RX FIFO bit grouping the following interruption RF0LL: Rx FIFO 0 message lost interrupt line
/// RF0FL: Rx FIFO 0 full interrupt line RF0NL: Rx FIFO 0 new message interrupt line Position: 0,
...
/// RX FIFO bit grouping the following interruption RF1LL: Rx FIFO 1 message lost interrupt line
/// RF1FL: Rx FIFO 1 full Interrupt line RF1NL: Rx FIFO 1 new message interrupt line Position: 1,
/// Width: 1 Access: read-write
using RxFIFO1 = BitField<1, 1>;
constexpr uint32_t RxFIFO1_Pos = 1;
constexpr uint32_t RxFIFO1_Msk = RxFIFO1::mask;

/// Status message bit grouping the following interruption TCFL: Transmission cancellation finished
/// interrupt line TCL: Transmission completed interrupt line HPML: High-priority message interrupt
/// line Position: 2, Width: 1 Access: read-write
using SMSG = BitField<2, 1>;
constexpr uint32_t SMSG_Pos = 2;
constexpr uint32_t SMSG_Msk = SMSG::mask;

/// Tx FIFO ERROR grouping the following interruption TEFLL: Tx event FIFO element lost interrupt
/// line TEFFL: Tx event FIFO full interrupt line TEFNL: Tx event FIFO new entry interrupt line
/// TFEL: Tx FIFO empty interrupt line Position: 3, Width: 1 Access: read-write
using TFERR = BitField<3, 1>;
constexpr uint32_t TFERR_Pos = 3;
constexpr uint32_t TFERR_Msk = TFERR::mask;

/// Interrupt regrouping the following interruption TOOL: Timeout occurred interrupt line MRAFL:
/// Message RAM access failure interrupt line TSWL: Timestamp wraparound interrupt line Position: 4,
/// Width: 1 Access: read-write
...
/// Protocol error grouping the following interruption ARAL: Access to reserved address line PEDL:
/// Protocol error in data phase line PEAL: Protocol error in arbitration phase line WDIL: Watchdog
/// interrupt line BOL: Bus_Off status EWL: Warning status interrupt line Position: 6, Width: 1
/// Access: read-write
...
/// Reject remote frames extended These are protected write (P) bits, which means that write access
/// by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set
/// to 1. Position: 0, Width: 1 Access: read-write
...
/// Reject remote frames standard These are protected write (P) bits, which means that write access
/// by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set
/// to 1. Position: 1, Width: 1 Access: read-write
...
/// Accept non-matching frames extended Defines how received messages with 29-bit IDs that do not
/// match any element of the filter list are treated. These are protected write (P) bits, which
/// means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of
/// CCCR register are set to 1. Position: 2, Width: 2 Access: read-write
...
/// Accept Non-matching frames standard Defines how received messages with 11-bit IDs that do not
/// match any element of the filter list are treated. These are protected write (P) bits, which
/// means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of
/// CCCR register are set to 1. Position: 4, Width: 2 Access: read-write
...
/// FIFO 1 operation mode (overwrite or blocking) This is a protected write (P) bits, which means
/// that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR
/// register are set to 1. Position: 8, Width: 1 Access: read-write
using F1OM = BitField<8, 1>;
constexpr uint32_t F1OM_Pos = 8;
constexpr uint32_t F1OM_Msk = F1OM::mask;

/// FIFO 0 operation mode (overwrite or blocking) This is protected write (P) bits, which means that
/// write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register
/// are set to 1. Position: 9, Width: 1 Access: read-write
using F0OM = BitField<9, 1>;
constexpr uint32_t F0OM_Pos = 9;
constexpr uint32_t F0OM_Msk = F0OM::mask;

/// List size standard >28: Values greater than 28 are interpreted as 28. These are protected write
/// (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and
/// bit 0 [INIT] of CCCR register are set to 1. Position: 16, Width: 5 Access: read-write
...
/// List size extended >8: Values greater than 8 are interpreted as 8. These are protected write (P)
/// bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0
/// [INIT] of CCCR register are set to 1. Position: 24, Width: 4 Access: read-write
...
/// Extended ID mask For acceptance filtering of extended frames the Extended ID AND Mask is AND-ed
/// with the Message ID of a received frame. Intended for masking of 29-bit IDs in SAE J1939. With
/// the reset value of all bits set to 1 the mask is not active. These are protected write (P) bits,
/// which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT]
/// of CCCR register are set to 1. Position: 0, Width: 29 Access: read-write
...
/// Buffer index Index of Rx FIFO element to which the message was stored. Only valid when MSI[1]
/// = 1. Position: 0, Width: 3 Access: read-only
...
/// Rx FIFO 0 message lost This bit is a copy of interrupt flag IR[RF0L]. When IR[RF0L] is reset,
/// this bit is also reset. Position: 25, Width: 1 Access: read-only
...
/// Rx FIFO 0 acknowledge index After the Host has read a message or a sequence of messages from Rx
/// FIFO 0 it has to write the buffer index of the last element read from Rx FIFO 0 to F0AI. This
/// sets the Rx FIFOÂ 0 get index RXF0S[F0GI] to F0AI + 1 and update the FIFO 0 fill level
/// RXF0S[F0FL]. Position: 0, Width: 3 Access: read-write
...
/// Rx FIFO 1 message lost This bit is a copy of interrupt flag IR[RF1L]. When IR[RF1L] is reset,
/// this bit is also reset. Position: 25, Width: 1 Access: read-only
...
/// Rx FIFO 1 acknowledge index After the Host has read a message or a sequence of messages from Rx
/// FIFO 1 it has to write the buffer index of the last element read from Rx FIFO 1 to F1AI. This
/// sets the Rx FIFOÂ 1 get index RXF1S[F1GI] to F1AI + 1 and update the FIFO 1 Fill Level
/// RXF1S[F1FL]. Position: 0, Width: 3 Access: read-write
...
/// Tx FIFO/queue mode This is a protected write (P) bit, which means that write access by the bits
/// is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1. Position:
...
/// Tx FIFO free level Number of consecutive free Tx FIFO elements starting from TFGI, range 0 to 3.
/// Read as 0 when Tx queue operation is configured (TXBC[TFQM] = 1). Position: 0, Width: 3 Access:
/// read-only
using TFFL = BitField<0, 3>;
constexpr uint32_t TFFL_Pos = 0;
constexpr uint32_t TFFL_Msk = TFFL::mask;

/// Tx FIFO get index Tx FIFO read index pointer, range 0 to 3. Read as 0 when Tx queue operation is
/// configured (TXBC.TFQM = 1) Position: 8, Width: 2 Access: read-only
...
/// Transmission request pending Each Tx Buffer has its own transmission request pending bit. The
/// bits are set via register TXBAR. The bits are reset after a requested transmission has completed
/// or has been canceled via register TXBCR. After a TXBRP bit has been set, a Tx scan is started to
/// check for the pending Tx request with the highest priority (Tx Buffer with lowest Message ID). A
/// cancellation request resets the corresponding transmission request pending bit of register
/// TXBRP. In case a transmission has already been started when a cancellation is requested, this is
/// done at the end of the transmission, regardless whether the transmission was successful or not.
/// The cancellation request bits are reset directly after the corresponding TXBRP bit has been
/// reset. After a cancellation has been requested, a finished cancellation is signaled via TXBCF
/// after successful transmission together with the corresponding TXBTO bit when the transmission
/// has not yet been started at the point of cancellation when the transmission has been aborted due
/// to lost arbitration when an error occurred during frame transmission In DAR mode all
/// transmissions are automatically canceled if they are not successful. The corresponding TXBCF bit
/// is set for all unsuccessful transmissions. Position: 0, Width: 3 Access: read-only
...
/// Add request Each Tx buffer has its own add request bit. Writing a 1 sets the corresponding add
/// request bit; writing a 0 has no impact. This enables the Host to set transmission requests for
/// multiple Tx buffers with one write to TXBAR. When no Tx scan is running, the bits are reset
/// immediately, else the bits remain set until the Tx scan process has completed. Position: 0,
/// Width: 3 Access: read-write
...
/// corresponding CR bit; writing a 0 has no impact. This enables the Host to set cancellation
/// requests for multiple Tx buffers with one write to TXBCR. The bits remain set until the
/// corresponding TXBRP bit is reset. Position: 0, Width: 3 Access: read-write
...
/// Transmission occurred. Each Tx buffer has its own TO bit. The bits are set when the
/// corresponding TXBRP bit is cleared after a successful transmission. The bits are reset when a
...
/// Cancellation finished Each Tx buffer has its own CF bit. The bits are set when the corresponding
/// TXBRP bit is cleared after a cancellation was requested via TXBCR. In case the corresponding
/// TXBRP bit was not set at the point of cancellation, CF is set immediately. The bits are reset
/// when a new transmission is requested by writing a 1 to the corresponding bit of register TXBAR.
/// Position: 0, Width: 3
/// Access: read-only
...
/// Tx Event FIFO element lost This bit is a copy of interrupt flag IR[TEFL]. When IR[TEFL] is
/// reset, this bit is also reset. 0 No Tx event FIFO element lost 1 Tx event FIFO element lost,
/// also set after write attempt to Tx Event FIFO of size 0. Position: 25, Width: 1 Access:
/// read-only
...
/// Event FIFO acknowledge index After the Host has read an element or a sequence of elements from
/// the Tx event FIFO, it has to write the index of the last element read from Tx event FIFO to
/// EFAI. This sets the Tx event FIFO get index TXEFS[EFGI] to EFAI + 1 and updates the FIFO 0 fill
/// level TXEFS[EFFL]. Position: 0, Width: 2 Access: read-write
...
/// input clock divider The APB clock could be divided prior to be used by the CAN sub system. The
/// rate must be computed using the divider output clock. These are protected write (P) bits, which
/// means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of
/// CCCR register are set to 1. Position: 0, Width: 4 Access: read-write
