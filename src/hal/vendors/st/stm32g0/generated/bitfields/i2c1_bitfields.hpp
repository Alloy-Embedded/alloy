...
namespace ucore::hal::st::stm32g0::i2c1 {
...
namespace i2c_cr1 {
/// Peripheral enable Note: When PE=0, the I2C SCL and SDA lines are released. Internal state
/// machines and status bits are put back to their reset value. When cleared, PE must be kept low
/// for at least 3 APB clock cycles. Position: 0, Width: 1 Access: read-write
...
/// Transfer Complete interrupt enable Note: Any of these events generate an interrupt: Transfer
/// Complete (TC) Transfer Complete Reload (TCR) Position: 6, Width: 1 Access: read-write
...
/// Error interrupts enable Note: Any of these errors generate an interrupt: Arbitration Loss (ARLO)
/// Bus Error detection (BERR) Overrun/Underrun (OVR) Timeout detection (TIMEOUT) PEC error
/// detection (PECERR) Alert pin event detection (ALERT) Position: 7, Width: 1 Access: read-write
...
/// input. The digital filter, filters spikes with a length of up to DNF[3:0] * tI2CCLK ... Note: If
/// the analog filter is also enabled, the digital filter is added to the analog filter. This filter
/// can only be programmed when the I2C is disabled (PE = 0). Position: 8, Width: 4 Access:
/// read-write
...
/// Clock stretching disable This bit is used to disable clock stretching in slave mode. It must be
/// kept cleared in master mode. Note: This bit can only be programmed when the I2C is disabled (PE
/// = 0). Position: 17, Width: 1 Access: read-write
...
/// Wakeup from Stop mode enable Note: If the Wakeup from Stop mode feature is not supported, this
/// bit is reserved and forced by hardware to '0â. Refer to . Note: WUPEN can be set only when
/// DNF = '0000â Position: 18, Width: 1 Access: read-write
...
/// SMBus Host Address enable Note: If the SMBus feature is not supported, this bit is reserved and
/// forced by hardware to '0â. Refer to . Position: 20, Width: 1 Access: read-write
...
/// SMBus Device Default Address enable Note: If the SMBus feature is not supported, this bit is
/// reserved and forced by hardware to '0â. Refer to . Position: 21, Width: 1 Access:
/// read-write
...
/// SMBus alert enable Note: When ALERTEN=0, the SMBA pin can be used as a standard GPIO. If the
/// SMBus feature is not supported, this bit is reserved and forced by hardware to '0â. Refer
/// to . Position: 22, Width: 1 Access: read-write
...
/// PEC enable Note: If the SMBus feature is not supported, this bit is reserved and forced by
/// hardware to '0â. Refer to . Position: 23, Width: 1 Access: read-write
...
namespace i2c_cr2 {
/// Slave address (master mode) In 7-bit addressing mode (ADD10 = 0): SADD[7:1] should be written
/// with the 7-bit slave address to be sent. The bits SADD[9], SADD[8] and SADD[0] are don't care.
/// In 10-bit addressing mode (ADD10 = 1): SADD[9:0] should be written with the 10-bit slave address
/// to be sent. Note: Changing these bits when the START bit is set is not allowed. Position: 0,
/// Width: 10 Access: read-write
using SADD = BitField<0, 10>;
constexpr uint32_t SADD_Pos = 0;
constexpr uint32_t SADD_Msk = SADD::mask;

/// Transfer direction (master mode) Note: Changing this bit when the START bit is set is not
/// allowed. Position: 10, Width: 1 Access: read-write
...
/// 10-bit addressing mode (master mode) Note: Changing this bit when the START bit is set is not
/// allowed. Position: 11, Width: 1 Access: read-write
...
/// 10-bit address header only read direction (master receiver mode) Note: Changing this bit when
/// the START bit is set is not allowed. Position: 12, Width: 1 Access: read-write
...
/// Start generation This bit is set by software, and cleared by hardware after the Start followed
/// by the address sequence is sent, by an arbitration loss, by a timeout error detection, or when
/// PE = 0. It can also be cleared by software by writing '1â to the ADDRCF bit in the I2C_ICR
/// register. If the I2C is already in master mode with AUTOEND = 0, setting this bit generates a
/// Repeated Start condition when RELOAD=0, after the end of the NBYTES transfer. Otherwise setting
/// this bit generates a START condition once the bus is free. Note: Writing '0â to this bit
/// has no effect. The START bit can be set even if the bus is BUSY or I2C is in slave mode. This
/// bit has no effect when RELOAD is set. Position: 13, Width: 1 Access: read-write
...
/// Stop generation (master mode) The bit is set by software, cleared by hardware when a STOP
/// condition is detected, or when PE = 0. In Master Mode: Note: Writing '0â to this bit has no
/// effect. Position: 14, Width: 1 Access: read-write
...
/// NACK generation (slave mode) The bit is set by software, cleared by hardware when the NACK is
/// sent, or when a STOP condition or an Address matched is received, or when PE=0. Note: Writing
/// '0â to this bit has no effect. This bit is used in slave mode only: in master receiver
/// mode, NACK is automatically generated after last byte preceding STOP or RESTART condition,
/// whatever the NACK bit value. When an overrun occurs in slave receiver NOSTRETCH mode, a NACK is
/// automatically generated whatever the NACK bit value. When hardware PEC checking is enabled
/// (PECBYTE=1), the PEC acknowledge value does not depend on the NACK value. Position: 15, Width: 1
/// Access: read-write
...
/// Number of bytes The number of bytes to be transmitted/received is programmed there. This field
/// is donât care in slave mode with SBC=0. Note: Changing these bits when the START bit is set
/// is not allowed. Position: 16, Width: 8 Access: read-write
...
/// Automatic end mode (master mode) This bit is set and cleared by software. Note: This bit has no
/// effect in slave mode or when the RELOAD bit is set. Position: 25, Width: 1 Access: read-write
...
/// Packet error checking byte This bit is set by software, and cleared by hardware when the PEC is
/// transferred, or when a STOP condition or an Address matched is received, also when PE=0. Note:
/// Writing '0â to this bit has no effect. This bit has no effect when RELOAD is set. This bit
/// has no effect is slave mode when SBC=0. If the SMBus feature is not supported, this bit is
/// reserved and forced by hardware to '0â. Refer to . Position: 26, Width: 1 Access:
/// read-write
...
namespace i2c_oar1 {
/// Interface own slave address 7-bit addressing mode: OA1[7:1] contains the 7-bit own slave
/// address. The bits OA1[9], OA1[8] and OA1[0] are don't care. 10-bit addressing mode: OA1[9:0]
...
namespace i2c_oar2 {
/// Interface address 7-bit addressing mode: 7-bit address Note: These bits can be written only when
/// OA2EN=0. Position: 1, Width: 7 Access: read-write
using OA2 = BitField<1, 7>;
constexpr uint32_t OA2_Pos = 1;
constexpr uint32_t OA2_Msk = OA2::mask;

/// Own Address 2 masks Note: These bits can be written only when OA2EN=0. As soon as OA2MSK is not
/// equal to 0, the reserved I2C addresses (0b0000xxx and 0b1111xxx) are not acknowledged even if
/// the comparison matches. Position: 8, Width: 3 Access: read-write
...
/// Bus Timeout A This field is used to configure: The SCL low timeout condition tTIMEOUT when
/// TIDLE=0 tTIMEOUT= (TIMEOUTA+1) x 2048 x tI2CCLK The bus idle condition (both SCL and SDA high)
/// when TIDLE=1 tIDLE= (TIMEOUTA+1) x 4 x tI2CCLK Note: These bits can be written only when
/// TIMOUTEN=0. Position: 0, Width: 12 Access: read-write
...
/// Bus timeout B This field is used to configure the cumulative clock extension timeout: In master
/// mode, the master cumulative clock low extend time (tLOW:MEXT) is detected In slave mode, the
/// slave cumulative clock low extend time (tLOW:SEXT) is detected tLOW:EXT= (TIMEOUTB+1) x 2048 x
/// tI2CCLK Note: These bits can be written only when TEXTEN=0. Position: 16, Width: 12 Access:
/// read-write
...
/// Transfer direction (Slave mode) This flag is updated when an address match event occurs
/// (ADDR=1). Position: 16, Width: 1 Access: read-only
