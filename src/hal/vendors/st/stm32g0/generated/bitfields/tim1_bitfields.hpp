...
namespace tim1_cr1 {
/// Counter enable Note: External clock, gated mode and encoder mode can work only if the CEN bit
/// has been previously set by software. However trigger mode can set the CEN bit automatically by
/// hardware. Position: 0, Width: 1 Access: read-write
...
/// Update disable This bit is set and cleared by software to enable/disable UEV event generation.
/// Counter overflow/underflow Setting the UG bit Update generation through the slave mode
/// controller Buffered registers are then loaded with their preload values. Position: 1, Width: 1
/// Access: read-write
...
/// Counter overflow/underflow Setting the UG bit Update generation through the slave mode
/// controller Position: 2, Width: 1 Access: read-write
...
/// Direction Note: This bit is read only when the timer is configured in Center-aligned mode or
/// Encoder mode. Position: 4, Width: 1 Access: read-write
...
/// Center-aligned mode selection Note: Switch from edge-aligned mode to center-aligned mode as long
/// as the counter is enabled (CEN=1) is not allowed Position: 5, Width: 2 Access: read-write
...
/// Clock division This bit-field indicates the division ratio between the timer clock (CK_INT)
/// frequency and the dead-time and sampling clock (tDTS)used by the dead-time generators and the
/// digital filters (ETR, TIx): Note: tDTS = 1/fDTS, tCK_INT = 1/fCK_INT. Position: 8, Width: 2
/// Access: read-write
...
namespace tim1_cr2 {
/// Capture/compare preloaded control Note: This bit acts only on channels that have a complementary
/// output. Position: 0, Width: 1 Access: read-write
...
/// Capture/compare control update selection Note: This bit acts only on channels that have a
/// complementary output. Position: 2, Width: 1 Access: read-write
...
/// Master mode selection These bits allow selected information to be sent in master mode to slave
/// timers for synchronization (TRGO). The combination is as follows: Note: The clock of the slave
/// timer or ADC must be enabled prior to receive events from the master timer, and must not be
/// changed on-the-fly while triggers are received from the master timer. Position: 4, Width: 3
/// Access: read-write
...
/// Output Idle state 1 (OC1 output) Note: This bit can not be modified as long as LOCK level 1, 2
/// or 3 has been programmed (LOCK bits in TIMx_BDTR register). Position: 8, Width: 1 Access:
/// read-write
...
/// Output Idle state 1 (OC1N output) Note: This bit can not be modified as long as LOCK level 1, 2
/// or 3 has been programmed (LOCK bits in TIMx_BDTR register). Position: 9, Width: 1 Access:
/// read-write
...
/// Master mode selection 2 These bits allow the information to be sent to ADC for synchronization
/// (TRGO2) to be selected. The combination is as follows: Note: The clock of the slave timer or ADC
/// must be enabled prior to receive events from the master timer, and must not be changed
/// on-the-fly while triggers are received from the master timer. Position: 20, Width: 4 Access:
/// read-write
...
namespace tim1_smcr {
/// Slave mode selection When external signals are selected the active edge of the trigger signal
/// (TRGI) is linked to the polarity selected on the external input (see Input Control register and
/// Control Register description. Note: The gated mode must not be used if TI1F_ED is selected as
/// the trigger input (TS=00100). Indeed, TI1F_ED outputs 1 pulse for each transition on TI1F,
/// whereas the gated mode checks the level of the trigger signal. Note: The clock of the slave
/// peripherals (timer, ADC, ...) receiving the TRGO or the TRGO2 signals must be enabled prior to
/// receive events from the master timer, and the clock frequency (prescaler) must not be changed
/// on-the-fly while triggers are received from the master timer. Position: 0, Width: 3 Access:
/// read-write
...
/// Trigger selection This bit-field selects the trigger input to be used to synchronize the
/// counter. Others: Reserved See for more details on ITRx meaning for each Timer. Note: These bits
/// must be changed only when they are not used (e.g. when SMS=000) to avoid wrong edge detections
/// at the transition. Position: 4, Width: 3 Access: read-write
...
/// External trigger filter This bit-field then defines the frequency used to sample ETRP signal and
/// the length of the digital filter applied to ETRP. The digital filter is made of an event counter
/// in which N consecutive events are needed to validate a transition on the output: Position: 8,
...
/// External trigger prescaler External trigger signal ETRP frequency must be at most 1/4 of fCK_INT
/// frequency. A prescaler can be enabled to reduce ETRP frequency. It is useful when inputting fast
/// external clocks. Position: 12, Width: 2 Access: read-write
...
/// External clock enable This bit enables External clock mode 2. Note: Setting the ECE bit has the
/// same effect as selecting external clock mode 1 with TRGI connected to ETRF (SMS=111 and
/// TS=00111). It is possible to simultaneously use external clock mode 2 with the following slave
/// modes: reset mode, gated mode and trigger mode. Nevertheless, TRGI must not be connected to ETRF
/// in this case (TS bits must not be 00111). If external clock mode 1 and external clock mode 2 are
/// enabled at the same time, the external clock input is ETRF. Position: 14, Width: 1 Access:
/// read-write
...
/// peripherals (timer, ADC, ...) receiving the TRGO or the TRGO2 signals must be enabled prior to
/// receive events from the master timer, and the clock frequency (prescaler) must not be changed
/// on-the-fly while triggers are received from the master timer. Position: 16, Width: 1 Access:
/// read-write
...
/// counter. Others: Reserved See for more details on ITRx meaning for each Timer. Note: These bits
/// must be changed only when they are not used (e.g. when SMS=000) to avoid wrong edge detections
/// at the transition. Position: 20, Width: 2 Access: read-write
...
/// 0) and if the UDIS=0 in the TIMx_CR1 register. When CNT is reinitialized by software using the
/// UG bit in TIMx_EGR register, if URS=0 and UDIS=0 in the TIMx_CR1 register. When CNT is
/// reinitialized by a trigger event (refer to control register (TIM1_SMCRTIMx_SMCR)N/A), if URS=0
/// and UDIS=0 in the TIMx_CR1 register. Position: 0, Width: 1 Access: read-write
...
/// Capture/Compare 1 interrupt flag This flag is set by hardware. It is cleared by software (input
/// capture or output compare mode) or by reading the TIMx_CCR1 register (input capture mode only).
/// If channel CC1 is configured as output: this flag is set when he content of the counter TIMx_CNT
/// matches the content of the TIMx_CCR1 register. When the content of TIMx_CCR1 is greater than the
/// content of TIMx_ARR, the CC1IF bit goes high on the counter overflow (in up-counting and
/// up/down-counting modes) or underflow (in down-counting mode). There are 3 possible options for
/// flag setting in center-aligned mode, refer to the CMS bits in the TIMx_CR1 register for the full
/// description. If channel CC1 is configured as input: this bit is set when counter value has been
/// captured in TIMx_CCR1 register (an edge has been detected on IC1, as per the edge sensitivity
/// defined with the CC1P and CC1NP bits setting, in TIMx_CCER). Position: 1, Width: 1 Access:
/// read-write
...
/// COM interrupt flag This flag is set by hardware on COM event (when Capture/compare Control bits
/// - CCxE, CCxNE, OCxM - have been updated). It is cleared by software. Position: 5, Width: 1
/// Access: read-write
...
/// Trigger interrupt flag This flag is set by hardware on the TRG trigger event (active edge
/// detected on TRGI input when the slave mode controller is enabled in all modes but gated mode. It
/// is set when the counter starts or stops when gated mode is selected. It is cleared by software.
/// Position: 6, Width: 1
/// Access: read-write
...
/// Break interrupt flag This flag is set by hardware as soon as the break input goes active. It can
/// be cleared by software if the break input is not active. Position: 7, Width: 1 Access:
/// read-write
...
/// Break 2 interrupt flag This flag is set by hardware as soon as the break 2 input goes active. It
/// can be cleared by software if the break 2 input is not active. Position: 8, Width: 1 Access:
/// read-write
...
/// Capture/Compare 1 overcapture flag This flag is set by hardware only when the corresponding
/// channel is configured in input capture mode. It is cleared by software by writing it to
/// '0â. Position: 9, Width: 1 Access: read-write
...
/// System Break interrupt flag This flag is set by hardware as soon as the system break input goes
/// active. It can be cleared by software if the system break input is not active. This flag must be
/// reset to re-start PWM operation. Position: 13, Width: 1 Access: read-write
...
/// Compare 5 interrupt flag Refer to CC1IF description (Note: Channel 5 can only be configured as
/// output) Position: 16, Width: 1 Access: read-write
using CC5IF = BitField<16, 1>;
constexpr uint32_t CC5IF_Pos = 16;
constexpr uint32_t CC5IF_Msk = CC5IF::mask;

/// Compare 6 interrupt flag Refer to CC1IF description (Note: Channel 6 can only be configured as
/// output) Position: 17, Width: 1 Access: read-write
...
/// Capture/Compare 1 generation This bit is set by software in order to generate an event, it is
/// automatically cleared by hardware. If channel CC1 is configured as output: CC1IF flag is set,
/// Corresponding interrupt or DMA request is sent if enabled. If channel CC1 is configured as
/// input: The current value of the counter is captured in TIMx_CCR1 register. The CC1IF flag is
/// set, the corresponding interrupt or DMA request is sent if enabled. The CC1OF flag is set if the
/// CC1IF flag was already high. Position: 1, Width: 1 Access: write-only
...
/// Capture/Compare control update generation This bit can be set by software, it is automatically
/// cleared by hardware Note: This bit acts only on channels having a complementary output.
/// Position: 5, Width: 1
/// Access: write-only
...
/// Trigger generation This bit is set by software in order to generate an event, it is
/// automatically cleared by hardware. Position: 6, Width: 1 Access: write-only
...
/// Break generation This bit is set by software in order to generate an event, it is automatically
/// cleared by hardware. Position: 7, Width: 1 Access: write-only
...
/// Break 2 generation This bit is set by software in order to generate an event, it is
/// automatically cleared by hardware. Position: 8, Width: 1 Access: write-only
...
/// Capture/Compare 1 selection This bit-field defines the direction of the channel (input/output)
/// as well as the used input. Note: CC1S bits are writable only when the channel is OFF (CC1E =
/// '0â in TIMx_CCER). Position: 0, Width: 2 Access: read-write
...
/// Output Compare 1 fast enable This bit decreases the latency between a trigger event and a
/// transition on the timer output. It must be used in one-pulse mode (OPM bit set in TIMx_CR1
/// register), to have the output pulse starting as soon as possible after the starting trigger.
/// Position: 2, Width: 1
/// Access: read-write
...
/// Output Compare 1 preload enable Note: These bits can not be modified as long as LOCK level 3 has
/// been programmed (LOCK bits in TIMx_BDTR register) and CC1S=â00â (the channel is
/// configured in output). The PWM mode can be used without validating the preload register only in
/// one pulse mode (OPM bit set in TIMx_CR1 register). Else the behavior is not guaranteed.
/// Position: 3, Width: 1
/// Access: read-write
...
/// which OC1 and OC1N are derived. OC1REF is active high whereas OC1 and OC1N active level depends
/// on CC1P and CC1NP bits. Note: These bits can not be modified as long as LOCK level 3 has been
/// programmed (LOCK bits in TIMx_BDTR register) and CC1S=â00â (the channel is configured
/// in output). Note: In PWM mode, the OCREF level changes only when the result of the comparison
/// changes or when the output compare mode switches from âfrozenâ mode to âPWMâ
/// mode. Note: On channels having a complementary output, this bit field is preloaded. If the CCPC
/// bit is set in the TIMx_CR2 register then the OC1M active bits take the new value from the
/// preloaded bits only when a COM event is generated. Note: The OC1M[3] bit is not contiguous,
/// located in bit 16. Position: 4, Width: 3 Access: read-write
...
/// Capture/Compare 2 selection This bit-field defines the direction of the channel (input/output)
/// as well as the used input. Note: CC2S bits are writable only when the channel is OFF (CC2E =
/// '0â in TIMx_CCER). Position: 8, Width: 2 Access: read-write
...
/// bit is set in the TIMx_CR2 register then the OC1M active bits take the new value from the
/// preloaded bits only when a COM event is generated. Note: The OC1M[3] bit is not contiguous,
/// located in bit 16. Position: 16, Width: 1 Access: read-write
...
/// Input capture 1 prescaler This bit-field defines the ratio of the prescaler acting on CC1 input
/// (IC1). The prescaler is reset as soon as CC1E=â0â (TIMx_CCER register). Position: 2,
...
/// Input capture 1 filter This bit-field defines the frequency used to sample TI1 input and the
/// length of the digital filter applied to TI1. The digital filter is made of an event counter in
/// which N consecutive events are needed to validate a transition on the output: Position: 4,
/// Width: 4 Access: read-write
...
/// Capture/Compare 3 selection This bit-field defines the direction of the channel (input/output)
/// as well as the used input. Note: CC3S bits are writable only when the channel is OFF (CC3E =
/// '0â in TIMx_CCER). Position: 0, Width: 2 Access: read-write
...
/// Capture/Compare 4 selection This bit-field defines the direction of the channel (input/output)
/// as well as the used input. Note: CC4S bits are writable only when the channel is OFF (CC4E =
/// '0â in TIMx_CCER). Position: 8, Width: 2 Access: read-write
...
namespace tim1_ccer {
/// Capture/Compare 1 output enable When CC1 channel is configured as output, the OC1 level depends
/// on MOE, OSSI, OSSR, OIS1, OIS1N and CC1NE bits, regardless of the CC1E bits state. Refer to for
/// details. Note: On channels having a complementary output, this bit is preloaded. If the CCPC bit
/// is set in the TIMx_CR2 register then the CC1E active bit takes the new value from the preloaded
/// bit only when a Commutation event is generated. Position: 0, Width: 1 Access: read-write
...
/// Capture/Compare 1 output polarity When CC1 channel is configured as input, both CC1NP/CC1P bits
/// select the active polarity of TI1FP1 and TI2FP1 for trigger or capture operations. CC1NP=0,
/// CC1P=0: non-inverted/rising edge. The circuit is sensitive to TIxFP1 rising edge (capture or
/// trigger operations in reset, external clock or trigger mode), TIxFP1 is not inverted (trigger
/// operation in gated mode or encoder mode). CC1NP=0, CC1P=1: inverted/falling edge. The circuit is
/// sensitive to TIxFP1 falling edge (capture or trigger operations in reset, external clock or
/// trigger mode), TIxFP1 is inverted (trigger operation in gated mode or encoder mode). CC1NP=1,
/// CC1P=1: non-inverted/both edges/ The circuit is sensitive to both TIxFP1 rising and falling
/// edges (capture or trigger operations in reset, external clock or trigger mode), TIxFP1is not
/// inverted (trigger operation in gated mode). This configuration must not be used in encoder mode.
/// CC1NP=1, CC1P=0: The configuration is reserved, it must not be used. Note: This bit is not
/// writable as soon as LOCK level 2 or 3 has been programmed (LOCK bits in TIMx_BDTR register). On
/// channels having a complementary output, this bit is preloaded. If the CCPC bit is set in the
/// TIMx_CR2 register then the CC1P active bit takes the new value from the preloaded bit only when
/// a Commutation event is generated. Position: 1, Width: 1 Access: read-write
...
/// Capture/Compare 1 complementary output enable On channels having a complementary output, this
/// bit is preloaded. If the CCPC bit is set in the TIMx_CR2 register then the CC1NE active bit
/// takes the new value from the preloaded bit only when a Commutation event is generated. Position:
/// 2, Width: 1 Access: read-write
...
/// Capture/Compare 1 complementary output polarity CC1 channel configured as output: CC1 channel
/// configured as input: This bit is used in conjunction with CC1P to define the polarity of TI1FP1
/// and TI2FP1. Refer to CC1P description. Note: This bit is not writable as soon as LOCK level 2 or
/// 3 has been programmed (LOCK bits in TIMx_BDTR register) and CC1S=â00â (channel
/// configured as output). On channels having a complementary output, this bit is preloaded. If the
/// CCPC bit is set in the TIMx_CR2 register then the CC1NP active bit takes the new value from the
/// preloaded bit only when a Commutation event is generated. Position: 3, Width: 1 Access:
/// read-write
...
/// UIF copy This bit is a read-only copy of the UIF bit of the TIMx_ISR register. If the UIFREMAP
/// bit in the TIMxCR1 is reset, bit 31 is reserved and read at 0. Position: 31, Width: 1 Access:
/// read-only
...
/// TIM1_PSC - prescaler
namespace tim1_psc {
/// Prescaler value The counter clock frequency (CK_CNT) is equal to fCK_PSC / (PSC[15:0] + 1). PSC
/// contains the value to be loaded in the active prescaler register at each update event (including
/// when the counter is cleared through UG bit of TIMx_EGR register or through trigger controller
/// when configured in âreset modeâ). Position: 0, Width: 16 Access: read-write
...
namespace tim1_arr {
/// Auto-reload value ARR is the value to be loaded in the actual auto-reload register. Refer to the
/// for more details about ARR update and behavior. The counter is blocked while the auto-reload
/// value is null. Position: 0, Width: 16 Access: read-write
...
namespace tim1_bdtr {
/// Dead-time generator setup This bit-field defines the duration of the dead-time inserted between
/// the complementary outputs. DT correspond to this duration. DTG[7:5]=0xx => DT=DTG[7:0]x tDTG
/// with tDTG=tDTS. DTG[7:5]=10x => DT=(64+DTG[5:0])xtDTG with tDTG=2xtDTS. DTG[7:5]=110 =>
/// DT=(32+DTG[4:0])xtDTG with tDTG=8xtDTS. DTG[7:5]=111 => DT=(32+DTG[4:0])xtDTG with tDTG=16xtDTS.
/// Example if tDTS=125Â ns (8Â MHz), dead-time possible values are: 0 to 15875Â ns by 125Â ns
/// steps, 16Â Î¼s to 31750Â nsÂ by 250Â ns steps, 32Â Î¼s to 63Â Î¼s by 1Â Î¼s steps, 64Â Î¼s to
/// 126Â Î¼s by 2Â Î¼s steps Note: This bit-field can not be modified as long as LOCK level 1, 2 or
/// 3 has been programmed (LOCK bits in TIMx_BDTR register). Position: 0, Width: 8 Access:
/// read-write
using DTG = BitField<0, 8>;
constexpr uint32_t DTG_Pos = 0;
constexpr uint32_t DTG_Msk = DTG::mask;

/// Lock configuration These bits offer a write protection against software errors. Note: The LOCK
/// bits can be written only once after the reset. Once the TIMx_BDTR register has been written,
/// their content is frozen until the next reset. Position: 8, Width: 2 Access: read-write
...
/// Off-state selection for Idle mode This bit is used when MOE=0 due to a break event or by a
/// software write, on channels configured as outputs. See OC/OCN enable description for more
/// details (enable register (TIM1_CCERTIMx_CCER)N/A). Note: This bit can not be modified as soon as
/// the LOCK level 2 has been programmed (LOCK bits in TIMx_BDTR register). Position: 10, Width: 1
/// Access: read-write
...
/// Off-state selection for Run mode This bit is used when MOE=1 on channels having a complementary
/// output which are configured as outputs. OSSR is not implemented if no complementary output is
/// implemented in the timer. See OC/OCN enable description for more details (enable register
/// (TIM1_CCERTIMx_CCER)N/A). Note: This bit can not be modified as soon as the LOCK level 2 has
/// been programmed (LOCK bits in TIMx_BDTR register). Position: 11, Width: 1 Access: read-write
...
/// Break enable This bit enables the complete break protection (including all sources connected to
/// bk_acth and BKIN sources, as per ). Note: This bit cannot be modified when LOCK level 1 has been
/// programmed (LOCK bits in TIMx_BDTR register). Note: Any write operation to this bit takes a
/// delay of 1 APB clock cycle to become effective. Position: 12, Width: 1 Access: read-write
...
/// Break polarity Note: This bit can not be modified as long as LOCK level 1 has been programmed
/// (LOCK bits in TIMx_BDTR register). Note: Any write operation to this bit takes a delay of 1 APB
/// clock cycle to become effective. Position: 13, Width: 1 Access: read-write
...
/// Automatic output enable Note: This bit can not be modified as long as LOCK level 1 has been
/// programmed (LOCK bits in TIMx_BDTR register). Position: 14, Width: 1 Access: read-write
...
/// Main output enable This bit is cleared asynchronously by hardware as soon as one of the break
/// inputs is active (BRK or BRK2). It is set by software or automatically depending on the AOE bit.
/// It is acting only on the channels which are configured in output. In response to a break event
/// or if MOE is written to 0: OC and OCN outputs are disabled or forced to idle state depending on
/// the OSSI bit. See OC/OCN enable description for more details (enable register
/// (TIM1_CCERTIMx_CCER)N/A). Position: 15, Width: 1 Access: read-write
...
/// Break filter This bit-field defines the frequency used to sample BRK input and the length of the
/// digital filter applied to BRK. The digital filter is made of an event counter in which N
/// consecutive events are needed to validate a transition on the output: Note: This bit cannot be
/// modified when LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register). Position: 16,
/// Width: 4 Access: read-write
...
/// Break 2 filter This bit-field defines the frequency used to sample BRK2 input and the length of
/// the digital filter applied to BRK2. The digital filter is made of an event counter in which N
/// consecutive events are needed to validate a transition on the output: Note: This bit cannot be
/// modified when LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register). Position: 20,
/// Width: 4 Access: read-write
...
/// Break 2 enable Note: The BRK2 must only be used with OSSR = OSSI = 1. Note: This bit cannot be
/// modified when LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register). Note: Any
/// write operation to this bit takes a delay of 1 APB clock cycle to become effective. Position:
/// 24, Width: 1 Access: read-write
...
/// Break 2 polarity Note: This bit cannot be modified as long as LOCK level 1 has been programmed
/// (LOCK bits in TIMx_BDTR register). Note: Any write operation to this bit takes a delay of 1 APB
/// clock cycle to become effective. Position: 25, Width: 1 Access: read-write
...
/// Break Disarm This bit is cleared by hardware when no break source is active. The BKDSRM bit must
/// be set by software to release the bidirectional output control (open-drain output in Hi-Z state)
/// and then be polled it until it is reset by hardware, indicating that the fault condition has
/// disappeared. Note: Any write operation to this bit takes a delay of 1 APB clock cycle to become
/// effective. Position: 26, Width: 1 Access: read-write
...
/// Break Bidirectional In the bidirectional mode (BKBID bit set to 1), the break input is
/// configured both in input mode and in open drain output mode. Any active break event asserts a
/// low logic level on the Break input to indicate an internal break event to external devices.
/// Note: This bit cannot be modified as long as LOCK level 1 has been programmed (LOCK bits in
/// TIMx_BDTR register). Note: Any write operation to this bit takes a delay of 1 APB clock cycle to
/// become effective. Position: 28, Width: 1 Access: read-write
...
namespace tim1_dcr {
/// DMA base address This 5-bits vector defines the base-address for DMA transfers (when read/write
/// access are done through the TIMx_DMAR address). DBA is defined as an offset starting from the
/// address of the TIMx_CR1 register. Example: ... Position: 0, Width: 5 Access: read-write
...
/// DMA burst length This 5-bit vector defines the length of DMA transfers (the timer recognizes a
/// burst transfer when a read or a write access is done to the TIMx_DMAR address), i.e. the number
/// of transfers. Transfers can be in half-words or in bytes (see example below). ... Example: Let
/// us consider the following transfer: DBL = 7 bytes & DBA = TIMx_CR1. If DBL = 7 bytes and DBA =
/// TIMx_CR1 represents the address of the byte to be transferred, the address of the transfer
/// should be given by the following equation: (TIMx_CR1 address) + DBA + (DMA index), where DMA
/// index = DBL In this example, 7 bytes are added to (TIMx_CR1 address) + DBA, which gives us the
/// address from/to which the data is copied. In this case, the transfer is done to 7 registers
/// starting from the following address: (TIMx_CR1 address) + DBA According to the configuration of
/// the DMA Data Size, several cases may occur: If the DMA Data Size is configured in half-words,
/// 16-bit data is transferred to each of the 7 registers. If the DMA Data Size is configured in
/// bytes, the data is also transferred to 7 registers: the first register contains the first MSB
/// byte, the second register, the first LSB byte and so on. So with the transfer Timer, one also
/// has to specify the size of data transferred by DMA. Position: 8, Width: 5 Access: read-write
...
namespace tim1_dmar {
/// DMA register for burst accesses A read or write operation to the DMAR register accesses the
/// register located at the address (TIMx_CR1 address) + (DBA + DMA index) x 4 where TIMx_CR1
/// address is the address of the control register 1, DBA is the DMA base address configured in
/// TIMx_DCR register, DMA index is automatically controlled by the DMA transfer, and ranges from 0
/// to DBL (DBL configured in TIMx_DCR). Position: 0, Width: 32 Access: read-write
...
namespace tim1_ccr5 {
/// Capture/Compare 5 value CCR5 is the value to be loaded in the actual capture/compare 5 register
/// (preload value). It is loaded permanently if the preload feature is not selected in the
/// TIMx_CCMR3 register (bit OC5PE). Else the preload value is copied in the active capture/compare
/// 5 register when an update event occurs. The active capture/compare register contains the value
/// to be compared to the counter TIMx_CNT and signaled on OC5 output. Position: 0, Width: 16
/// Access: read-write
using CCR5 = BitField<0, 16>;
constexpr uint32_t CCR5_Pos = 0;
constexpr uint32_t CCR5_Msk = CCR5::mask;

/// Group Channel 5 and Channel 1 Distortion on Channel 1 output: This bit can either have immediate
/// effect or be preloaded and taken into account after an update event (if preload feature is
/// selected in TIMxCCMR1). Note: it is also possible to apply this distortion on combined PWM
/// signals. Position: 29, Width: 1 Access: read-write
...
/// Group Channel 5 and Channel 2 Distortion on Channel 2 output: This bit can either have immediate
/// effect or be preloaded and taken into account after an update event (if preload feature is
/// selected in TIMxCCMR1). Note: it is also possible to apply this distortion on combined PWM
/// signals. Position: 30, Width: 1 Access: read-write
...
/// Group Channel 5 and Channel 3 Distortion on Channel 3 output: This bit can either have immediate
/// effect or be preloaded and taken into account after an update event (if preload feature is
/// selected in TIMxCCMR2). Note: it is also possible to apply this distortion on combined PWM
/// signals. Position: 31, Width: 1 Access: read-write
...
namespace tim1_af1 {
/// BRK BKIN input enable This bit enables the BKIN alternate function input for the timerâs
/// BRK input. BKIN input is 'ORedâ with the other BRK sources. Note: This bit can not be
/// modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
/// Position: 0, Width: 1 Access: read-write
...
/// BRK COMP1 enable This bit enables the COMP1 for the timerâs BRK input. COMP1 output is
/// 'ORedâ with the other BRK sources. Note: This bit can not be modified as long as LOCK level
/// 1 has been programmed (LOCK bits in TIMx_BDTR register). Position: 1, Width: 1 Access:
/// read-write
...
/// BRK COMP2 enable This bit enables the COMP2 for the timerâs BRK input. COMP2 output is
/// 'ORedâ with the other BRK sources. Note: This bit can not be modified as long as LOCK level
/// 1 has been programmed (LOCK bits in TIMx_BDTR register). Position: 2, Width: 1 Access:
/// read-write
...
/// BRK BKIN input polarity This bit selects the BKIN alternate function input sensitivity. It must
/// be programmed together with the BKP polarity bit. Note: This bit can not be modified as long as
/// LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register). Position: 9, Width: 1
/// Access: read-write
...
/// BRK COMP1 input polarity This bit selects the COMP1 input sensitivity. It must be programmed
/// together with the BKP polarity bit. Note: This bit can not be modified as long as LOCK level 1
/// has been programmed (LOCK bits in TIMx_BDTR register). Position: 10, Width: 1 Access: read-write
...
/// BRK COMP2 input polarity This bit selects the COMP2 input sensitivity. It must be programmed
/// together with the BKP polarity bit. Note: This bit can not be modified as long as LOCK level 1
/// has been programmed (LOCK bits in TIMx_BDTR register). Position: 11, Width: 1 Access: read-write
...
/// ETR source selection These bits select the ETR input source. Others: Reserved Note: These bits
/// can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR
/// register). Position: 14, Width: 4 Access: read-write
...
namespace tim1_af2 {
/// BRK2 BKIN input enable This bit enables the BKIN2 alternate function input for the timerâs
/// BRK2 input. BKIN2 input is 'ORedâ with the other BRK2 sources. Note: This bit can not be
/// modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
/// Position: 0, Width: 1
/// Access: read-write
...
/// BRK2 COMP1 enable This bit enables the COMP1 for the timerâs BRK2 input. COMP1 output is
/// 'ORedâ with the other BRK2 sources. Note: This bit can not be modified as long as LOCK
/// level 1 has been programmed (LOCK bits in TIMx_BDTR register). Position: 1, Width: 1 Access:
/// read-write
...
/// BRK2 COMP2 enable This bit enables the COMP2 for the timerâs BRK2 input. COMP2 output is
/// 'ORedâ with the other BRK2 sources. Note: This bit can not be modified as long as LOCK
/// level 1 has been programmed (LOCK bits in TIMx_BDTR register). Position: 2, Width: 1 Access:
/// read-write
...
/// BRK2 BKIN2 input polarity This bit selects the BKIN2 alternate function input sensitivity. It
/// must be programmed together with the BK2P polarity bit. Note: This bit can not be modified as
/// long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register). Position: 9, Width:
/// 1 Access: read-write
...
/// BRK2 COMP1 input polarity This bit selects the COMP1 input sensitivity. It must be programmed
/// together with the BK2P polarity bit. Note: This bit can not be modified as long as LOCK level 1
/// has been programmed (LOCK bits in TIMx_BDTR register). Position: 10, Width: 1 Access: read-write
...
/// BRK2 COMP2 input polarity This bit selects the COMP2 input sensitivity. It must be programmed
/// together with the BK2P polarity bit. Note: This bit can not be modified as long as LOCK level 1
/// has been programmed (LOCK bits in TIMx_BDTR register). Position: 11, Width: 1 Access: read-write
