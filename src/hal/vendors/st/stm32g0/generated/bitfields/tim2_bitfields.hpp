...
namespace ucore::hal::st::stm32g0::tim2 {
...
/// hardware. CEN is cleared automatically in one-pulse mode, when an update event occurs. Position:
/// 0, Width: 1 Access: read-write
...
/// Center-aligned mode selection Note: It is not allowed to switch from edge-aligned mode to
/// center-aligned mode as long as the counter is enabled (CEN=1) Position: 5, Width: 2 Access:
/// read-write
...
/// frequency and sampling clock used by the digital filters (ETR, TIx), Position: 8, Width: 2
/// Access: read-write
...
/// Master mode selection These bits permit to select the information to be sent in master mode to
/// slave timers for synchronization (TRGO). The combination is as follows: When the Counter Enable
/// signal is controlled by the trigger input, there is a delay on TRGO, except if the master/slave
/// mode is selected (see the MSM bit description in TIMx_SMCR register). Note: The clock of the
/// slave timer or ADC must be enabled prior to receive events from the master timer, and must not
/// be changed on-the-fly while triggers are received from the master timer. Position: 4, Width: 3
/// Access: read-write
...
/// Control Register description. reinitializes the counter, generates an update of the registers
/// and starts the counter. Note: The gated mode must not be used if TI1F_ED is selected as the
/// trigger input (TS=00100). Indeed, TI1F_ED outputs 1 pulse for each transition on TI1F, whereas
/// the gated mode checks the level of the trigger signal. Note: The clock of the slave peripherals
/// (timer, ADC, ...) receiving the TRGO or the TRGO2 signals must be enabled prior to receive
/// events from the master timer, and the clock frequency (prescaler) must not be changed on-the-fly
/// while triggers are received from the master timer. Position: 0, Width: 3 Access: read-write
...
/// External trigger prescaler External trigger signal ETRP frequency must be at most 1/4 of CK_INT
...
/// (timer, ADC, ...) receiving the TRGO or the TRGO2 signals must be enabled prior to receive
/// events from the master timer, and the clock frequency (prescaler) must not be changed on-the-fly
/// while triggers are received from the master timer. Position: 16, Width: 1 Access: read-write
...
/// At overflow or underflow and if UDIS=0 in the TIMx_CR1 register. When CNT is reinitialized by
/// software using the UG bit in TIMx_EGR register, if URS=0 and UDIS=0 in the TIMx_CR1 register.
/// When CNT is reinitialized by a trigger event (refer to the synchro control register
/// description), if URS=0 and UDIS=0 in the TIMx_CR1 register. Position: 0, Width: 1 Access:
/// read-write
...
/// as well as the used input. Note: CC1S bits are writable only when the channel is OFF (CC1E = 0
/// in TIMx_CCER). Position: 0, Width: 2 Access: read-write
...
/// Output compare 1 preload enable Note: The PWM mode can be used without validating the preload
/// register only in one-pulse mode (OPM bit set in TIMx_CR1 register). Else the behavior is not
/// guaranteed. Position: 3, Width: 1 Access: read-write
...
/// on CC1P and CC1NP bits. Note: In PWM mode, the OCREF level changes only when the result of the
/// comparison changes or when the output compare mode switches from âfrozenâ mode to
/// âPWMâ mode. Note: The OC1M[3] bit is not contiguous, located in bit 16. Position: 4,
...
/// as well as the used input. Note: CC2S bits are writable only when the channel is OFF (CC2E = 0
/// in TIMx_CCER). Position: 8, Width: 2 Access: read-write
...
/// as well as the used input. Note: CC3S bits are writable only when the channel is OFF (CC3E = 0
/// in TIMx_CCER). Position: 0, Width: 2 Access: read-write
using CC3S = BitField<0, 2>;
constexpr uint32_t CC3S_Pos = 0;
constexpr uint32_t CC3S_Msk = CC3S::mask;
...
/// as well as the used input. Note: CC4S bits are writable only when the channel is OFF (CC4E = 0
/// in TIMx_CCER). Position: 8, Width: 2 Access: read-write
using CC4S = BitField<8, 2>;
constexpr uint32_t CC4S_Pos = 8;
constexpr uint32_t CC4S_Msk = CC4S::mask;
...
/// CC1NP=1, CC1P=0: This configuration is reserved, it must not be used. Position: 1, Width: 1
/// Access: read-write
...
/// Capture/Compare 1 output Polarity. CC1 channel configured as output: CC1NP must be kept cleared
/// in this case. CC1 channel configured as input: This bit is used in conjunction with CC1P to
/// define TI1FP1/TI2FP1 polarity. refer to CC1P description. Position: 3, Width: 1 Access:
/// read-write
...
namespace arr {
/// High auto-reload value (TIM2) nullLow Auto-reload value ARR is the value to be loaded in the
/// actual auto-reload register. Refer to the for more details about ARR update and behavior. The
/// counter is blocked while the auto-reload value is null. Position: 0, Width: 32 Access:
/// read-write
...
namespace ccr1 {
/// High Capture/Compare 1 value (TIM2) nullLow Capture/Compare 1 value If channel CC1 is configured
/// as output: CCR1 is the value to be loaded in the actual capture/compare 1 register (preload
/// value). It is loaded permanently if the preload feature is not selected in the TIMx_CCMR1
/// register (bit OC1PE). Else the preload value is copied in the active capture/compare 1 register
/// when an update event occurs. The active capture/compare register contains the value to be
/// compared to the counter TIMx_CNT and signaled on OC1 output. If channel CC1is configured as
/// input: CCR1 is the counter value transferred by the last input capture 1 event (IC1). The
/// TIMx_CCR1 register is read-only and cannot be programmed. Position: 0, Width: 32 Access:
/// read-write
...
namespace ccr2 {
/// High Capture/Compare 2 value (TIM2) nullLow Capture/Compare 2 value If channel CC2 is configured
/// as output: CCR2 is the value to be loaded in the actual capture/compare 2 register (preload
/// value). It is loaded permanently if the preload feature is not selected in the TIMx_CCMR1
/// register (bit OC2PE). Else the preload value is copied in the active capture/compare 2 register
/// when an update event occurs. The active capture/compare register contains the value to be
/// compared to the counter TIMx_CNT and signalled on OC2 output. If channel CC2 is configured as
/// input: CCR2 is the counter value transferred by the last input capture 2 event (IC2). The
/// TIMx_CCR2 register is read-only and cannot be programmed. Position: 0, Width: 32 Access:
/// read-write
...
namespace ccr3 {
/// High Capture/Compare 3 value (TIM2) nullLow Capture/Compare value If channel CC3 is configured
/// as output: CCR3 is the value to be loaded in the actual capture/compare 3 register (preload
/// value). It is loaded permanently if the preload feature is not selected in the TIMx_CCMR2
/// register (bit OC3PE). Else the preload value is copied in the active capture/compare 3 register
/// when an update event occurs. The active capture/compare register contains the value to be
/// compared to the counter TIMx_CNT and signalled on OC3 output. If channel CC3is configured as
/// input: CCR3 is the counter value transferred by the last input capture 3 event (IC3). The
/// TIMx_CCR3 register is read-only and cannot be programmed. Position: 0, Width: 32 Access:
/// read-write
...
namespace ccr4 {
/// High Capture/Compare 4 value (TIM2) nullLow Capture/Compare value if CC4 channel is configured
/// as output (CC4S bits): CCR4 is the value to be loaded in the actual capture/compare 4 register
/// (preload value). It is loaded permanently if the preload feature is not selected in the
/// TIMx_CCMR2 register (bit OC4PE). Else the preload value is copied in the active capture/compare
/// 4 register when an update event occurs. The active capture/compare register contains the value
/// to be compared to the counter TIMx_CNT and signalled on OC4 output. if CC4 channel is configured
/// as input (CC4S bits in TIMx_CCMR4 register): CCR4 is the counter value transferred by the last
/// input capture 4 event (IC4). The TIMx_CCR4 register is read-only and cannot be programmed.
/// Position: 0, Width: 32
/// Access: read-write
...
namespace dcr {
/// DMA base address This 5-bit vector defines the base-address for DMA transfers (when read/write
...
/// DBL = 7 transfers & DBA = TIMx_CR1. In this case the transfer is done to/from 7 registers
...
/// DMA burst length This 5-bit vector defines the number of DMA transfers (the timer recognizes a
...
namespace tisel {
/// TI1[0] to TI1[15] input selection These bits select the TI1[0] to TI1[15] input source. Others:
/// Reserved Position: 0, Width: 4 Access: read-write
...
/// TI2[0] to TI2[15] input selection These bits select the TI2[0] to TI2[15] input source. Others:
/// Reserved Position: 8, Width: 4 Access: read-write
