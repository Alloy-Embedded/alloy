...
namespace ucore::hal::st::stm32g0::tim6 {
...
namespace cr1 {
/// Counter enable Note: Gated mode can work only if the CEN bit has been previously set by
/// software. However trigger mode can set the CEN bit automatically by hardware. CEN is cleared
/// automatically in one-pulse mode, when an update event occurs. Position: 0, Width: 1 Access:
/// read-write
...
namespace cr2 {
/// Master mode selection These bits are used to select the information to be sent in master mode to
...
/// mode is selected (see the MSM bit description in the TIMx_SMCR register). Note: The clock of the
...
/// register. When CNT is reinitialized by software using the UG bit in the TIMx_EGR register, if
/// URS = 0 and UDIS = 0 in the TIMx_CR1 register. Position: 0, Width: 1 Access: read-write
...
/// bit in TIMx_CR1 is reset, bit 31 is reserved and read as 0. Position: 31, Width: 1 Access:
/// read-only
