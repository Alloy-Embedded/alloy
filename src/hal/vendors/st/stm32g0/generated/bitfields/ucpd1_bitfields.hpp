...
/// Division ratio for producing half-bit clock The bitfield determines the division ratio (the
/// bitfield value plus one) of a ucpd_clk divider producing half-bit clock (hbit_clk). Position: 0,
/// Width: 6 Access: read-write
...
/// ratio (the bitfield value minus one) of a ucpd_clk divider producing inter-frame gap timer clock
/// (tInterFrameGap). The division ratio 15 is to apply for Tx clock at the USB PD 2.0 specification
/// nominal value. The division ratios below 15 are to apply for Tx clock below nominal, and the
/// division ratios above 15 for Tx clock above nominal. Position: 6, Width: 5 Access: read-write
...
/// Transition window duration The bitfield determines the division ratio (the bitfield value minus
/// one) of a hbit_clk divider producing tTransitionWindow interval. Set a value that produces an
/// interval of 12 to 20 us, taking into account the ucpd_clk frequency and the HBITCLKDIV[5:0]
/// bitfield setting. Position: 11, Width: 5 Access: read-write
...
/// Pre-scaler division ratio for generating ucpd_clk The bitfield determines the division ratio of
/// a kernel clock pre-scaler producing UCPD peripheral clock (ucpd_clk). It is recommended to use
/// the pre-scaler so as to set the ucpd_clk frequency in the range from 6 to 9 MHz. Position: 17,
...
/// Receiver ordered set enable The bitfield determines the types of ordered sets that the receiver
/// must detect. When set/cleared, each bit enables/disables a specific function: 0bxxxxxxxx1: SOP
/// detect enabled 0bxxxxxxx1x: SOP' detect enabled 0bxxxxxx1xx: SOP'' detect enabled 0bxxxxx1xxx:
/// Hard Reset detect enabled 0bxxxx1xxxx: Cable Detect reset enabled 0bxxx1xxxxx: SOP'_Debug
/// enabled 0bxx1xxxxxx: SOP''_Debug enabled 0bx1xxxxxxx: SOP extension#1 enabled 0b1xxxxxxxx: SOP
/// extension#2 enabled Position: 20, Width: 9 Access: read-write
...
/// UCPD peripheral enable General enable of the UCPD peripheral. Upon disabling, the peripheral
/// instantly quits any ongoing activity and all control bits and bitfields default to their reset
/// values. They must be set to their desired values each time the peripheral transits from disabled
/// to enabled state. Position: 31, Width: 1 Access: read-write
...
/// BMC decoder Rx pre-filter enable The sampling clock is that of the receiver (that is, after
/// pre-scaler). Position: 0, Width: 1 Access: read-write
...
/// BMC decoder Rx pre-filter sampling method Number of consistent consecutive samples before
/// confirming a new value. Position: 1, Width: 1 Access: read-write
...
namespace ucpd_cr {
/// Type of Tx packet Writing the bitfield triggers the action as follows, depending on the value:
/// Others: invalid From V1.1 of the USB PD specification, there is a counter defined for the
/// duration of the BIST Carrier Mode 2. To quit this mode correctly (after the "tBISTContMode"
/// delay), disable the peripheral (UCPDEN = 0). Position: 0, Width: 2 Access: read-write
...
/// Command to send a Tx packet The bit is cleared by hardware as soon as the packet transmission
/// begins or is discarded. Position: 2, Width: 1 Access: read-write
...
/// Command to send a Tx Hard Reset The bit is cleared by hardware as soon as the message
/// transmission begins or is discarded. Position: 3, Width: 1 Access: read-write
...
/// Receiver mode Determines the mode of the receiver. When the bit is set, RXORDSET behaves
/// normally, RXDR no longer receives bytes yet the CRC checking still proceeds as for a normal
/// message. Position: 4, Width: 1 Access: read-write
...
/// USB Power Delivery receiver enable Both CC1 and CC2 receivers are disabled when the bit is
/// cleared. Only the CC receiver selected via the PHYCCSEL bit is enabled when the bit is set.
/// Position: 5, Width: 1
/// Access: read-write
...
/// CC1/CC2 line selector for USB Power Delivery signaling The selection depends on the cable
/// orientation as discovered at attach. Position: 6, Width: 1 Access: read-write
...
/// Analog PHY operating mode The bit takes effect upon setting the UCPDx_STROBE bit of the
/// SYS_CONFIG register. The use of CC1 and CC2 depends on CCENABLE. Refer to ANAMODE, ANASUBMODE
/// and link with TYPEC_VSTATE_CCx for the effect of this bitfield in conjunction with
/// ANASUBMODE[1:0]. Position: 9, Width: 1 Access: read-write
...
/// CC line enable This bitfield enables CC1 and CC2 line analog PHYs (pull-ups and pull-downs)
/// according to ANAMODE and ANASUBMODE[1:0] setting. A single line PHY can be enabled when, for
/// example, the other line is driven by VCONN via an external VCONN switch. Enabling both PHYs is
/// the normal usage for sink/source. Position: 10, Width: 2 Access: read-write
...
/// Dead battery function enable The bit takes effect upon setting the USBPDstrobe bit of the
/// SYS_CONFIG register. Dead battery function only operates if the external circuit is
/// appropriately configured. Position: 15, Width: 1 Access: read-write
...
/// FRS event detection enable Setting the bit enables FRS Rx event (FRSEVT) detection on the CC
/// line selected through the PHYCCSEL bit. 0: Disable Clear the bit when the device is attached to
/// an FRS-incapable source/sink. Position: 16, Width: 1 Access: read-write
...
/// FRS Tx signaling enable. Setting the bit enables FRS Tx signaling. The bit is cleared by
/// hardware after a delay respecting the USB Power Delivery specification Revision 3.0. Position:
/// 17, Width: 1 Access: read-write
...
/// Rdch condition drive The bit drives Rdch condition on the CC line selected through the PHYCCSEL
/// bit (thus associated with VCONN), by remaining set during the source-only UnattachedWait.SRC
/// state, to respect the Type-C state. Refer to "USB Type-C ECN for Source VCONN Discharge". The
/// CCENABLE[1:0] bitfield must be set accordingly, too. Changing the bit value only takes effect
/// upon setting the UCPDx_STROBE bit of the SYSCFG_CFGR1 register. Position: 18, Width: 1 Access:
/// read-write
...
/// CC1 Type-C detector disable The bit disables the Type-C detector on the CC1 line. When enabled,
/// the Type-C detector for CC1 is configured through ANAMODE and ANASUBMODE[1:0]. Position: 20,
...
/// CC2 Type-C detector disable The bit disables the Type-C detector on the CC2 line. When enabled,
/// the Type-C detector for CC2 is configured through ANAMODE and ANASUBMODE[1:0]. Position: 21,
/// Width: 1 Access: read-write
...
namespace ucpd_sr {
/// Transmit interrupt status The flag indicates that the UCPD_TXDR register is empty and new data
/// write is required (as the amount of data sent has not reached the payload size defined in the
/// TXPAYSZ bitfield). The flag is cleared with the data write into the UCPD_TXDR register.
/// Position: 0, Width: 1
/// Access: read-only
...
/// Message transmission discarded The flag indicates that a message transmission was dropped. The
/// flag is cleared by setting the TXMSGDISCCF bit. Transmission of a message can be dropped if
/// there is a concurrent receive in progress or at excessive noise on the line. After a Tx message
/// is discarded, the flag is only raised when the CC line becomes idle. Position: 1, Width: 1
/// Access: read-only
...
/// Message transmission completed The flag indicates the completion of packet transmission. It is
/// cleared by setting the TXMSGSENTCF bit. In the event of a message transmission interrupted by a
/// Hard Reset, the flag is not raised. Position: 2, Width: 1 Access: read-only
...
/// Transmit message abort The flag indicates that a Tx message is aborted due to a subsequent Hard
/// Reset message send request taking priority during transmit. It is cleared by setting the
/// TXMSGABTCF bit. Position: 3, Width: 1 Access: read-only
...
/// Hard Reset discarded The flag indicates that the Hard Reset message is discarded. The flag is
/// cleared by setting the HRSTDISCCF bit. Position: 4, Width: 1 Access: read-only
...
/// Hard Reset message sent The flag indicates that the Hard Reset message is sent. The flag is
/// cleared by setting the HRSTSENTCF bit. Position: 5, Width: 1 Access: read-only
...
/// Tx data underrun detection The flag indicates that the Tx data register (UCPD_TXDR) was not
/// written in time for a transmit message to execute normally. It is cleared by setting the TXUNDCF
/// bit. Position: 6, Width: 1 Access: read-only
...
/// Receive data register not empty detection The flag indicates that the UCPD_RXDR register is not
/// empty. It is automatically cleared upon reading UCPD_RXDR. Position: 8, Width: 1 Access:
/// read-only
...
/// Rx ordered set (4 K-codes) detection The flag indicates the detection of an ordered set. The
/// relevant information is stored in the RXORDSET[2:0] bitfield of the UCPD_RX_ORDSET register. It
/// is cleared by setting the RXORDDETCF bit. Position: 9, Width: 1 Access: read-only
...
/// Rx Hard Reset receipt detection The flag indicates the receipt of valid Hard Reset message. It
/// is cleared by setting the RXHRSTDETCF bit. Position: 10, Width: 1 Access: read-only
...
/// Rx data overflow detection The flag indicates Rx data buffer overflow. It is cleared by setting
/// the RXOVRCF bit. The buffer overflow can occur if the received data are not read fast enough.
/// Position: 11, Width: 1
/// Access: read-only
...
/// Rx message received The flag indicates whether a message (except Hard Reset message) has been
/// received, regardless the CRC value. The flag is cleared by setting the RXMSGENDCF bit. The RXERR
/// flag set when the RXMSGEND flag goes high indicates errors in the last-received message.
/// Position: 12, Width: 1
/// Access: read-only
...
/// Receive message error The flag indicates errors of the last Rx message declared (via RXMSGEND),
/// such as incorrect CRC or truncated message (a line becoming static before EOP is met). It is
/// asserted whenever the RXMSGEND flag is set. Position: 13, Width: 1 Access: read-only
...
/// bitfield value, which corresponds to a new Type-C event. It is cleared by setting the
/// TYPECEVT2CF bit. Position: 14, Width: 1 Access: read-only
...
/// bitfield value, which corresponds to a new Type-C event. It is cleared by setting the
/// TYPECEVT2CF bit. Position: 15, Width: 1 Access: read-only
...
/// The status bitfield indicates the voltage level on the CC1 line in its steady state. The voltage
/// variation on the CC1 line during USB PD messages due to the BMC PHY modulation does not impact
/// the bitfield value. Position: 16, Width: 2 Access: read-only
...
/// CC2 line voltage level The status bitfield indicates the voltage level on the CC2 line in its
/// steady state. The voltage variation on the CC2 line during USB PD messages due to the BMC PHY
/// modulation does not impact the bitfield value. Position: 18, Width: 2 Access: read-only
...
namespace ucpd_icr {
/// Tx message discard flag (TXMSGDISC) clear Setting the bit clears the TXMSGDISC flag in the
/// UCPD_SR register. Position: 1, Width: 1 Access: write-only
using TXMSGDISCCF = BitField<1, 1>;
constexpr uint32_t TXMSGDISCCF_Pos = 1;
constexpr uint32_t TXMSGDISCCF_Msk = TXMSGDISCCF::mask;

/// Tx message send flag (TXMSGSENT) clear Setting the bit clears the TXMSGSENT flag in the UCPD_SR
/// register. Position: 2, Width: 1 Access: write-only
using TXMSGSENTCF = BitField<2, 1>;
constexpr uint32_t TXMSGSENTCF_Pos = 2;
constexpr uint32_t TXMSGSENTCF_Msk = TXMSGSENTCF::mask;

/// Tx message abort flag (TXMSGABT) clear Setting the bit clears the TXMSGABT flag in the UCPD_SR
/// register. Position: 3, Width: 1 Access: write-only
using TXMSGABTCF = BitField<3, 1>;
constexpr uint32_t TXMSGABTCF_Pos = 3;
constexpr uint32_t TXMSGABTCF_Msk = TXMSGABTCF::mask;

/// Hard reset discard flag (HRSTDISC) clear Setting the bit clears the HRSTDISC flag in the UCPD_SR
...
/// Hard reset send flag (HRSTSENT) clear Setting the bit clears the HRSTSENT flag in the UCPD_SR
/// register. Position: 5, Width: 1 Access: write-only
...
/// Rx ordered set detect flag (RXORDDET) clear Setting the bit clears the RXORDDET flag in the
/// UCPD_SR register. Position: 9, Width: 1 Access: write-only
using RXORDDETCF = BitField<9, 1>;
constexpr uint32_t RXORDDETCF_Pos = 9;
constexpr uint32_t RXORDDETCF_Msk = RXORDDETCF::mask;

/// Rx Hard Reset detect flag (RXHRSTDET) clear Setting the bit clears the RXHRSTDET flag in the
/// UCPD_SR register. Position: 10, Width: 1 Access: write-only
...
/// Rx message received flag (RXMSGEND) clear Setting the bit clears the RXMSGEND flag in the
/// UCPD_SR register. Position: 12, Width: 1 Access: write-only
using RXMSGENDCF = BitField<12, 1>;
constexpr uint32_t RXMSGENDCF_Pos = 12;
constexpr uint32_t RXMSGENDCF_Msk = RXMSGENDCF::mask;

/// Type-C CC1 event flag (TYPECEVT1) clear Setting the bit clears the TYPECEVT1 flag in the UCPD_SR
/// register Position: 14, Width: 1 Access: write-only
using TYPECEVT1CF = BitField<14, 1>;
constexpr uint32_t TYPECEVT1CF_Pos = 14;
constexpr uint32_t TYPECEVT1CF_Msk = TYPECEVT1CF::mask;

/// Type-C CC2 line event flag (TYPECEVT2) clear Setting the bit clears the TYPECEVT2 flag in the
/// UCPD_SR register Position: 15, Width: 1 Access: write-only
...
/// Ordered set to transmit The bitfield determines a full 20-bit sequence to transmit, consisting
/// of four K-codes, each of five bits, defining the packet to transmit. The bit 0 (bit 0 of
/// K-code1) is the first, the bit 19 (bit 4 of K‑code4) the last. Position: 0, Width: 20 Access:
/// read-write
...
/// Payload size yet to transmit The bitfield is modified by software and by hardware. It contains
/// the number of bytes of a payload (including header but excluding CRC) yet to transmit: each time
/// a data byte is written into the UCPD_TXDR register, the bitfield value decrements and the TXIS
/// bit is set, except when the bitfield value reaches zero. The enumerated values are standard
/// payload sizes before the start of transmission. Position: 0, Width: 10 Access: read-write
...
/// Rx payload size received This bitfield contains the number of bytes of a payload (including
/// header but excluding CRC) received: each time a new data byte is received in the UCPD_RXDR
/// register, the bitfield value increments and the RXMSGEND flag is set (and an interrupt generated
/// if enabled). The bitfield may return a spurious value when a byte reception is ongoing (the
/// RXMSGEND flag is low). Position: 0, Width: 10 Access: read-only
...
/// Ordered set 1 received The bitfield contains a full 20-bit sequence received, consisting of four
/// K‑codes, each of five bits. The bit 0 (bit 0 of K‑code1) is receive first, the bit 19 (bit 4 of
/// K‑code4) last. Position: 0, Width: 20 Access: read-write
...
/// Ordered set 2 received The bitfield contains a full 20-bit sequence received, consisting of four
/// K‑codes, each of five bits. The bit 0 (bit 0 of K‑code1) is receive first, the bit 19 (bit 4 of
/// K‑code4) last. Position: 0, Width: 20 Access: read-write
