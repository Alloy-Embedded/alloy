...
/// Voltage reference buffer mode enable This bit is used to enable the voltage reference buffer
/// mode. Position: 0, Width: 1 Access: read-write
...
/// High impedance mode This bit controls the analog switch to connect or not the VREF+ pin. Refer
/// to for the mode descriptions depending on ENVR bit configuration. Position: 1, Width: 1 Access:
/// read-write
...
/// Trimming code These bits are automatically initialized after reset with the trimming value
/// stored in the Flash memory during the production test. Writing into these bits allows the tuning
/// of the internal reference buffer voltage. Position: 0, Width: 6 Access: read-write
