#pragma once

// can_event::token<PeripheralId, InterruptKind> — static completion signal
// per (peripheral, interrupt-kind) pair.
//
// Vendor CAN ISR hooks call `can_event::token<P, Kind>::signal()`.
// `async::can::wait_for<Kind>(handle)` resets the token, arms the
// interrupt, and returns operation<can_event::token<P, Kind>>.

#include "device/runtime.hpp"
#include "hal/can.hpp"
#include "runtime/event.hpp"

#if ALLOY_DEVICE_CAN_SEMANTICS_AVAILABLE

namespace alloy::runtime::can_event {

/// Tag type — uniquely identifies one (peripheral, interrupt kind) pair.
template <device::PeripheralId Peripheral, hal::can::InterruptKind Kind>
struct tag {
    static constexpr auto peripheral = Peripheral;
    static constexpr auto kind       = Kind;
};

/// Completion token. Distinct static state per (Peripheral, Kind) pair.
template <device::PeripheralId Peripheral, hal::can::InterruptKind Kind>
using token = event::completion<tag<Peripheral, Kind>>;

}  // namespace alloy::runtime::can_event

#endif  // ALLOY_DEVICE_CAN_SEMANTICS_AVAILABLE
