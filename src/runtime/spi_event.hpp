...
// spi_event::token<PeripheralId, InterruptKind> — static completion signal
...
// `signaled` flag.  The vendor SPI ISR calls:
//
//   spi_event::token<P, InterruptKind::CrcError>::signal();
//
// The async::spi::wait_for<Kind>(port) wrapper resets the token, arms the
// interrupt via port.enable_interrupt(Kind), then returns
// operation<spi_event::token<P, Kind>> which the caller polls or awaits.
//
...
#include "hal/spi/detail/backend.hpp"  // InterruptKind enum
#include "runtime/event.hpp"

namespace alloy::runtime::spi_event {

/// Tag type — uniquely identifies one (peripheral, interrupt kind) combination.
template <device::PeripheralId Peripheral, hal::spi::InterruptKind Kind>
struct tag {
