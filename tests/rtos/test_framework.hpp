    ...
    do {                                                                                       \
        ...
        if (_value < _min || _value > _max) {                                                  \
            ...
            printf("    Value: %lu, Min: %lu, Max: %lu\n", static_cast<unsigned long>(_value), \
                   static_cast<unsigned long>(_min), static_cast<unsigned long>(_max));        \
            printf("    at %s:%d\n", __FILE__, __LINE__);                                      \
            return;                                                                            \
        }                                                                                      \
    } while (0)
...
class PerfTimer {
   public:
    ...
    core::u32 elapsed_us() const { return hal::SysTick::micros() - start_; }

    core::u32 elapsed_ms() const { return elapsed_us() / 1000; }

    void reset() { start_ = hal::SysTick::micros(); }
...
inline void print_test_summary() {
    ...
    printf("Passed:  %lu tests (%u%%)\n", ctx.stats.passed_tests, ctx.stats.pass_percentage());
