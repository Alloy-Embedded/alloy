#!/usr/bin/env python3
"""
Generate Platform-Level Adc Implementation

This generator creates type-safe, zero-overhead Adc classes for different
...
Usage:
    python3 generate_adc.py --family same70 --output src/hal/platform/same70/adc.hpp
    python3 generate_adc.py --family stm32f4 --output src/hal/platform/stm32f4/adc.hpp
"""
...
def load_metadata(family: str) -> Dict[str, Any]:
    """
    Load Adc metadata for a specific family.
    ...
    """
    metadata_file = SCRIPT_DIR / "metadata" / f"{family}_adc.json"
    ...
    print_success(f"Loaded metadata for {metadata['family']} Adc")
...
def list_available_families() -> list:
    """
    List all available Adc metadata files.
    ...
    families = []
    for file in metadata_dir.glob("*_adc.json"):
        family_name = file.stem.replace('_adc', '')
...
def generate_adc(family: str, output_path: Path = None, dry_run: bool = False) -> bool:
    """
    Generate Adc implementation for a family.
    ...
    try:
        ...
        template = env.get_template('adc.hpp.j2')
        ...
        if not output_path:
            output_path = Path(f"src/hal/platform/{family}/adc.hpp")
        ...
        print_success(f"Successfully generated Adc for {family}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generate platform-level Adc implementation',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Generate for SAME70
  python3 generate_adc.py --family same70

  # Generate for STM32F4 with custom output
  python3 generate_adc.py --family stm32f4 --output custom/path/adc.hpp

  # Dry run (preview only)
  python3 generate_adc.py --family same70 --dry-run

  # List available families
  python3 generate_adc.py --list
        """
    )
    ...
    parser.add_argument(
        '--output', '-o',
        type=Path,
        help='Output file path (default: src/hal/platform/{family}/adc.hpp)'
    )
    ...
    # Print header
    print_header("Platform Adc Generator")
