#!/usr/bin/env python3
"""
Generate Platform-Level Clock Implementation

This generator creates type-safe, zero-overhead Clock classes for different
MCU families by reading metadata JSON files and applying Jinja2 templates.

Features:
- Singleton clock management
...
Usage:
    python3 generate_clock.py --family same70 --output src/hal/platform/same70/clock.hpp
    python3 generate_clock.py --family stm32f4 --output src/hal/platform/stm32f4/clock.hpp
"""
...
def load_metadata(family: str) -> Dict[str, Any]:
    """
    Load Clock metadata for a specific family.
    ...
    """
    metadata_file = SCRIPT_DIR / "metadata" / f"{family}_clock.json"
    ...
    print_success(f"Loaded metadata for {metadata['family']} Clock")
...
def list_available_families() -> list:
    """
    List all available Clock metadata files.
    ...
    families = []
    for file in metadata_dir.glob("*_clock.json"):
        family_name = file.stem.replace('_clock', '')
...
def generate_clock(family: str, output_path: Path = None, dry_run: bool = False) -> bool:
    """
    Generate Clock implementation for a family.
    ...
    try:
        ...
        template = env.get_template('clock.hpp.j2')
        ...
        if not output_path:
            output_path = Path(f"src/hal/platform/{family}/clock.hpp")
        ...
        print_success(f"Successfully generated Clock for {family}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generate platform-level Clock implementation',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Generate for SAME70
  python3 generate_clock.py --family same70

  # Generate for STM32F4 with custom output
  python3 generate_clock.py --family stm32f4 --output custom/path/clock.hpp

  # Dry run (preview only)
  python3 generate_clock.py --family same70 --dry-run

  # List available families
  python3 generate_clock.py --list
        """
    )
    ...
    parser.add_argument(
        '--output', '-o',
        type=Path,
        help='Output file path (default: src/hal/platform/{family}/clock.hpp)'
    )
    ...
    # Print header
    print_header("Platform Clock Generator")
