#!/usr/bin/env python3
"""
Generate Platform-Level Dma Implementation

This generator creates type-safe, zero-overhead Dma classes for different
...
Usage:
    python3 generate_dma.py --family same70 --output src/hal/platform/same70/dma.hpp
    python3 generate_dma.py --family stm32f4 --output src/hal/platform/stm32f4/dma.hpp
"""
...
def load_metadata(family: str) -> Dict[str, Any]:
    """
    Load Dma metadata for a specific family.
    ...
    """
    metadata_file = SCRIPT_DIR / "metadata" / f"{family}_dma.json"
    ...
    print_success(f"Loaded metadata for {metadata['family']} Dma")
...
def list_available_families() -> list:
    """
    List all available Dma metadata files.
    ...
    families = []
    for file in metadata_dir.glob("*_dma.json"):
        family_name = file.stem.replace('_dma', '')
...
def generate_dma(family: str, output_path: Path = None, dry_run: bool = False) -> bool:
    """
    Generate Dma implementation for a family.
    ...
    try:
        ...
        template = env.get_template('dma.hpp.j2')
        ...
        if not output_path:
            output_path = Path(f"src/hal/platform/{family}/dma.hpp")
        ...
        print_success(f"Successfully generated Dma for {family}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generate platform-level Dma implementation',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Generate for SAME70
  python3 generate_dma.py --family same70

  # Generate for STM32F4 with custom output
  python3 generate_dma.py --family stm32f4 --output custom/path/dma.hpp

  # Dry run (preview only)
  python3 generate_dma.py --family same70 --dry-run

  # List available families
  python3 generate_dma.py --list
        """
    )
    ...
    parser.add_argument(
        '--output', '-o',
        type=Path,
        help='Output file path (default: src/hal/platform/{family}/dma.hpp)'
    )
    ...
    # Print header
    print_header("Platform Dma Generator")
