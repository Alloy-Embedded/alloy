#!/usr/bin/env python3
"""
Generate Platform-Level GPIO Implementation

This generator creates type-safe, zero-overhead GPIO classes for different
...
Usage:
    python3 generate_gpio.py --family same70 --output src/hal/platform/same70/gpio.hpp
    python3 generate_gpio.py --family stm32f4 --output src/hal/platform/stm32f4/gpio.hpp
"""
...
def load_metadata(family: str) -> Dict[str, Any]:
    """
    Load GPIO metadata for a specific family.
    ...
    """
    metadata_file = SCRIPT_DIR / "metadata" / f"{family}_gpio.json"
    ...
    required_fields = [
        ...
        'port_bases', 'registers', 'operations', 'examples'
    ]
    ...
    print_success(f"Loaded metadata for {metadata['family']} GPIO")
    print_info(f"  Vendor: {metadata['vendor']}")
    print_info(f"  Peripheral: {metadata['peripheral_name']}")
    print_info(f"  Ports: {len(metadata['port_bases'])}")
...
def list_available_families() -> list:
    """
    List all available GPIO metadata files.
    ...
    families = []
    for file in metadata_dir.glob("*_gpio.json"):
        family_name = file.stem.replace('_gpio', '')
...
def generate_gpio(family: str, output_path: Path = None, dry_run: bool = False) -> bool:
    """
    Generate GPIO implementation for a family.
    ...
    try:
        ...
        template = env.get_template('gpio.hpp.j2')
        ...
        if not output_path:
            output_path = Path(f"src/hal/platform/{family}/gpio.hpp")
        ...
        print_success(f"Successfully generated GPIO for {family}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generate platform-level GPIO implementation',
...
Examples:
  ...
  python3 generate_gpio.py --family stm32f4 --output custom/path/gpio.hpp

  # Dry run (preview only)
  python3 generate_gpio.py --family same70 --dry-run

  # List available families
  python3 generate_gpio.py --list
        """
    )
    ...
    parser.add_argument(
        '--output', '-o',
        type=Path,
        help='Output file path (default: src/hal/platform/{family}/gpio.hpp)'
    )
    ...
    # Print header
    print_header("Platform GPIO Generator")
