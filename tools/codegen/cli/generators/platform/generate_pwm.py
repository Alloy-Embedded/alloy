#!/usr/bin/env python3
"""
Generate Platform-Level Pwm Implementation

This generator creates type-safe, zero-overhead Pwm classes for different
...
Usage:
    python3 generate_pwm.py --family same70 --output src/hal/platform/same70/pwm.hpp
    python3 generate_pwm.py --family stm32f4 --output src/hal/platform/stm32f4/pwm.hpp
"""
...
def load_metadata(family: str) -> Dict[str, Any]:
    """
    Load Pwm metadata for a specific family.
    ...
    """
    metadata_file = SCRIPT_DIR / "metadata" / f"{family}_pwm.json"
    ...
    print_success(f"Loaded metadata for {metadata['family']} Pwm")
...
def list_available_families() -> list:
    """
    List all available Pwm metadata files.
    ...
    families = []
    for file in metadata_dir.glob("*_pwm.json"):
        family_name = file.stem.replace('_pwm', '')
...
def generate_pwm(family: str, output_path: Path = None, dry_run: bool = False) -> bool:
    """
    Generate Pwm implementation for a family.
    ...
    try:
        ...
        template = env.get_template('pwm.hpp.j2')
        ...
        if not output_path:
            output_path = Path(f"src/hal/platform/{family}/pwm.hpp")
        ...
        print_success(f"Successfully generated Pwm for {family}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generate platform-level Pwm implementation',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Generate for SAME70
  python3 generate_pwm.py --family same70

  # Generate for STM32F4 with custom output
  python3 generate_pwm.py --family stm32f4 --output custom/path/pwm.hpp

  # Dry run (preview only)
  python3 generate_pwm.py --family same70 --dry-run

  # List available families
  python3 generate_pwm.py --list
        """
    )
    ...
    parser.add_argument(
        '--output', '-o',
        type=Path,
        help='Output file path (default: src/hal/platform/{family}/pwm.hpp)'
    )
    ...
    # Print header
    print_header("Platform Pwm Generator")
