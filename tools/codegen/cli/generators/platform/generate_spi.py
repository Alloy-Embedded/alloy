#!/usr/bin/env python3
"""
Generate Platform-Level SPI Implementation

This generator creates type-safe, zero-overhead SPI classes for different
...
Usage:
    python3 generate_spi.py --family same70 --output src/hal/platform/same70/spi.hpp
    python3 generate_spi.py --family stm32f4 --output src/hal/platform/stm32f4/spi.hpp
"""
...
def load_metadata(family: str) -> Dict[str, Any]:
    """
    Load SPI metadata for a specific family.
    ...
    """
    metadata_file = SCRIPT_DIR / "metadata" / f"{family}_spi.json"
    ...
    print_success(f"Loaded metadata for {metadata['family']} SPI")
...
def list_available_families() -> list:
    """
    List all available SPI metadata files.
    ...
    families = []
    for file in metadata_dir.glob("*_spi.json"):
        family_name = file.stem.replace('_spi', '')
...
def generate_spi(family: str, output_path: Path = None, dry_run: bool = False) -> bool:
    """
    Generate SPI implementation for a family.
    ...
    try:
        ...
        template = env.get_template('spi.hpp.j2')
        ...
        if not output_path:
            output_path = Path(f"src/hal/platform/{family}/spi.hpp")
        ...
        print_success(f"Successfully generated SPI for {family}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generate platform-level SPI implementation',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Generate for SAME70
  python3 generate_spi.py --family same70

  # Generate for STM32F4 with custom output
  python3 generate_spi.py --family stm32f4 --output custom/path/spi.hpp

  # Dry run (preview only)
  python3 generate_spi.py --family same70 --dry-run

  # List available families
  python3 generate_spi.py --list
        """
    )
    ...
    parser.add_argument(
        '--output', '-o',
        type=Path,
        help='Output file path (default: src/hal/platform/{family}/spi.hpp)'
    )
    ...
    # Print header
    print_header("Platform SPI Generator")
