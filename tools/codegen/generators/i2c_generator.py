#!/usr/bin/env python3
"""
I2C Template Generator

Generates I2C hardware policy code from metadata using Jinja2 templates.

Usage:
    python i2c_generator.py <platform_name>
    python i2c_generator.py --all

Examples:
    python i2c_generator.py stm32f4
    python i2c_generator.py same70
    python i2c_generator.py --all

Features:
    - Generates I2C hardware policies from metadata
    - Validates metadata against JSON schema
    - Supports multiple I2C styles (STM32, SAM)
...
class I2cGenerator:
    """I2C code generator using Jinja2 templates."""
    ...
    def load_i2c_metadata(self, platform_name: str) -> Dict[str, Any]:
        """
        Load I2C metadata for a platform.

        Args:
            platform_name: Platform name

        Returns:
            I2C metadata dictionary
        """
        platform_dir = self.metadata_dir / "platforms" / platform_name
        i2c_file = platform_dir / "i2c.json"

        if not i2c_file.exists():
            raise FileNotFoundError(f"I2C metadata not found: {i2c_file}")
    ...
    def prepare_template_context(
        ...
        Args:
            ...
            i2c_data: I2C metadata

        Returns:
            Template context dictionary
        """
        # Extract template variables
        template_vars = i2c_data.get("template_variables", {})
        ...
        context = {
            ...
            # I2C-specific
            "i2c": {
                "name": i2c_data["peripheral"]["name"],
                "type": i2c_data["peripheral"]["type"],
                "description": i2c_data["peripheral"].get("description", ""),
                "instances": i2c_data["instances"],
                "registers": i2c_data["registers"],
                "capabilities": i2c_data.get("capabilities", {}),
                "style": template_vars.get("style", "stm32"),
                "register_type": template_vars.get("register_type", "USART_TypeDef"),
                "baud_rates": i2c_data.get("i2c_specific", {}).get("baud_rates", []),
                "data_bits": i2c_data.get("i2c_specific", {}).get("data_bits", []),
                "parity": i2c_data.get("i2c_specific", {}).get("parity", []),
            },
            ...
            "generator_name": "i2c_generator.py",
            "generator_version": "1.0.0",
            "metadata_file": f"tools/codegen/metadata/platforms/{platform_name}/i2c.json",
        }

        return context

    def generate_i2c(self, platform_name: str) -> bool:
        """
        Generate I2C code for a platform.
        ...
        """
        print(f"📦 Generating I2C for {platform_name}...")

        try:
            # Load metadata
            platform_data = self.load_platform_metadata(platform_name)
            i2c_data = self.load_i2c_metadata(platform_name)

            # Validate
            if not self.validate_metadata(i2c_data, "peripheral"):
                return False
            ...
            # Load template
            try:
                template = self.jinja_env.get_template("platform/i2c.hpp.j2")
            except TemplateNotFound:
                print(f"❌ Template not found: platform/i2c.hpp.j2")
                return False
            ...
            output_file = output_dir / "i2c.hpp"
            ...
            print(f"❌ Error generating I2C: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def list_platforms(self) -> List[str]:
        """
        List all available platforms with I2C metadata.
    ...
    def generate_all(self) -> bool:
        """
        Generate I2C code for all platforms.
        ...
        if not platforms:
            print("❌ No platforms with I2C metadata found")
            return False
        ...
        for platform in platforms:
            if not self.generate_i2c(platform):
                success = False
            print()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate I2C hardware policies from metadata",
...
Examples:
  ...
  List available platforms:
    ...
    # Create generator
    generator = I2cGenerator(project_root)
    ...
    success = generator.generate_i2c(args.platform)
    sys.exit(0 if success else 1)
