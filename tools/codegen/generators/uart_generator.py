#!/usr/bin/env python3
"""
UART Template Generator

Generates UART hardware policy code from metadata using Jinja2 templates.

Usage:
    python uart_generator.py <platform_name>
    python uart_generator.py --all

Examples:
    python uart_generator.py stm32f4
    python uart_generator.py same70
    python uart_generator.py --all

Features:
    - Generates UART hardware policies from metadata
    - Validates metadata against JSON schema
    - Supports multiple UART styles (STM32, SAM)
...
class UartGenerator:
    """UART code generator using Jinja2 templates."""
    ...
    def load_uart_metadata(self, platform_name: str) -> Dict[str, Any]:
        """
        Load UART metadata for a platform.

        Args:
            platform_name: Platform name

        Returns:
            UART metadata dictionary
        """
        platform_dir = self.metadata_dir / "platforms" / platform_name
        uart_file = platform_dir / "uart.json"

        if not uart_file.exists():
            raise FileNotFoundError(f"UART metadata not found: {uart_file}")
    ...
    def prepare_template_context(
        ...
        Args:
            ...
            uart_data: UART metadata

        Returns:
            Template context dictionary
        """
        # Extract template variables
        template_vars = uart_data.get("template_variables", {})
        ...
        context = {
            ...
            # UART-specific
            "uart": {
                "name": uart_data["peripheral"]["name"],
                "type": uart_data["peripheral"]["type"],
                "description": uart_data["peripheral"].get("description", ""),
                "instances": uart_data["instances"],
                "registers": uart_data["registers"],
                "capabilities": uart_data.get("capabilities", {}),
                "style": template_vars.get("style", "stm32"),
                "register_type": template_vars.get("register_type", "USART_TypeDef"),
                "baud_rates": uart_data.get("uart_specific", {}).get("baud_rates", []),
                "data_bits": uart_data.get("uart_specific", {}).get("data_bits", []),
                "parity": uart_data.get("uart_specific", {}).get("parity", []),
            },
            ...
            "generator_name": "uart_generator.py",
            "generator_version": "1.0.0",
            "metadata_file": f"tools/codegen/metadata/platforms/{platform_name}/uart.json",
        }

        return context

    def generate_uart(self, platform_name: str) -> bool:
        """
        Generate UART code for a platform.
        ...
        """
        print(f"📦 Generating UART for {platform_name}...")

        try:
            # Load metadata
            platform_data = self.load_platform_metadata(platform_name)
            uart_data = self.load_uart_metadata(platform_name)

            # Validate
            if not self.validate_metadata(uart_data, "peripheral"):
                return False
            ...
            # Load template
            try:
                template = self.jinja_env.get_template("platform/uart.hpp.j2")
            except TemplateNotFound:
                print(f"❌ Template not found: platform/uart.hpp.j2")
                return False
            ...
            output_file = output_dir / "uart.hpp"
            ...
            print(f"❌ Error generating UART: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def list_platforms(self) -> List[str]:
        """
        List all available platforms with UART metadata.
    ...
    def generate_all(self) -> bool:
        """
        Generate UART code for all platforms.
        ...
        if not platforms:
            print("❌ No platforms with UART metadata found")
            return False
        ...
        for platform in platforms:
            if not self.generate_uart(platform):
                success = False
            print()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate UART hardware policies from metadata",
...
Examples:
  ...
  List available platforms:
    ...
    # Create generator
    generator = UartGenerator(project_root)
    ...
    success = generator.generate_uart(args.platform)
    sys.exit(0 if success else 1)
